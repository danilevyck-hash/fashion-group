...
const P = [
  ...
  {
    // 🩸 El estado que de verdad podía ensanchar: tres campos de texto dentro
    // de un cuadro que a 390 px tiene ~326 px útiles.
    id: "ayuda-editando",
    titulo: 'Mobiliario — el "?" con un renglón EN EDICIÓN',
    url: "/marketing/mobiliario",
    espera: 9000,
    async preparar(page) {
      const b = page.locator('[aria-haspopup="dialog"]').first();
      ...
      await b.click({ timeout: 8000 }).catch(() => {});
      await page.waitForTimeout(2500);
      const editar = page.getByRole("dialog").getByRole("button", { name: /^Editar el precio de/ }).first();
      if (!(await editar.count())) return false;
      await editar.click({ timeout: 8000 }).catch(() => {});
      await page.waitForTimeout(900);
      return (await page.getByRole("dialog").getByRole("button", { name: "Guardar" }).count()) > 0;
    },
  },
  {
    id: "ayuda-borrando",
    titulo: 'Mobiliario — el "?" confirmando un borrado',
    url: "/marketing/mobiliario",
    espera: 9000,
    async preparar(page) {
      const b = page.locator('[aria-haspopup="dialog"]').first();
      ...
      await editar.click({ timeout: 8000 }).catch(() => {});
      await page.waitForTimeout(900);
      const borrar = page.getByRole("dialog").getByRole("button", { name: "Borrar" }).first();
      if (!(await borrar.count())) return false;
      await borrar.click({ timeout: 8000 }).catch(() => {});
      await page.waitForTimeout(900);
      // Se llega hasta la confirmación y NO se toca "Sí, borrar": este script
      // es de solo lectura y no puede borrarle un precio a Daniel.
      return (await page.getByRole("dialog").getByRole("button", { name: "Sí, borrar" }).count()) > 0;
    },
  },
  {
    id: "ayuda-agregando",
    titulo: 'Mobiliario — el "?" agregando un precio nuevo',
    url: "/marketing/mobiliario",
    espera: 9000,
    async preparar(page) {
      const b = page.locator('[aria-haspopup="dialog"]').first();
      ...
      await b.click({ timeout: 8000 }).catch(() => {});
      await page.waitForTimeout(2500);
      const agregar = page.getByRole("dialog").getByRole("button", { name: "+ Agregar precio" }).first();
      if (!(await agregar.count())) return false;
