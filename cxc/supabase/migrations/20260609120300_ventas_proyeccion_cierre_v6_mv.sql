-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: ventas_proyeccion_cierre_v6  →  totales historicos desde la MV
--
-- Tier 2 perf. Clon de 20260529000600 (v6) con 3 CTEs swapeadas a
-- ventas_rollup_mensual_mv: `totales` (totales multi-ano para tasas de
-- crecimiento), `total_prev` (ano previo full) y `empresas_set` (set de
-- empresas). Todos son rangos de anos CERRADOS / set estable -> la MV es exacta
-- al centavo.
--
-- LO QUE NO SE TOCA (queda EN VIVO, grano-dia, sin regresion de frescura):
--   - dia_ventas / ytd_cur / ytd_prev_sp: el YTD same-period necesita corte por
--     dia (d <= fecha_corte). La MV mensual no puede expresarlo. Sigue leyendo
--     switch_facturas + ventas_raw en vivo -> ytd sync-fresh (corte = hoy).
--   - v_fecha_corte: MAX(fecha) de switch_facturas del ano en curso, en vivo.
--
-- Mismo nombre (v6) y shape de salida -> sin cambio en src/lib/ventas/queries.ts.
--
...
BEGIN
  ...
  totales AS (
    SELECT empresa_key AS empresa, anio, SUM(ventas_netas)::numeric AS total
  ...
  empresas_set AS (
    SELECT DISTINCT empresa_key AS empresa FROM ventas_rollup_mensual_mv WHERE empresa_key IS NOT NULL
  ),
  ytd_cur AS (
