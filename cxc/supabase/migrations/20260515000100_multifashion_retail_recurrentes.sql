-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: multifashion_retail_recurrentes(p_year int, p_limit int)
--
-- Sirve a la sección "Retail recurrentes" del sub-tab Clientes Multifashion
-- (Bloque 2 polish). Lista clientes retail (no wholesale) con ≥ 2 tickets
-- en el año, excluyendo CONTADO / CONSUMIDOR FINAL / vacíos.
--
-- Mismo shape que multifashion_wholesale_clientes pero:
--   - is_wholesale = false (retail mostrador)
--   - Exclusión de clientes genéricos
--   - HAVING COUNT(*) >= 2 (al menos 2 tickets para "recurrente")
--   - Top p_limit (default 30) ordenado por total YTD desc
--   - Cada cliente trae meses[12] para sparkline
--
...
BEGIN
  -- ── Filtrar universo: retail (is_wholesale=false), no genéricos,
  --    al menos 2 tickets en el año ─────────────────────────────────────
  WITH base AS (
    SELECT
      cliente,
      subtotal,
  ...
  -- Totales agregados de la sección (incluye solo el top p_limit retornado).
  SELECT
