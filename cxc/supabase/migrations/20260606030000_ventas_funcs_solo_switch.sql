-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: Paso 4a (proyecto Fuente Única API) — ventas_dashboard_prev_same_period
-- y ventas_proyeccion_cierre_v6 dejan de leer ventas_raw para VENTAS.
-- Cambio quirúrgico: el CTE dia_ventas (blend switch >= 2025-05-01 UNION ventas_raw
-- < 2025-05-01) pasa a SOLO switch_facturas (historia completa backfilleada).
-- El costo (dia_costo / vistas de costo) NO se toca: sigue de ventas_raw (Opcion A).
-- Cuerpos idénticos a sus definiciones vigentes salvo ese bloque.
