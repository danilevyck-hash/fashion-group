-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: multifashion_dia_a_dia → v4 (escape hatch runtime stale)
--
-- Síntoma reportado: el endpoint /api/multifashion/mes-en-curso devuelve
-- dia_actual=12, pero ejecutar la RPC `multifashion_dia_a_dia(2026, 5)`
-- directo en SQL devuelve dia_actual=9. pg_proc confirma una sola versión
-- con HAVING SUM(subtotal) > 0 (v3 aplicada). Código TS y route.ts usan
-- data.dia_actual directo sin override (verificado por grep).
--
-- Causa raíz: mismo bug runtime stale (Vercel/PostgREST) que ya nos pegó
-- 4 veces antes con multifashion_vendedoras y multifashion_mensual.
-- pg_proc tiene el body nuevo pero el runtime serve sigue sirviendo el
-- viejo desde caché interna.
--
-- Solución: renombrar la función a _v4 y dropear _v3 / _v2 / _v1.
-- Forzar a PostgREST + Vercel a recompilar/re-resolver el nombre.
-- Body idéntico a v3 (HAVING SUM > 0 incluido).
--
-- Aplicar manual en Supabase Dashboard → SQL Editor.
-- ─────────────────────────────────────────────────────────────────────────────

DROP FUNCTION IF EXISTS multifashion_dia_a_dia(int, int);

CREATE OR REPLACE FUNCTION multifashion_dia_a_dia_v4(p_year int, p_mes int)
RETURNS jsonb
...
BEGIN
  ...
  -- v_dia_actual = MAX(día con SUM(subtotal) > 0) — días con solo NCs o
  -- subtotal neto 0 NO desplazan el corte.
...
GRANT EXECUTE ON FUNCTION multifashion_dia_a_dia_v4(int, int) TO service_role;
