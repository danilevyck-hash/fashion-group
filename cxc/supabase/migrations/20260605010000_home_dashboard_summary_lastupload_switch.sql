-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: home_dashboard_summary — lastUpload desde switch_estadocuenta
--
-- El campo 'lastUpload' (timestamp de ultima actualizacion de CXC en el
-- dashboard) leia cxc_uploads.uploaded_at (CSV legacy de Angela). cxcTotal y
-- cxcVencida ya migraron a switch_estadocuenta_aging; este era el ultimo
-- consumidor de cxc_uploads en el dashboard. Ahora 'lastUpload' = MAX(synced_at)
-- de switch_estadocuenta (sync API diario). Resto de la funcion IDENTICO.
--
-- CREATE OR REPLACE in-place (misma firma) como todas las versiones previas de
-- home_dashboard_summary; el frontend no cambia.
--
