-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: ventas_proyeccion_cierre_v5 — cierre año anterior + delta.
--
-- Cambio vs v4:
--   Agrega comparativo proyección vs cierre del año anterior. La meta deja
--   de ser el medidor principal del Resumen y pasa a ser solo referencia
--   en /ventas/metas. El nuevo "baseline" es la realidad de 2025.
--
-- Campos nuevos por empresa:
--   cierre_anio_anterior          — total ventas año p_anio-1 (alias de ventas_prev_year
--                                   pero con nombre explícito para la UI nueva).
--   delta_vs_anio_anterior        — proyeccion_cierre - cierre_anio_anterior.
--   delta_vs_anio_anterior_pct    — delta / cierre (NULL si cierre=0).
--
-- Campos nuevos en totales_grupo:
--   cierre_anio_anterior_total
--   delta_vs_anio_anterior_total
--   delta_vs_anio_anterior_pct
--
-- Todo lo que devolvía v4 se preserva 1:1 (meta_anual_manual, meta_sugerida,
-- meta_efectiva, ritmo_actual, status, etc.).
--
-- Aplicar manual en Supabase Dashboard → SQL Editor.
-- ─────────────────────────────────────────────────────────────────────────────

DROP FUNCTION IF EXISTS ventas_proyeccion_cierre_v5(int);

CREATE OR REPLACE FUNCTION ventas_proyeccion_cierre_v5(p_anio int)
RETURNS jsonb
...
BEGIN
  ...
  IF v_fecha_corte IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'totales_grupo', jsonb_build_object(
        ...
        'delta_vs_anio_anterior_total', NULL,
        'delta_vs_anio_anterior_pct', NULL
      )
    );
  END IF;
  ...
  sugerida_calc AS (
    SELECT
      r.empresa, r.v_prev, r.ritmo_historico, r.historia_disponible,
  ...
  base AS (
    SELECT
      ...
      s.meta_sugerida,
      CASE WHEN COALESCE(ys.ventas_prev_ytd_sp, 0) > 0
        THEN yc.ventas_ytd / ys.ventas_prev_ytd_sp ELSE NULL
      END AS ritmo_actual,
      CASE WHEN COALESCE(tp.total_prev_year, 0) > 0
        THEN ys.ventas_prev_ytd_sp / tp.total_prev_year ELSE NULL
      END AS frac_ytd
    FROM empresas_set e
  ...
  SELECT
    jsonb_agg(
      jsonb_build_object(
        ...
        -- v5: alias semánticamente claro de ventas_prev_year (= cierre real
        -- del año anterior completo). UI nueva consume estos campos.
        'cierre_anio_anterior', COALESCE(f.ventas_prev_year, 0),
        'delta_vs_anio_anterior',
          CASE WHEN COALESCE(f.ventas_prev_year, 0) > 0
            THEN f.proyeccion_cierre - f.ventas_prev_year
            ELSE NULL END,
        'delta_vs_anio_anterior_pct',
          CASE WHEN COALESCE(f.ventas_prev_year, 0) > 0
            THEN (f.proyeccion_cierre - f.ventas_prev_year) / f.ventas_prev_year
            ELSE NULL END,
  ...
  SELECT jsonb_build_object(
    ...
    -- v5: cierre año anterior agregado del grupo + delta proyección vs ese cierre.
    'cierre_anio_anterior_total',   COALESCE(g2.cierre_total_prev, 0),
    'delta_vs_anio_anterior_total',
      CASE WHEN COALESCE(g2.cierre_total_prev, 0) > 0
        THEN g2.proyeccion_cierre - g2.cierre_total_prev ELSE NULL END,
    'delta_vs_anio_anterior_pct',
      CASE WHEN COALESCE(g2.cierre_total_prev, 0) > 0
        THEN (g2.proyeccion_cierre - g2.cierre_total_prev) / g2.cierre_total_prev
        ELSE NULL END,
    'status', CASE
...
GRANT EXECUTE ON FUNCTION ventas_proyeccion_cierre_v5(int) TO service_role;
