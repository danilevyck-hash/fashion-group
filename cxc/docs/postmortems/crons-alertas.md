...
- 🔔 **Solo 3 (+2) alertas de SISTEMA**, y la lista es cerrada: *un dato que mirás está viejo* (+24 h) · *algo se rompió y no se arregló solo* (2 fallos seguidos del mismo par `(empresa, sync_type)`) · *la base está en problemas* (>80% de memoria) · *el reloj de asistencia tiene un hueco que ya no entra solo* · **(+1) *el lector de facturas dejó de leer*** (11-sep-2026 — ver abajo).
...
- 🔴 **HAY TAREAS CUYO PRODUCTO ES EL MENSAJE, y vuelven a vigilarse** (7-sep-2026). Daniel: *«Siempre que me llega un telegrama me dices que es falsa alarma. Quiero que me lleguen de veras.»* El vigía de crons se apagó el 30-jul y estuvo bien —medía el MECANISMO—, pero dejó sin cobertura a los crons para los que «¿el dato está viejo?» no se puede preguntar: si `cheques-alert` deja de correr, el mensaje de las 9 a.m. no llega, ninguna pantalla queda desactualizada y **nada lo dice**. Son **cinco** desde el 11-sep-2026 (eran seis: `prestamos-caducan` se retiró con la aprobación de préstamos), en UNA sola lista (`src/lib/alertas/crons-que-avisan.ts`): los cuatro cuyo producto ES el mensaje (`cheques-alert` · `guias-pendientes` · `acs-resumen-diario` · `grupo-resumen-mensual`) más **`acs-fidelizacion`**, que NO manda ningún mensaje pero **no registra su corrida** —medido: 0 filas suyas en `switch_sync_log` en 90 días— así que ni la regla 2 ni A ni B pueden verlo. 🔴 **No es una regla nueva**: es la regla 1 mirando la frescura del AVISO. 🔴 **Los syncs de Switch NO vuelven** — hay candado que exige que ningún cron de la lista escriba en `switch_sync_log`, comprobado contra `SYNC_TYPES_POR_CRON`. Reusa el umbral que ya existe (`cronIsStale`, con los **33 días** del resumen mensual) y el freno de la alerta fantasma (`staleEsPendingRecovery`, la 2ª entrada de `acs-fidelizacion` a las 16:30). Anti-loop de 7 días por cron, **cero entradas nuevas de cron**: cuelga de la reconciliación.
- 🩸 **LA REGLA 2 NO TENÍA ANTI-LOOP: la cartera de Boston mandó CINCO mensajes casi idénticos, uno por día** (20 al 25-ago-2026). Ahora hay una llave por **(par, arranque de la racha)** con **48 h** entre repeticiones — y 48 y no 24 porque `boston-cartera` corre UNA vez al día, así que con 24 h volvía a avisar todos los días igual; ni 7 días, que es demasiado silencio para algo que se arregla hoy. **El primer mensaje de una avería no se demora nada** y una avería NUEVA suena en el acto (la llave lleva el arranque adentro). Y el umbral de fallos seguidos ahora depende del **RITMO del par**: **3 fallos desde 5 corridas/día** (las ventas), 2 para todo lo demás. 🔴 **Cinco y no cuatro, y el número salió de la medición**: con el corte en 4 entraban los recibos y el backtest perdía una avería real (`active_shoes/recibos`, 21-jun-2026, volvió recién 24 h después). El «cuántas veces al día» se **DERIVA del cronograma** (`corridasPorDiaDelPar`, contando `SWITCH_CRON_ENTRADAS`), nunca se escribe a mano. **Backtest de 90 días: 20 mensajes → 13, con las 10 averías reales intactas y 0 perdidas.**
- 🩸 **El dedup de las alertas A y B se marca DESPUÉS de que Telegram confirme** (7-sep-2026). Iba antes, y un envío fallido **quemaba los 7 días de silencio** de ese módulo: la fila puesta, el mensaje nunca enviado, nadie enterado hasta la semana siguiente. Mismo orden que `cheques.aviso_vencido_en`. Vale también para el anti-loop nuevo de la regla 2 y para el de los crons. ⚠️ La **regla 1** (`datos-frescos.ts`) sigue marcando antes: su ventana es de 20 h, así que un envío fallido cuesta a lo sumo un día de silencio y no una semana — queda pendiente de unificar.
- ⚠️ **`switch_recibos` y `switch_ingresos_mercancia` NO entran a la alerta B, y está medido** (7-sep-2026). Recibos **escribe solo lo que cambió**: con todo sano y el cron corriendo 4×/día, **7 de las 8 empresas** llevan más de las 40 h del umbral (active_wear **263 h**, joystep 95, cuatro en 67) y el peor hueco histórico de joystep es de **581 h**. Ingresos sí reescribe su ventana entera, pero la ventana son **45 días** y la compra es un hecho del negocio que puede no ocurrir: el hueco más largo sin una sola compra es **452 días** en active_wear, 206 en joystep, 139 en active_shoes — cinco de las seis por encima de la ventana, o sea que la ventana vacía no escribe nada y la alerta sonaría para siempre. Hay candado con los números adentro para que nadie los agregue sin volver a medir.
- ⚠️ **El resumen «Switch estuvo caído… sin impacto» NO va a Telegram desde el 27-jul-2026** y ahora hay candado. Medido: 3 filas en 90 días (2, 4 y 13 de agosto), las tres solo en `cron_email_errors`. Un aviso que te dice que no pasó nada entrena a ignorar los que sí.
- Candados de los cuatro arreglos: `alertas-que-llegan.test.ts` (29 casos); **35 mutaciones, 35 cazadas** con 2 controles (`scripts/_mutar-candados-alertas-que-llegan.sh`); medición `scripts/_medir-alertas-cuatro-arreglos.mjs` y `scripts/_medir-alertas-90-dias.mjs`.
- 🩸 **EL LECTOR DE FACTURAS DEJÓ DE LEER Y NADIE SE ENTERÓ** (11-sep-2026). La `ANTHROPIC_API_KEY` de producción estuvo **inválida un tiempo que no se puede medir** —no hay tabla que lo registre— y los DOS lectores de PDF fallaron en silencio: Marketing (*«+ Registrar gasto»* y la pantalla del proyecto) y Reclamos. Las dos pantallas hacían lo correcto —«no se pudo leer», sigue tecleando a mano— y por eso **nadie reportó nada y nadie rotó la llave**: el sistema se degradó a la versión de antes de la IA sin decir una palabra. Daniel, textual: *«si se me acaba o algo que me llegue notificación a telegram»*. 🔴 **No es una regla nueva**: es la regla 2 («algo se rompió y no se arregló solo») mirando un servicio de afuera en vez de un cron — lo que crece es UNA superficie vigilada, y por eso la lista dice «(+1) el lector de facturas dejó de leer».
  - 🔴 **AVISAN TRES CAUSAS Y NADA MÁS** (`clasificarFalloAnthropic`, módulo puro `src/lib/alertas/lector-facturas.ts`): **llave que no sirve** (401/403 `authentication_error`/`permission_error`, y también la llave AUSENTE — desde afuera se ve idéntico y la pantalla que hay que abrir es la misma), **crédito agotado** (402, o el 400 cuyo MENSAJE dice `credit balance`/`billing`; se mira el texto y no el status justamente porque el 400 es el mismo de un PDF que no se pudo procesar) y **tope de uso persistente** (429 / `rate_limit_error`).
  - 🔴 **LO QUE NO AVISA ES LA MITAD DEL DISEÑO**: un PDF ilegible, un 400 de documento, un timeout, un 500 o un 529 «overloaded» **no suenan**. Se arreglan volviendo a intentar y la pantalla ya se lo dice a quien está ahí parado; una alerta por cada PDF raro convertiría este aviso en ruido y el día que de verdad se acabe el crédito ya nadie lo leería.
  - 🔑 **«Persistente» está medido, no supuesto**: el cliente fija `maxRetries: MAX_REINTENTOS` (2) **a propósito y no por herencia del default**, así que un 429 que llega al aviso ya se reintentó dos veces con espera. Si ese número se toca, hay que volver a pensar el mensaje de esa causa.
  - 🔴 **UN SOLO PUNTO DE LLAMADA A ANTHROPIC**: `src/lib/ia/anthropic.ts` (`leerPdfConAnthropic`). Antes cada ruta armaba su cliente, su `try/catch` y su `console.error` — dos copias del mismo silencio. **Ni un prompt ni un modelo cambiaron**: cada lector sigue mandando los suyos y el cliente no elige ninguno (hay candado que prohíbe que un `claude-…` aparezca ahí). El error se **vuelve a lanzar** tal cual: las dos rutas contestan el mismo 500 de siempre y quien sube la factura ve exactamente lo de antes — el aviso es un COLATERAL, no cambia la pantalla.
  - 🔴 **Anti-loop de 7 días POR CAUSA** (`cron_email_errors.tipo` = `lector_facturas:<causa>`, la misma llave del guard de montos y del silencio de datos), y **el dedup se marca DESPUÉS de que Telegram confirme** — marcarlo antes y que el envío falle quemaría los siete días de silencio de esa causa, o sea el único aviso que iba a haber. La causa va en la llave para que una llave vencida no tape que después se acabe el crédito. Fail-OPEN: si no se puede leer el registro, se avisa igual. Y **avisar NUNCA lanza**: cuelga del camino de alguien que está subiendo un PDF.
  - El mensaje trae la regla de tres y manda a la pantalla exacta: **llave** → console.anthropic.com › API Keys y después Vercel (`ANTHROPIC_API_KEY`, Production); **crédito** → console.anthropic.com › Billing, *«la llave no hay que cambiarla»*; **tope** → esperar y, si sigue, console.anthropic.com › Limits. Los tres dicen que **no se perdió nada y la factura se sigue escribiendo a mano**.
  - Candado: `lector-facturas-avisa.test.ts` (29 casos) — las tres causas, el CONTROL de que un 500 de PDF y un 529 no avisan, el anti-loop en las cuatro direcciones (avisa · no repite · Telegram no confirma → no marca · fail-open) y el barrido que exige que **solo `lib/ia/anthropic.ts` importe el SDK**.
...
## Alertas a Telegram — DOS canales, TRES formas de mandar (movido desde CLAUDE.md el 14-sep-2026, verbatim)
