...
>
> ### El cliente que Switch borró se seguía ofreciendo (4-sep-2026)
>
> **Medido:** 147 códigos del grupo en `switch_clientes`; **2 con `activo = false` en las 6 empresas** — `D-30` «City Moda Chorrera» (el duplicado de D-26 que confundió a Daniel, borrado en las 6 el 13-ago-2026) y `D-135` «Rey Store (Aguas)». `switch_clientes` los notaba desde julio (`sync-empresa` marca `activo=false` + `ausente_desde` con guard de lista completa), pero `clientes_master` se refrescaba con un **upsert puro** que nunca marcaba lo que dejó de llegar: los dos se ofrecían en el ClientePicker de Guías, Cheques y en el selector de pedidos. Ninguno tenía guías, cheques, pedidos ni CXC vivos (verificado contra producción).
>
> **Lo aprobado por Daniel (textual: «APROBADO»):** deja de ofrecerse al buscar pero NO se borra (guías y facturas viejas siguen con su nombre normal); la ficha lo muestra con «Ya no está en Switch» y desde cuándo; ausente = **ninguna** de las 6 lo manda (vivo en una sola → vivo); y vuelve solo si Switch lo manda de nuevo.
>
> **Cómo quedó:** columna nueva `clientes_master.ausente_desde` (migración `20260919120000`, **pendiente de aplicar** — mientras tanto todo funciona igual que hoy, fail-open). NO se reusó `deleted`: `deleted` significa «no existe» (lo filtran el índice UNIQUE de nombre, las vistas 12m, la ficha con 404) y un ausente tiene que seguir existiendo. La marca la escribe SOLO `sync-clientes-master` reusando el veredicto de `switch_clientes.activo` — la corrida buena se distingue en capas: `activo` solo cambia con una lista de Switch **completa y no vacía**; una lectura fallida devuelve `ok:false` sin marcar; sin datos de `activo` la pasada se omite; y un **freno** (>10% del directorio saldría ausente → no se marca a nadie) para el día que un dato roto aguas arriba diga «ausente masivo». El default de `leerClientesDelGrupo` ahora excluye ausentes (Guías, Cheques, chips de «más usados» y el candado de atar quedan cubiertos sin tocar cada caller); `/api/clientes` y la lista van con `incluirAusentes: true` porque de ahí salen la ficha con rótulo y los nombres de las guías viejas. El selector de pedidos de catálogo (que ofrece `switch_clientes` por empresa) filtra `activo = true`.
>
> **Medido antes/después:** hoy se ofrecen 150; al aplicar la migración y correr el sync quedarían **148** (los 2 marcados = 1,4% del directorio, lejos del freno). Candados: `clientes-ausentes-de-switch.test.ts` (16) + `clientes-ausentes-selector-y-ficha.test.tsx` (5); **14 mutaciones, 14 cazadas** (`scripts/_mutar-candados-clientes-ausentes.sh`) — incluida «una corrida fallida marca» que SOBREVIVIÓ en la primera corrida porque el fallo se disfrazaba de «espejo vacío»: se cerró exigiendo que el error diga QUÉ falló.
...
>
> **Decidido el 3-sep-2026.** Daniel: *«esconder rey stoute. si capitiliza reynaldo.»* y, sobre la primera versión del cambio (que solo escondía la fila): *«te dije que eliminaras Rey Stoute Aguas.»* (1) **Retirado**, no escondido: `VENDEDORES_OCULTOS` salió de la vista y vive como `VENDEDORES_RETIRADOS` en `src/lib/comisiones/retirados.ts` (un solo lugar; `REY STOUTE AGUAS` + la grafía `AGUAS`; `estaRetirado()` compara por el canónico, pasando por `aplicarAlias`). Lo leen la matriz, Por empresa, las tarjetas, el modal de detalle, los tres Excel y Configuración (tabla de tasas y desplegable); nada de él en tablas ni en totales; el servidor contesta 400 «Ese vendedor ya no está en Comisiones.» a una tasa o una exclusión a su nombre; su fila de tasa se desactiva con `20260916120000_retirar_rey_stoute_aguas.sql` (`activo = false`, nunca DELETE); el alias se queda; sus 4 facturas siguen en Switch. Medido con la RPC real: 2026 = 6,89 + 5,66 + 11,62 + 4,58 + 3,49 + 4,30 + 8,34 + 3,55 + 1,40 = **49,83**, todo Vistana; total pagable 82.109,56 → 82.059,73. (2) `nombreVendedorEnPantalla` en las dos tablas, las tarjetas, el detalle (título y encabezado impreso) y el Excel; la clave sigue en mayúsculas y 0 números cambian. Candado `comisiones-retirados-y-mayusculas.test.tsx`; 32 mutaciones, 32 cazadas.
>
> ### La pantalla
>
...
- 🩸 **Ventas › Resumen ya NO lleva píldora «Sincronizado»** (4-sep-2026). Vigilaba **3 empresas de 8** —`SWITCH_FACTURAS_EMPRESA_KEYS`, una lista escrita a mano que se quedó atrás del cron— y pintaba de VERDE un Resumen con Vistana o Fashion Wear congeladas. Daniel: *«¿de qué sirve tenerlo si ya el sistema corre fluido y si no me avisa por Telegram para arreglarlo?»*. Quien vigila las ventas es `src/lib/datos-frescos.ts`, que **DERIVA** su lista de `empresasConFacturas()` —las 8— y avisa por Telegram a las +24 h, más la regla de dos fallos seguidos. La constante se retiró; el candado (`textos-pendientes-284.test.ts`) cambió de dirección y trae CONTROL de que el Resumen sí se pinta.
...
- 🔴 **El botón «Actualizar datos de Switch» de Referencia lo ve TODO el que entra al módulo** (4-sep-2026). 🩸 **Y desde el 11-sep-2026 SE VE DESDE QUE ABRES**: se dibujaba dentro del bloque de resultados, así que quien entraba y no buscaba nada no lo encontraba — justo lo contrario de lo que dice el aviso de novedades que esa persona tiene en pantalla. ⚠️ **Sin búsqueda no hay qué actualizar, y se DICE** («Busca un código primero — se actualiza la empresa de lo que encuentres»), como TEXTO a la vista y no en un `title`: el catálogo se trae **por empresa** y actualizar las 6 a ciegas no es opción (vistana sola midió **155 s / 8.122 artículos**, y son sesiones de Switch que tumban al cron de esa empresa). El Excel sigue apareciendo solo con resultados. Daniel: *«activa el botón de Referencia»*, *«referencia lo puede ver todos, y sin aviso»*. La ruta (`POST /api/ventas/referencia/actualizar`) siguió viva mientras su botón desapareció en el rediseño del 11-ago (`9b1899e1`) — colateral, no una decisión. Los roles salen de **`REFERENCIA_ROLES`** (`src/lib/ventas/referencia.ts`), UNA lista para la página, la búsqueda y el botón; tres copias a mano fue lo que dejó el POST en solo-admin. **Sin aviso** en pantalla, y con el **acelerador de Guías** (`SYNC_NOW_COOLDOWN_MIN` = 10 min, contra `switch_sync_log`): dos toques seguidos no abren dos sesiones de Switch. Candado: `referencia-boton-actualizar.test.ts`.
...
- **Las 6 empresas del grupo comisionan igual** (`comision_b2b_v8`, vía `lib/comisiones/rpc` con red a la v7, la v6 y la v5 mientras cada DDL no corra — la respuesta dice `version`, `exclusiones_aplicadas` y `alias_aplicado`): 0,5% sobre la **VENTA** de las facturas con `pct_utilidad > 20`. La utilidad es el **criterio de entrada**, no la base. Retenciones y `TCKCTA` quedan fuera.
- 🔴 **Hay CLIENTES QUE NO COMISIONAN para un vendedor concreto (3-sep-2026).** Daniel: *«crea configuración en comisiones para desactivar cálculos de clientes»* — grano **(empresa, cliente, vendedor)**, *«cliente vendedor»*, y aplica a venta y cobro: *«correcto, también venta»*. Tabla `comision_exclusion` (código del cliente + vendedor en `UPPER(TRIM())`, **soft delete firmado, nunca DELETE**, única solo entre ACTIVAS, RLS service_role), `comision_b2b_v7` = v6 byte a byte + `LEFT JOIN … WHERE ce.id IS NULL` en `ventas` y `cobros` (el detalle excluye lo mismo). Si otro vendedor le vende o le cobra al mismo cliente, ese otro **sí** comisiona. Se administra en **Comisiones › Configuración** (pestaña a pantalla completa del módulo `/comisiones`, **solo admin**, no está en la pestaña Comisiones de Ventas): «Tasas por vendedor» (venta y cobro por separado) y **«Clientes que no comisionan»** — nunca se dice «exclusión» en pantalla. Migración `20260912120000` (**aplicada el 3-sep-2026**) cargó las 11 de Daniel (17 filas, una por grafía de Reinaldo) y fijó a Reinaldo en **1 % / 1 %** (ya estaba así en producción desde el 26-ago). Medido ene–sep 2026: solo Reinaldo se mueve, Active Shoes −447,67 · Active Wear −151,19 · grupo −598,86. Candados: `comision-exclusion-v7.test.ts` (el SQL corriendo en pglite cuando está instalado) · `comisiones-configuracion-pantalla.test.tsx`; **29 mutaciones, 29 cazadas** (`scripts/_mutar-candados-comision-exclusiones-v7.sh`); medición `scripts/_medir-comision-exclusiones-v7.mjs`.
- 🔴 **UNA PERSONA, UNA FILA, UNA TASA — alias de vendedor (3-sep-2026, noche).** Daniel: *«¿por qué hay 4 Reinaldo?»*, *«llámalo Reynaldo y no Reinaldo»*. Switch manda el nombre de la misma persona distinto según la empresa y con tipeos (`REINALDO` · `REYNALDO` · `REINDALDO` · `REINDALDO ` con espacio), y `AGUAS` = `REY STOUTE AGUAS`. Tabla **`comision_vendedor_alias`** (`nombre_switch → vendedor_canonico`, en `UPPER(TRIM())`) + función **`comision_vendedor_canonico(text)`** (con alias la persona; sin alias el nombre **solo recortado**, así «Rodrigo» sigue cruzando). **Todo lo que agrupa por vendedor pasa por ahí primero**: `comision_b2b_v8` (= v7 con el alias, candado que compara los cuerpos), `comision_b2b_detalle` v5, la tabla de tasas (colapsó 9 filas → 5, `REYNALDO ESPINOSA` 1 %/1 %, trigger que canonicaliza lo que entre), las exclusiones (17 → 11 activas, las 6 repetidas apagadas con soft delete `migracion-alias-v8`, trigger), los descuentos fijos, y el lado Node (`aplicarAlias` en `lib/comisiones/alias.ts`, espejo de la función, fallando abierto). El canónico es **REYNALDO con Y**; en la pestaña Configuración se muestra capitalizado (`nombreVendedorEnPantalla`: «Reynaldo Espinosa»), sin nota de «N nombres». **Daniel Levy no aparece en «Tasas por vendedor»** (*«quítalo»*) — sigue en `VENDEDORES_SIN_PAGO` y en la tabla de comisiones, gris. Migración `20260913120000` (**aplicada el 3-sep-2026**). Medido ene–sep 2026 con el SQL real, por persona: **0 celdas cambian** (136 filas → 123; grupo 90.190,93 = 90.190,93); la fila de tasa `REINDALDO` con cobro **0 %** era un error que solo tocaba 2023 (+1.017,31) y 2024 (+327,79). 🔴 **Rey Stoute Aguas está RETIRADO de Comisiones** (3-sep-2026). Daniel: *«esconder rey stoute. si capitiliza reynaldo.»* y, corrigiendo la primera versión del cambio, *«te dije que eliminaras Rey Stoute Aguas.»* La matriz lo escondía por el nombre `AGUAS` dentro de la propia vista; con el alias llegó como `REY STOUTE AGUAS` y la fila volvió ($49,83 en 2026, todo en Vistana). Ahora la lista vive en **UN solo lugar, `src/lib/comisiones/retirados.ts`** (`VENDEDORES_RETIRADOS` = `REY STOUTE AGUAS` + la grafía `AGUAS` por si el alias falla abierto; `estaRetirado()` compara por el **canónico**, pasando el nombre por `aplicarAlias` primero) y la leen TODAS las superficies: matriz, Por empresa, tarjetas, modal de detalle, los tres Excel y Configuración (ni en «Tasas por vendedor» ni en el desplegable). **Ni en tablas ni en totales** (medido: total pagable 2026 82.109,56 → 82.059,73, exactamente su fila), y en pantalla no se dice nada de él: no existe. El servidor **rechaza con mensaje** (`AVISO_VENDEDOR_RETIRADO`, 400) una tasa (`PUT config`) o una exclusión (`POST exclusiones`) a su nombre. Su fila en `comision_vendedor_tasa` se **desactiva** (`activo = false`, **nunca DELETE**) con la migración `20260916120000_retirar_rey_stoute_aguas.sql` (aplicada el 3-sep-2026). **El capitalizado (`nombreVendedorEnPantalla`) aplica a las tablas, las tarjetas, el detalle (título y encabezado impreso) y el Excel**, no solo a Configuración: solo cambia cómo se MUESTRA («Reynaldo Espinosa», «Edwin», «Daniel Levy (no se paga)»; la oficina sigue «Oficina (DEFAULT)»); la clave de agrupación, `VENDEDORES_SIN_PAGO`, los descuentos y lo que viaja al detalle y al Excel siguen en mayúsculas, y 0 números cambian. Candados: `comisiones-retirados-y-mayusculas.test.tsx` · `comision-alias-v8.test.ts`; **32 mutaciones, 32 cazadas** (`scripts/_mutar-candados-comisiones-retirados-mayusculas.sh`); medición `scripts/_medir-comisiones-aguas-retirado.mjs`. **`COLABORADOR` también está retirado (3-sep-2026, Daniel: *«quita colaborador»*)**: no es una persona sino el usuario genérico de Switch en Vistana con el que se facturó el mostrador 2023-2025 (1.024 facturas, 827 recibos, todo Vistana, último documento 23-mar-2026); en 2026 le quedan 2 movimientos y la v8 le arma una fila de **−$5,28** (ene +1,72 por un cobro de $343,75 · mar −7,00 por una NC de $1.400), así que al retirarlo el total pagable **sube** 81.866,22 → 81.871,50. Sin tasa, sin exclusión, sin alias, sin descuento fijo: **no hace falta migración**; medición `scripts/_medir-comisiones-colaborador-retirado.mjs`, mutación «se quita COLABORADOR de la lista» agregada al mismo script (33 mutaciones). Candados: `comision-alias-v8.test.ts` · `comisiones-configuracion-pantalla.test.tsx`; **38 mutaciones, 38 cazadas** (`scripts/_mutar-candados-comision-alias-v8.sh`); medición `scripts/_medir-comision-alias-v8.mjs`.
- 🔴 **Las exclusiones distinguen VENTA de COBRO (3-sep-2026, noche).** Daniel: *«poder quitar comisiones en ventas o comisiones sin que tengan que ser de los dos»*, *«las 11 que ya cargamos quedan con las dos marcadas»*, *«arranca con las dos marcadas pero yo deselecciono»*. `comision_exclusion.excluye_venta` / `excluye_cobro` (`DEFAULT true`, CHECK «al menos una»); la v8 mira `excluye_venta` en `ventas` y `excluye_cobro` en `cobros` (el detalle igual). En pantalla dos casillas por fila (se cambian al momento, `PATCH ?id=`), y una fila con las dos apagadas **no se guarda y se avisa**. La lista va **agrupada por empresa** (encabezado + contador, sin columna Empresa) y el botón **«Configurar» de Por empresa se quitó** (*«configuración en dos lados»*): el chip es la única entrada. Con las dos marcadas las casillas no viajan en el POST (default de la tabla), así el alta sigue funcionando mientras la DDL no corra.
- 🔴 **Tres vendedores, tres papeles (3-sep-2026).** «Vendedor» de la factura (`switch_facturas.vendedor_nombre`) → comisión de **VENTA**. «Vendedor Recibo» = quien REGISTRÓ el pago (`switch_recibos.vendedor_registro`) → comisión de **COBRO**. «Vendedor de cartera» (dueño del cliente, `vendedor_cartera`) → **no alimenta ninguna comisión**. Daniel: *«el que vende a veces no es el que cobra. Edwin puede vender 50k a City Mall y Daniel o DEFAULT cobrar esa plata»*. Migración `20260911120000` (**aplicada el 3-sep-2026**); la v5 (cobro por cartera) se conserva para comparar. Medido ene–ago 2026: grupo +1.253,58 · Reinaldo +2.507,14 · Daniel +1.943,86 · Edwin −2.640,50.
...
- 🔴 **UN DESCUENTO TIENE FECHAS — Y ES PARA EL PRÓXIMO, NO PARA EL DE HOY (6-sep-2026).** 🩸 `comision_descuentos_fijos` tenía OCHO columnas y **ninguna de fecha**, así que un descuento se resta en TODOS los meses, para siempre y **hacia atrás**. Las dos filas vivas son de Reynaldo en Fashion Shoes («Descuento» $1.400,00 + «Descuento de adelanto» $173,08 = **$1.573,08/mes**), creadas el 8-jul-2026 y restándose también en enero–junio. Daniel: *«no sé [qué era] pero hay que descontarlo así mensual»* y, al ver el cuadro del antes/después, **textual: «pero el descuento es indefinido. No hay hasta. Ponlo desde enero que se le descuenta esos 1500 y pico.»** → `desde = 2026-01-01`, `hasta` **NULL**. 🔴 **NINGÚN número de 2026 se mueve**: los 9 meses quedan como están y el total del año sigue siendo **$67.815,75** (medido por persona y por mes, 27 celdas, cero cambios — `scripts/_medir-descuento-vigencia.mjs`). 🔑 **El mecanismo es para el descuento que viene**: el que se cargue en octubre no puede aparecer restado en marzo. La tabla gana `desde` y `hasta`; el «hasta» **corta INCLUSIVE** (mismo criterio que Recordatorios) y es **OPCIONAL — dejarlo vacío es lo NORMAL**: el campo lo dice en su rótulo, la tabla muestra **«Sin fin»** (no «—», que se lee como dato faltante) y guardar sin él no pide ni avisa nada. Sin `desde`, la fila se comporta como siempre, así nada se apaga en silencio mientras la DDL no corra. El grano es el **MES**. Regla en un módulo puro (`src/lib/comisiones/vigencia.ts`), aplicada en `leerDescuentosEfectivos` **antes** de buscar la excepción del mes. Migración `20261007120000` (**pendiente de aplicar**).
- 🔴 **Los descuentos se administran en Comisiones › Configuración, en una TERCERA tarjeta «Descuentos» (6-sep-2026).** 🩸 Antes `comision_descuentos_fijos` **no tenía POST, ni PUT, ni DELETE en ninguna parte de la app**: las 2 filas se escribieron directo en la base y solo asomaban dentro del modal de detalle — la palanca que más plata mueve del módulo era la única que no se podía ver. Daniel: *«sí, minimalista»*. Campos: vendedor · empresa · concepto · monto · desde · hasta, **y nada más** (sin totales, sin explicaciones, sin chips). En pantalla se dice **«Descuentos»**, nunca «descuento fijo». Solo admin, ruta `/api/ventas/comisiones/descuentos-fijos`, **soft delete (`activo = false`), NUNCA DELETE**; el alta REVIVE una fila quitada con la misma llave (el único de la tabla no es parcial por `activo`). ⚠️ La excepción por MES (prender/apagar un descuento en un mes suelto) sigue en `/api/ventas/comisiones/descuentos`, que es otra cosa y otra lista de roles. La pestaña se partió en cuatro archivos (`src/components/ventas/comisiones-config/`): con la tercera tarjeta adentro pasaba las 800 líneas.
- 🔴 **Comisiones abre en el ÚLTIMO MES CERRADO, y el «hoy» es el de PANAMÁ (6-sep-2026).** 🩸 Abría en el mes EN CURSO: medido el 5-sep, con 5 días de septiembre el bruto de las 6 era **$101,77** y el descuento entero se restaba igual, así que lo primero que se veía era **«Total a pagar −$1.471,31»**. Daniel eligió **«a»**: abre en el mes completo anterior y el mes en curso queda a un toque. 🩸 De paso, el mes lo decidía **el reloj del NAVEGADOR** (`new Date().getMonth()+1`), en el shell y en el selector de período; ahora los dos usan `hoyPanama` y el módulo puro `src/lib/comisiones/mes-inicial.ts` (enero abre en diciembre del año anterior). **Ningún cálculo cambia**: solo con qué período abre y qué meses quedan apagados.
- 🩸 **Se QUITÓ el interruptor «Activo» de «Tasas por vendedor» (6-sep-2026).** Daniel: *«quitarlo»*. MEDIDO: **no le quitaba la comisión a nadie** — `comision_b2b_v8` une la tabla de tasas **sin filtrar por `activo`** (el `t.activo = true` de la CTE `universo` solo arma la lista de quien NO vendió ni cobró nada; cualquiera con una venta o un cobro entra igual por el UNION y su tasa se aplica desde un LEFT JOIN sin filtro). Prueba: `REY STOUTE AGUAS` estaba en `activo = false` desde el 4-sep y **siguió comisionando los 9 meses ($49,83)**; solo desaparecía porque el navegador lo filtraba por la lista de retirados. 🔴 **La columna NO se dropea** (patrón de la casa): queda sin lectores ni escritores, y el GET/PUT de `/api/ventas/comisiones/config` dejaron de tocarla. Para sacar a alguien queda **UNA sola forma**: `src/lib/comisiones/retirados.ts`.
- 🔴 **MULTI FASHION HOLDING SALE DEL SQL: SE EXCLUYE POR CÓDIGO (D-108), CON UN COMODÍN DE «TODOS LOS VENDEDORES» (6-sep-2026).** 🩸 Vivía dentro de `comision_b2b_v8` como `f.cliente NOT ILIKE '%multi fashion holding%'` y `r.cliente_nombre NOT ILIKE …`: **203 facturas y 21 recibos de 2026** en las 6 empresas atados a un TEXTO que Switch puede cambiar con una letra, y la única exclusión de cliente que **no se veía en ninguna pantalla**. Va contra la regla de la casa (**la identidad del cliente es el CÓDIGO**). Daniel: *«debe de ser por código, ¿no?»* → sí. Pasa a `comision_exclusion` con `cliente_codigo = 'D-108'`, **venta y cobro**, **las 6 empresas**, y vendedor **`*`** = todos: enumerar los 5 nombres que hoy le venden (DEFAULT · Reynaldo · Edwin · Daniel Levy · Colaborador) no cerraba el agujero, porque **el día que un vendedor nuevo le facture esa factura vuelve a comisionar en silencio**. `*` y no una palabra («TODOS» podría chocar con el nombre real de alguien); en pantalla se dice **«Todos los vendedores»**, nunca el `*`. `comision_b2b_v9` = la v8 **byte a byte** salvo eso, con candado que compara los cuerpos (mismo patrón que `comision-alias-v8`); el comodín **no multiplica filas** (el JOIN sigue LEFT + `ce.id IS NULL`). Migraciones `20261008120000` (**pendiente**). **Medido con la RPC real, ene–sep 2026, las 6 empresas: la comisión por persona y por mes da IDÉNTICA** (56 pares, bruto $90.107,99 = $90.107,99); CONTROL sin las filas de D-108: 16 pares cambian. Medición `scripts/_medir-comision-v9-d108.mjs`; conducta probada con el SQL real en pglite (incluido el control al revés: si Switch le cambia el nombre, **la v8 le paga comisión y la v9 no**).
- 🔴 **El mes NEGATIVO se queda como está.** Reynaldo tiene **−$641,55** en Active Wear en agosto (devoluciones que superaron la venta del mes) y se muestra en rojo, sin una palabra de explicación y sin cambiar el cálculo. Daniel: *«c»*.
- Candados del rediseño (6-sep-2026): `comisiones-descuentos-vigencia.test.ts` · `comision-b2b-v9-por-codigo.test.ts` · `comisiones-mes-cerrado-panama.test.ts` · `comisiones-configuracion-descuentos.test.tsx` · `multifashion-cerrado-y-espejo.test.ts`; **44 mutaciones, 44 cazadas** con 2 controles (`scripts/_mutar-candados-comisiones-rediseno.sh`).
- 🔴 **El costo del Resumen incluye las notas de débito** (3-sep-2026): `switch_articulo_diario` no las trae y Active Wear agosto 2026 salió con costo **negativo** (−$44.483,03; utilidad mayor que la venta). El costo de las ND viene de `switch_factura_utilidad` (`switch_costo_unificado_v2` · `ventas_dashboard_summary_v2` · `ventas_dashboard_prev_same_period_v4`, migración `20260915120000`). **Ninguna lectura de costo del Resumen sale de `switch_costo_diario`** —su último día de cada mes vale $0 para siempre—; esa tabla solo alimenta el **cuadre mensual** (`cuadre-costo.ts`: >2 % y >$100 entre las dos fuentes → 🔧 SISTEMA, anti-loop 7 días por (empresa, mes), sin el último día del mes). Candados: `costo-con-notas-de-debito.test.ts` · `cuadre-costo.test.ts`; 24 mutaciones, 24 cazadas (`scripts/_mutar-costo-notas-de-debito.sh`).
- ⚠️ **Multifashion es OTRO módulo de comisiones y está bien como está — NO fusionar.** 🔴 Desde el 6-sep-2026 es **una opción más del único selector de empresa de Comisiones**, debajo de una línea y con la vista de Vendedoras de su módulo importada TAL CUAL. Daniel, textual: *«y multifashion es una empresa más. Así convive con el módulo, cambio mi opinión de que sea un espejo»* (antes había pedido un espejo en pestaña propia). **Convivir NO es fusionar**: se calculan distinto —el grupo paga 0,5 % solo sobre las facturas con más de 20 % de utilidad; Multifashion, 0,5 % sobre TODA la venta, sin ese filtro— y **nunca aparecen sumadas en un mismo número**. Medido en agosto 2026: Fashion Group **$5.978,55** contra Multifashion **$255,27** (Sheynee 92,67 · Jailine 68,83 · Milagros 67,62 · Jennifer 26,28 · Cindy −0,13). Candado: ni el consolidado ni la vista de una empresa nombran `american_classic`. **Multifashion no se tocó.**
- 🔴 **LA FORMA DE COMISIONES (6-sep-2026) — 17 cambios aprobados uno por uno, y NI UN NÚMERO SE MUEVE.** Medido con la RPC real contra producción antes y después: las **27 celdas** (3 personas × 9 meses de 2026) dan idénticas y el total sigue en **$67.815,75** (Edwin 9.037,17 · Reynaldo 58.544,09 · Rodrigo 234,49). Medición: `scripts/_medir-comisiones-forma.mjs`.
  - 🔴 **SE FUERON LAS CUATRO PESTAÑAS, en escritorio Y en celular.** Daniel: *«opino eliminar los tabs y dejar configuración como el depurador, estilo engranaje y ya… el merge de los tabs no es solo en el cel, sino también en desktop»*. En su lugar hay **UN selector de empresa**: **«Fashion Group»** (nunca «Todas» — *«en todas pon fashion group para no confundir»*), las **6** sueltas, una **línea sin rótulo** y **Multifashion**. El selector de empresa que vivía DENTRO de «Por empresa» se fue: eran dos controles para una sola pregunta, y en el iPhone la cuarta pestaña nacía cortada contra el borde. Un `?tab=` viejo y el `fg_comisiones_mode` guardado siguen llegando a su vista (`lib/comisiones/vistas.ts`).
  - 🔴 **Configuración vive en un ⚙ que está SIEMPRE, en el mismo lugar** (Daniel: *«a y con, para que no se sienta que desapareció un botón»*) — porque **no es de una empresa, es del módulo**. Solo admin, solo en `/comisiones`, y sigue siendo la única entrada.
  - 🔴 **UNA sola forma de decir «nada»: el GUION.** La misma fila mezclaba `—` y `$0.00`. Medido en septiembre 2026: la fila de Reynaldo tenía 4 celdas en `$0.00` y 2 en `—` con UN solo número que mirar. Con `$0.00` para todo la matriz sería una pared de ceros; con el guion queda a la vista lo único que importa. ⚠️ Una celda en cero **con descuento adentro SÍ muestra su número**: taparla escondería plata. Regla en `lib/comisiones/matriz-celda.ts`.
  - 🔴 **El descuento se VE en la celda**: debajo del neto, `$60.00 − $1,573.08` (septiembre 2026, Reynaldo en Fashion Shoes). Antes decía `−$1,513.08` y había que abrir el detalle para enterarse de que ahí dentro hay $1.573,08 restados. **Solo donde hay descuento.**
  - 🔴 **Los que NO se pagan (Oficina y Daniel Levy) van detrás de «Ver los que no se pagan (2)»** — $8.089,77 en 2026. 🔑 Arregla algo de paso: las filas visibles **no** sumaban el «Total a pagar»; escondiéndolas, lo que se ve suma exactamente lo que se paga. `VENDEDORES_SIN_PAGO` sigue siendo la fuente única, el cálculo no cambia y **el Excel los sigue llevando** con su «(no se paga)».
  - 🔴 **El detalle se abre ABAJO de la matriz, no encima**, con la celda tocada resaltada: la matriz son 3 filas y el modal tapaba justo lo que estabas mirando. ⚠️ **El modal se queda** (la vista de una empresa lo usa) y **las dos formas son el MISMO componente**; la hoja impresa viaja en un portal a `<body>`, así que imprimir sale igual desde las dos. Con **«Todo el año»** la celda no abre detalle y el pie lo dice: el reporte por vendedor es de UN mes.
  - 🔴 **«Todo el año» en el selector de período** (Daniel rechazó una columna fija de «2026»: *«lo verán todo el tiempo»*). **El año es LA SUMA DE SUS MESES**: la misma RPC mes a mes, neteada mes a mes por `netearComisiones` —la vigencia de un descuento puede empezar a mitad de año— y sumada en `lib/comisiones/acumular-anio.ts`. No hay una consulta «anual» aparte. El año en curso se corta en el mes en curso de **Panamá**. Medido: 72 llamadas (6 × 12) cuestan **143 ms** dentro de la base. **La tasa no se suma, se conserva la vigente.**
  - 🔴 **El detalle: el TOTAL ARRIBA** (pegado al título, con «Ventas $345.27 · Cobros $307.15» debajo en gris) — abría con 30+ filas y el número estaba al final. **La factura se muestra corta** (`3022`), con la MISMA regla de Guías (`lib/comisiones/factura-en-pantalla.ts`); ⚠️ **en el Excel y en el papel se queda LARGA** (`11-000003022`), Daniel: *«no»*. **Se fue la columna «Tipo» (FA/NC)** de la pantalla —la nota de crédito ya va en rojo y en negativo— y **se queda en el Excel y en el papel**, que es donde se concilia contra Switch.
  - 🩸 **El PDF se llamaba «Fashion Group.pdf»**: se imprime con `window.print()` y Chrome nombra el archivo con el `document.title`, que en toda la app es ese — los doce reportes de un cierre bajaban con el mismo nombre. Ahora es `Comisión-Edwin-Vistana-2026-08`, y el título **se devuelve como estaba aunque se cancele el diálogo** (`lib/comisiones/nombre-archivo.ts`, el MISMO nombre que usa el Excel del detalle).
  - 🔴 **El Excel del detalle: título en la fila 1, fila 2 VACÍA, encabezados en la 3 con FILTRO y fila fija.** Daniel: *«se puede mover a la fila 3 para separación y con filtro»* y, sobre los dos filtros posibles, *«como está entonces, filtro en facturas nomás»* — **UN solo filtro, sobre las facturas**; Cobros y el cierre se quedan sin él (una hoja admite un autofiltro y él lo sabe). La fila fija ya no está clavada en la 1: `excel-panel-fijo.ts` la **deriva del `ref` del filtro**. Y **la tasa del cierre es un número con formato de porcentaje**, no el texto `× 0.50%`: era la única columna que explicaba de dónde sale la comisión y la única que Excel no podía recalcular. ⚠️ **Las columnas siguen siendo CINCO** — nada de «% de utilidad» ni «Comisión» por línea (*«es a propósito»*).
  - 🔴 **Un cliente que no comisiona, en VARIAS empresas de una vez.** Medido: eran ~10 toques por empresa, **50 para cinco**, y ya estaba pasando (**D-104 cargado dos veces**, Active Shoes y Active Wear, por una sola decisión). Ahora el alta ofrece las 6 en pastillas y el servidor escribe **una fila por empresa** (el grano de la tabla no cambia); la que ya existía **no tira a las demás**. **50 → 12.**
  - 🔴 **Configuración tiene UN solo botón principal**: se fue «Guardar tasas» —un segundo botón negro compitiendo con «+ Agregar», y las tasas pedían guardar mientras las casillas de al lado se guardaban solas— y la tasa se manda **al salir del campo**, solo si cambió. **Se fue la columna «Desde»** de «Clientes que no comisionan»: decía «3 sept 2026» en TODAS las filas (el día en que se cargaron). `creado_en` sigue en la base.
  - 🔴 **Se fue la línea «N vendedores sin actividad este mes»**: un renglón para decir que no hay nada que decir. Y **contabilidad no ve el interruptor del descuento** (ya estaba así desde el 25-ago: el POST le contesta 403 y el toggle optimista revertía sin decir nada).
  - 🔴 **Los botones dicen QUÉ traen y el verbo es «Descargar»** (Daniel: *«a, pero descargar, no bajar, como esté en todos los módulos»*; medido: el sistema decía «Descargar» **23 veces** contra 5 formas raras). En Comisiones, **«Descargar el mes»** / **«Descargar el año»** y **«Descargar el detalle»**; y los 5 botones sueltos del sistema —CXC celular, Proveedores, Marketing, Referencia y Comprobantes— pasaron de «Exportar»/«Bajar» a **«Descargar»**, conservando qué descargan.
  - Diccionario § 0 aplicado al módulo: **nombre de empresa CORTO** («Vistana», no «Vistana International») en la matriz, las tarjetas, el detalle, Configuración y los nombres de archivo; **`text-sm` mínimo para los datos** en las tarjetas del celular (era el único lugar del módulo bajo el piso: 12 px).
  - Candados: `comisiones-forma.test.ts` · `comisiones-forma-pantalla.test.tsx`; **35 mutaciones, 35 cazadas** (`scripts/_mutar-candados-comisiones-forma.sh`, con 2 controles).

- 🔴 **DESCARGAR SIN ENTRAR AL DETALLE: LA FLECHITA DE LA CELDA (8-sep-2026).** 🩸 Bajar el reporte de un vendedor en una empresa costaba **CINCO toques** (tocar la celda → esperar → bajar al detalle → «Descargar el detalle» → cerrar), y otra vez por cada una de las seis empresas: el cierre de una persona eran ~30 toques. Ahora, **pegada al número y en gris**, va una **flechita ↓** con un menú de dos líneas —`Descargar en PDF` · `Descargar en Excel`— encabezado por `Reynaldo Espinosa · Fashion Shoes · Septiembre`. 🔴 **NINGÚN NÚMERO SE MUEVE**: medido con la RPC real antes y después, las **27 celdas** de 2026 dan idénticas y el total sigue en **$68.325,18** (Edwin 9.066,09 · Reynaldo 58.943,26 · Rodrigo 315,83) — ⚠️ el $67.815,75 del 6-sep ya no da porque **septiembre creció con dos días más de producción**: enero–agosto están idénticos al centavo (`scripts/_medir-comisiones-flecha.mjs` lo comprueba mes por mes).
  - 🔴 **LA FLECHA AGREGA UN CAMINO, NO REEMPLAZA NINGUNO.** Tocar el número sigue abriendo el detalle exactamente como antes; la flecha para el clic para que los dos convivan. **Una sola parada del clic**, en la caja que envuelve la flecha y su menú — ponerla también en el botón sería una segunda copia de la misma regla.
  - 🔴 **SOLO SE DIBUJA DONDE HAY ALGO QUE BAJAR**, y esa decisión **no se escribe dos veces**: `hayQueDescargar` (`lib/comisiones/descarga.ts`) se la pregunta a `celdaVacia`, la MISMA que decide si la celda dice `—`. Separarlas es cómo se llega a una flecha encima de un guion. ⚠️ Una celda en cero **con descuento adentro sí la lleva**: ahí hay plata.
  - 🔴 **TRES ALCANCES.** La flecha de una **celda** → esa empresa. La de la columna **Total** → **todas las empresas de esa persona en UN archivo** (una hoja por empresa, en el orden de las columnas). Los **dos botones de arriba** → el mes con **las 6 empresas**.
  - 🔴 **ARRIBA HAY DOS BOTONES: «Descargar el mes en PDF» y «Descargar el mes en Excel».** El Excel es **exactamente el de siempre** (la matriz consolidada, mismo archivo y mismo nombre); el PDF es esa misma matriz impresa, armada de las **mismas filas** que están en pantalla y con el **mismo pie** (`sumarPagable`) — ni una suma nueva. ⚠️ **«Descargar el año» se queda como está** (Excel y nada más): con «Todo el año» no hay PDF ni flechas, porque el reporte por vendedor es de **UN mes** (`comision_b2b_detalle` recibe year + mes) — la misma condición que ya apagaba el detalle.
  - 🔴 **SON LOS MISMOS ARCHIVOS DE SIEMPRE.** El Excel sale de `exportComisionDetalle` y el papel de `ImpresionComision`, con la MISMA lectura (`/detalle` + `/descuentos`) y el MISMO nombre (`nombre-archivo.ts`). No hay un generador nuevo. El de varias empresas se llama `Comisión-Reynaldo-Espinosa-**Todas**-2026-09` para no confundirse con el de una sola; el del mes reusa el nombre que ya tenía su Excel (`comisiones-consolidado-2026-09`).
  - 🩸 **IMPRIMIR CON EL NOMBRE CORRECTO PASÓ A UN SOLO LUGAR** (`lib/comisiones/imprimir.ts`): nació dentro del modal del detalle y la flecha imprime el mismo reporte **sin abrirlo**, así que una segunda copia sería una de las dos dejando el `document.title` de toda la app renombrado.
  - 🩸 **Y SOLO SE IMPRIME EL PAPEL QUE SE PIDIÓ.** Con el detalle abierto su hoja también está montada en `<body>`: sin el candado de CSS, el PDF de una empresa habría salido con el reporte de otra pegado atrás.
  - ⚠️ **En el celular la flechita también está** (las tarjetas son la matriz por debajo de 1024 px), y las tarjetas **siguen sin saber nada de rutas ni de archivos**: reciben la flecha ya dibujada por la vista, que es la dueña de la descarga.
  - 🩸 **El menú va en `DesplegableFlotante`, no en un `absolute`.** La tabla de la matriz vive dentro de un `overflow-x-auto`: un panel colgado de la celda lo habría RECORTADO justo en la fila de abajo (y subir el z-index no arregla eso). Lo cazó el barrido de la casa `desplegables-flotan.test.ts`.
  - ⚠️ **Decisión pendiente de Daniel**: «las 6 hojas» del botón de arriba se leyó como **las 6 EMPRESAS** (la matriz del mes en los dos formatos), no como seis reportes de detalle sueltos. Es lo único que quedó a interpretación.
  - Candados: `comisiones-flecha.test.ts` · `comisiones-flecha-pantalla.test.tsx`; **30 mutaciones, 30 cazadas** con 3 controles (`scripts/_mutar-candados-comisiones-flecha.sh`); medición `scripts/_medir-comisiones-flecha.mjs`. Tres candados **cambiaron de dirección con nota fechada, ninguno se borró**: `comisiones-forma` (el rótulo del Excel ahora sale de `rotuloDescargarExcel`, que le pega « en Excel» al MISMO `rotuloDescargarPeriodo`; y `imprimirComo` se exige en su módulo, con el CONTROL de que el detalle la sigue llamando) e `iphone-comisiones-encabezado` (dos botones en la barra, los dos con su rótulo del módulo puro).

- 🔴 **Con «Todo el año» y UNA empresa, tocar una fila NO abre el detalle** (11-sep-2026): `ComisionesPorEmpresaView` aplica el MISMO candado que Fashion Group (`conDetalle = !esTodoElAnio(mes)`) — la fila deja de ofrecerse como tocable y el pie dice «Elige un mes para ver el detalle». 🩸 Abría el detalle y mostraba el error crudo «mes inválido (1..12)». Candado: `comisiones-por-empresa-todo-el-anio.test.tsx`.
- 🔴 **El espejo de Multifashion recibe el AÑO ELEGIDO** (`<VendedorasSubtab selectedYear={year} />`). 🩸 Iba `inicial.year`: en enero `periodoInicial` abre en diciembre del año anterior y el ranking salía sobre el año pasado con «Diciembre (en curso)» sobre un año cerrado. Candados: `multifashion-cerrado-y-espejo.test.ts` · `multifashion-rediseno.test.ts` (cambió de dirección con nota).
...
- 🔴 **Vista General** decidía su mes con el reloj del **NAVEGADOR** (`new Date().getMonth()`): un iPhone en otra zona horaria abre el mes siguiente antes de tiempo. **Ventas** lo decidía con el reloj del **SERVIDOR**, que en Vercel corre en **UTC**: las últimas 5 horas de cada día —después de las 7 p.m. de Panamá— el día UTC ya es el siguiente, así que cada **31** le pedía a Multifashion el mes que viene y la noche del **31-dic** la página abría en el año nuevo, vacío. Las dos pasan a `hoyPanama()`, la única fuente de «hoy» de la casa. Es el mismo defecto ya corregido en Comisiones (6-sep) y en `ventas_dashboard_prev_same_period_v3` (3-sep). ⚠️ **Ningún cálculo cambia**: solo con qué período abre cada pantalla. Candado: `mes-de-panama-vista-general-y-ventas.test.ts`.

**El módulo Ventas, los 13 cambios (11-sep-2026).** Daniel aprobó uno por uno los trece puntos del mockup «Ventas, ahora vs después». 🔴 **NINGÚN TOTAL DE VENTA SE MUEVE**: medido con la RPC real y con la capa TS antes y después (`scripts/_medir-ventas-13-cambios.mjs` · `_medir-ventas-totales-ts.ts`): agosto de las 8 = **$788.475,49** y el año, idéntico al centavo a la misma hora ($6.430.087,46 a las 4 p.m.; los $6.427.806,78 del mediodía son la misma suma con menos horas de venta). Lo único que cambia de valor es la utilidad y el margen del mes en curso (#5), y eso está medido empresa por empresa. Detalle completo en `docs/estado-actual.md` (11-sep, noche).

- 🔴 **UN SOLO SELECTOR DE PERÍODO ARRIBA, que manda en las tres pestañas** (`src/lib/ventas/periodo.ts`, mismo patrón que Multifashion): `Año 2026 · 2025 · … · Últimos 12 meses · Últimos 6 meses`. Vive en la URL (`?periodo=` = `2026` · `u12` · `u6`) y se recuerda por usuario (`fg_last_ventas_periodo`); la URL manda, después la memoria, después el año en curso de **Panamá**. 🔴 **Cada pestaña ofrece SOLO lo que sabe servir** y un período que no sirve cae al año en curso: Resumen solo años (es la matriz de UN año); Productos años y las dos ventanas; Clientes años **y las ventanas solo cuando la vista las trae** (`ventanasDisponibles`, lo dice el SERVIDOR con las filas, nunca una lista a mano). 🩸 Había TRES controles de tiempo (el año suelto, «Clientes: últimos 12 meses» y el «Período» propio de Productos con un párrafo explicando que el año de arriba no aplicaba): los tres se retiraron, y con ellos la línea «8 empresas · cierre Ago (mes en curso Sep)» y el «Ene–Sep 2026 · vs 2025» repetido en las cuatro tarjetas (contra qué compara queda en el `title` del delta y en el pie de la matriz). La caja del mes dice «Septiembre · $… ▼ 50% · **vs $… en 2025**» — el «vs» SE QUEDA, Daniel lo pidió.
- 🔴 **Total y Proyección van FIJAS a la derecha** de la matriz (`data-col-fija`, `position: sticky`, fondo sólido y una sombra fina), con Empresa fija a la izquierda como siempre: cuando la matriz arrastra, lo que se pierde de vista son los meses del medio, nunca las dos columnas que más se miran.
- 🔴 **«Utilidad» y «Margen %» se fusionaron; «Trimestral» y «Anual» se retiraron.** Queda `Ventas · Utilidad`, y en Utilidad cada celda lleva debajo, chico y gris (`data-margen-celda`), el margen % del período. Medido: en Clientes, Utilidad y Margen % montaban el MISMO componente con la misma consulta y solo cambiaba el orden inicial. El detalle anual se abre tocando la empresa (histórico mes × año, que conserva su control de tres métricas). `ResumenAnual.tsx` y `/api/ventas/resumen-anual` se retiraron; `?modo=margen` guardado llega a `utilidad` (`modoHeredado`).
- 🔴 **EL MARGEN DEL MES EN CURSO DEJA DE MEZCLAR LA VENTA DE HOY CON EL COSTO DE AYER** (`src/lib/ventas/margen-mes-en-curso.ts`). 🩸 La RPC arma el mes con la venta hasta el instante y el costo de `switch_articulo_diario`, que llega hasta AYER: Vistana se veía **28,6 %** y con el mismo corte es **22,8 %** (venta hasta el 10-sep $56.418,00, costo $43.548,58); el grupo, 32,1 % contra 29,8 %; y el día 1 de cada mes el margen salía **100 %**. Daniel: *correcto y un día viejo*. La utilidad y el margen del mes en curso se calculan hasta el ÚLTIMO DÍA CON COSTO y el pie lo dice («Utilidad y margen de septiembre al 10 de septiembre, el último día con costo cargado»); **la VENTA del mes sigue siendo la de hoy**. El corte lo trae la RPC `ventas_mes_en_curso_corte_costo` (migración **`20261120120000`, pendiente de aplicar**; aditiva, no toca ninguna RPC); mientras no corra la lectura falla ABIERTA y el Resumen se comporta como hoy. Medido el 11-sep-2026 por empresa (margen hoy → con corte): Vistana 28,6 → 22,8 · Fashion Wear 27,6 → 27,6 · Boston 43,5 → 43,2 · Fashion Shoes 30,0 → 28,2 · Multifashion 36,4 → 31,6 · Active Wear, Active Shoes y Joystep sin cambio.
- 🔴 **Los tres botones dicen «Descargar en Excel» y bajan LO QUE ESTÁ EN PANTALLA** (`src/lib/ventas/descarga.ts`): el del Resumen baja la matriz en el modo elegido (el de Ventas es byte a byte el de siempre; el de Utilidad lleva la utilidad por mes y el margen del año), y **hay botón en el celular**, donde no había. Cada descarga se anota en `activity_logs` (`descarga_excel`, módulo `ventas`, con la pestaña adentro) — 🩸 medido: de Ventas no había NI UNA fila en 90 días. Daniel: *«sí»*.
- 🔴 **Clientes elige empresa con un DESPLEGABLE** (Daniel: «B»), no con siete píldoras. 🔴 **La primera opción dice «Todas las empresas»** porque la lista son solo las 6 del grupo; **«Fashion Group» solo cuando la lista mezcla grupo y no-grupo** (Comisiones, con Multifashion). La regla vive en `src/lib/ventas/rotulo-empresas.ts` (`rotuloDeTodas`), con candado en las dos direcciones, y las seis se DERIVAN de `B2B_EMPRESA_KEYS`.
- 🔴 **Ventas › Clientes se refresca con cada sync de facturas y dice de cuándo son los datos.** 🩸 `clientes_empresa_12m_vw` es una vista MATERIALIZADA (aunque termina en `_vw`) que se refrescaba a las 2:35 a.m.: al mediodía City Mall decía **$1.256.838,89** en Ventas y **$1.260.018,89** en su ficha — faltaban los $3.180,00 de esa mañana y ningún rótulo lo decía. Ahora la refresca `switch-sync tipo=facturas|all` cuando alguna de las 6 del grupo terminó bien (cero crons nuevos; tolerante: un tropiezo no marca el sync como fallido), y los TRES caminos que la refrescan (el sync, «Actualizar ahora» y el cron de las 07:35) dejan la marca `clientes-vw-refrescada` en `cron_heartbeats` por `src/lib/ventas/refrescar-vista-clientes.ts` — está en `HEARTBEATS_NO_CRON` (es una marca de agua, no un cron). La pestaña dice «115 clientes · datos de hoy 10:00 a.m.» (`lib/ventas/frescura.ts`, hora de Panamá); sin marca no dice nada.
- 🔴 **La columna dice QUÉ período suma**: «Compras · Año 2026» / «Compras · Últimos 12 meses» (`rotuloCompras`), y «vs 2025» / «vs año anterior». El desplegable «Clientes: últimos 12 meses / con compras en 2026» se retiró: en Utilidad no hacía nada y en Ventas su segunda opción era lo mismo que no abrir los «N clientes sin compras» plegados al final. ⚠️ Las ventanas de Clientes salen de la migración **`20261121120000`** (**pendiente de aplicar**: agrega `compras_12m/_prev` y `compras_6m/_prev` a la MV y a la vista agregada, ADITIVA, `compras_ytd` y `delta_vs_2025` byte a byte); mientras no corra el servidor sirve el año y lo dice (`ventana: null`), y el selector ofrece solo años en Clientes. **Nunca se rotula un período que no se sumó.**
- 🔴 **«Nuevo» en vez de «+0 %»** para el cliente sin base comparativa (`delta: null`; antes llegaba como `0` y 34 de 116 se veían estancados — la rama `NO_COMPARATIVE` existía y nunca se alcanzaba). Va al final al ordenar por cambio. **El porcentaje grande (+30994 %) SE QUEDA** — Daniel: *«deja el porcentaje para que todo tenga una misma línea»*.
- 🔴 **Multifashion salió del selector de Ventas › Productos** (`PRODUCTOS_EMPRESAS` deriva de `B2B_EMPRESA_KEYS`): no tiene ni una fila en `switch_factura_lineas`, así que su «quién lo compra» salía siempre vacío, y esa información vive en su propio módulo. **Boston NO entra** (Daniel: «NO»), aunque tiene 18.186 renglones — el comentario que decía «no se backfilleó» era falso y se corrigió. Se retiró el aviso «para mirar un año elige "Año en curso" o un mes» (mandaba a un selector que no existía desde el 24-ago) y queda **UN solo** descargo de «sin las ventas de mostrador» (con cliente puesto, la línea de arriba; sin cliente, el pie de «Quién lo compra»).
- 🔴 **La puerta de atrás se cerró**: las 6 rutas de datos de Ventas que aceptaban `contabilidad` (y `/v2/status`, que aceptaba `secretaria`) son solo `admin`, como la pantalla; `/api/ventas/v2`, `/v2/status`, `/años` y `/ventas/reporte` (que solo redirigía; ahora es un redirect en `next.config.js`) se retiraron con cero llamadores; los 16 componentes de Comisiones y los 3 de Referencia se mudaron a `src/components/comisiones/` y `src/components/referencia/` **sin cambiar ninguna pantalla**; y la búsqueda global dejó de ofrecerle «Ventas» a contabilidad, que no puede abrirlo. Candado: `ventas-puerta-cerrada.test.ts` (deriva las listas con glob).
...
## 🔴 Comisiones: el papel del vendedor lleva SOLO lo pagable, y Multifashion abre en el mes cerrado (22-sep-2026)

**Lo que Daniel aprobó, textual** (auditoría del 22-sep-2026, `scratchpad/auditoria-comisiones.md`): *«3. b) no salen»* (las facturas que no comisionan dejan de listarse en el PDF), *«recibo $0.00: a) se quita del papel»*, y *«7. a)»* (la opción Multifashion dentro de Comisiones abre en el último mes cerrado, igual que el grupo). El rastro de descargas y de configuración es informativo, aprobado sin preguntar. **Se queda como está, decidido por Daniel**: «TOTAL VENTAS + COBROS» en el PDF; la secretaria puede apagar el descuento de un mes; los vendedores no ven su comisión; Rey Stoute Aguas sigue retirado; el mes negativo tal cual.

### 🩸 Lo que pasaba

- **El PDF y el Excel del MISMO vendedor no decían lo mismo.** El RPC lista a propósito las facturas con utilidad ≤ 20 % con aporte $0.00 (en pantalla van en gris con la columna «% utilidad», que las explica); el Excel las quitaba (`ventasPagables`, «el Excel es el papel de lo PAGABLE») y el PDF no. Medido en los papeles reales del 20-sep: Reynaldo · Fashion Wear **51 renglones en el PDF contra 49 en el Excel**; Edwin · Vistana «De Moda · 11-000003024 · FA · $0.00» (utilidad 9,68 %). El papel no tiene la columna que lo explica: el vendedor lee «mi venta a De Moda vale $0».
- **Un recibo en $0.00 salía en los dos papeles** («20 ago · Jerusalem De Panama · $0.00», Reynaldo · Fashion Wear). En Cuentas por Cobrar la regla ya era «recibos en cero no cuentan» (`lib/cxc/pagos-por-fecha.ts`).
- **Multifashion abría en «Septiembre (en curso)» mientras el grupo abría en «Agosto» (cerrado)**, en la misma pantalla y con el mismo selector arriba: la vista de Vendedoras venía montada tal cual, con sus chips propios, y el período del shell no le llegaba.
- **Cero rastro.** Medido: `activity_logs` tenía 1.547 filas en 90 días y **ninguna** de Comisiones — ni descargas, ni tasas, ni clientes sin comisión, ni descuentos. Ventas sí anota cada descarga.

### Lo que se hizo

- 🔴 **UNA función decide qué renglones van al papel, y la leen los DOS papeles**: `lib/comisiones/papel-pagable.ts` › `renglonesDelPapel(data)` = facturas con aporte ≠ 0 y recibos con monto ≠ 0. La lee `reporte-comision.ts` (lo que dibuja el PDF: `filasVentas`/`filasCobros`) y `comisionExcel.ts` (`buildComisionDetalleSheet`); `ventasPagables` se conserva por nombre y delega. La **nota de crédito se queda** (es negativa, no cero). Interruptor `PAPEL_SOLO_PAGABLE` (hoy `true`); en `false` cada papel se porta como antes (el Excel sin facturas en $0 —ya lo hacía—, el PDF con todo, el recibo en cero en los dos).
- 🔴 **NINGÚN TOTAL SE MUEVE.** `ventas_base`, `cobros_base` y las comisiones son las del RPC y no se recalculan. **Medido contra producción con la RPC real (`comision_b2b_v9` × 6 empresas + descuentos como `netearComisiones`), antes y después, idéntico:** agosto 2026 = **$5.978,55** a pagar (Reynaldo 5.091,64 · Edwin 652,42 · Rodrigo 234,49; no pagados Daniel Levy 470,23 · Oficina 327,77; Rey Stoute Aguas 5,90 retirado). Y con `comision_b2b_detalle`: Reynaldo · Fashion Wear 51 ventas (2 en $0) y 11 cobros (1 en $0), `ventas_base` 206.345,44 con y sin ellos; Fashion Shoes 49 ventas (7 en $0); Active Shoes 6 (4 en $0); Edwin · Vistana 29 (1 en $0: De Moda). Script: `scripts/_medir-comisiones-papel-pagable.mjs` (solo lectura).
- 🔴 **Multifashion abre y se mueve con el período del grupo**: `lib/comisiones/multifashion-periodo.ts` traduce el selector de Comisiones al vocabulario de Multifashion (`periodoParaMultifashion`: mes → mes, «Todo el año» → año; `corteParaMultifashion(hoyPanama())`) y el shell se lo pasa a la MISMA `VendedorasSubtab` importada. Interruptor `MULTIFASHION_CON_EL_PERIODO_DEL_GRUPO` (hoy `true`); en `false`, `periodo` va en `undefined` y sus chips propios vuelven. ⚠️ **Su cálculo no se tocó ni se mezcla** con el del grupo. ⚠️ **Multifashion NO tiene papel**: ni su módulo ni la vista arman un PDF o un Excel, así que en esa opción **no se dibujan los botones de descarga** ni el «Actualizar ahora» de recibos del grupo — no se inventó una descarga (encargo: «si no existe ninguna, dilo y NO la inventes»). Los candados viejos que exigían `<VendedorasSubtab selectedYear={year} />` exacto (`multifashion-cerrado-y-espejo`, `multifashion-rediseno`) pasaron a vigilar `selectedYear={year}` y que las Metas sigan sin montarse.
- 🔴 **Queda rastro** (`lib/comisiones/rastro.ts` + `rastro-server.ts`, `entity_type = comisiones`): cada descarga (`descarga_pdf` / `descarga_excel`, con `alcance` vendedor · vendedor-todas · matriz-grupo · matriz-empresa, vendedor, empresa, año, mes) desde las cuatro puertas (flechita, modal del detalle, matriz del grupo, matriz de una empresa); y cada cambio de configuración lo anota el **servidor** con quién lo hizo: `config_tasa` (PUT config), `config_cliente_sin_comision` (POST/PATCH/DELETE exclusiones), `config_descuento` (POST/PATCH/DELETE descuentos-fijos) y `config_descuento_mes` (POST descuentos, el interruptor mensual que mueve $1.573,08). Anotar nunca frena ni tira la respuesta. ⚠️ Los alias de vendedor no tienen ruta de escritura en la app (se cargan por migración): no hay nada que anotar ahí.

### Candado y mutaciones

`src/__tests__/lib/comisiones-papel-pagable.test.ts` (24 pruebas): interruptores en `true`; la factura en $0 y el recibo en cero no salen ni en el PDF (texto extraído con pdfjs) ni en el Excel; el PDF CON los ceros del RPC es byte a byte el mismo texto que SIN ellos, con los totales del RPC; el PDF y el Excel importan `renglonesDelPapel` y no recorren `data.ventas`/`d.cobros` por su cuenta; `ventasPagables` delega; con el interruptor en `false` va todo; Multifashion recibe `periodo`/`corte` del selector que arranca en el mes cerrado; las cuatro puertas de descarga y las cuatro rutas anotan. **8 mutaciones a mano, 8 cazadas** (la 8ª —la flechita deja de anotar el PDF— pasó la primera vez y el candado se reforzó para cazarla): el PDF vuelve a `data.ventas` · el recibo en cero vuelve · `PAPEL_SOLO_PAGABLE = false` · segunda copia del filtro en el Excel · Multifashion sin `periodo` · `MULTIFASHION_CON_EL_PERIODO_DEL_GRUPO = false` · la ruta del descuento mensual deja de anotar · la flechita deja de anotar el PDF.

## 🔴 Ventas: UNA SOLA VENTA — la misma venta da el mismo número en las tres pestañas, 2022 y 2023 se sirven, y el Excel de Clientes se explica solo (23-sep-2026)

**Lo que Daniel decidió, textual** (auditoría del 22-sep-2026, `scratchpad/auditoria-ventas.md`): *«Debe de dar igual»* (la misma venta, el mismo número en Resumen · Clientes › Utilidad · Productos; la referencia es el Resumen), *«Sí»* mira 2022 y 2023, *«Sí»* baja el Excel de Clientes. Informativo, aprobado sin preguntar: la matriz dice «no vendiste» en vez de «n/a», y la causa del «Actualizar ahora» que se cortaba en la sexta empresa.

### 🩸 Lo que pasaba (medido el 23-sep-2026 contra producción, `scripts/_medir-ventas-una-sola-venta.mjs`, solo lectura)

- **Clientes › Utilidad no contaba el contado.** El reporte de utilidad de Switch (`switch_factura_utilidad`, el «Listado de comprobantes») **no lista las «Transacciones»**: 546 comprobantes de 2026 en las 6 del grupo están en `switch_facturas` y en ese reporte no. Empresa por empresa el hueco ES el contado, al centavo (±$0,01 de redondeo por documento en Fashion Wear, Fashion Shoes y Joystep). Casi todo es el mostrador (`TCKCTA`: 471 de 546), el resto son clientes reales que compraron de contado (Norma Vega $2.082, Zona Sur Dutty Free $3.567, Feria Int. de David $943,77…).
- **Productos no contaba las notas de débito** (`switch_articulo_diario`, el reporte por artículo, tiene 0 filas 'ND' en toda su historia) **y además le faltan renglones**: códigos de artículo que están en la factura (`switch_factura_lineas`) y que el reporte por artículo no devuelve ese día. Medido por día: FW `WW0WW40587W0G` $460 (9-feb), `MW0MW37267YBR` −$136 (19-mar), `MW0MW41354YBR` $30 (27-abr); FS `FW0FW08857TQQ` $720 (31-mar), `EM0EM01700BDS` $264 (8-sep). Los $313,93 «sin explicar» de la auditoría son eso, y hoy son $353,93 porque los datos se movieron.
- **2022 y 2023 se rechazaban** con `year < 2024` en las rutas de Productos (las tres) y de Utilidad, y la pantalla quedaba en «No se pudieron cargar los productos · Reintentar». El dato existía: Fashion Wear 2023 = $3.318.508,30 en 163 descripciones (RPC probada en las 6: vistana 2022 y 2023, FW/FS/AS/AW 2023; Joystep arranca en jul-2025 y FW en feb-2023).
- **El Excel de Clientes no sumaba sus filas**: el mostrador iba sin marca y el TOTAL lo dejaba afuera sin decirlo.
- **La matriz decía «n/a»** en Joystep: la frase existía desde el 5-sep pero ninguna de las 6 llamadas (4 de escritorio, 2 de celular) le pasaba la base del año anterior.

### La tabla, antes y después (2026, medido el 23-sep-2026; «después» = lo que dan las rutas con el interruptor prendido)

| Empresa | Resumen (referencia) | Utilidad antes | Utilidad después | Productos antes | Productos después | De eso, notas de débito | Renglones que el reporte por artículo no trae |
|---|---|---|---|---|---|---|---|
| Vistana | 1.404.885,04 | 1.395.752,44 | 1.404.885,04 | 1.400.241,04 | 1.404.885,04 | 4.644,03 | −0,03 |
| Fashion Wear | 2.411.430,20 | 2.389.954,14 | 2.411.430,20 · respaldo 2.411.430,21 | 2.406.204,79 | 2.411.430,20 | 4.871,48 | 353,93 |
| Fashion Shoes | 1.363.002,79 | 1.335.576,05 | 1.363.002,79 · respaldo 1.363.002,80 | 1.354.703,59 | 1.363.002,79 | 7.315,21 | 983,99 |
| Active Shoes | 494.580,25 | 492.145,05 | 494.580,25 | 494.567,49 | 494.580,25 | 12,76 | 0,00 |
| Active Wear | 341.544,30 | 337.830,30 | 341.544,30 | 267.704,13 | 341.544,30 | 73.840,17 | 0,00 |
| Joystep | 30.627,96 | 29.171,74 | 30.627,96 · respaldo 30.627,97 | 30.628,03 | 30.627,96 | 0,00 | −0,07 |
| **6 del grupo** | **6.046.070,54** | **5.980.429,72** | **6.046.070,54** | **5.954.049,07** | **6.046.070,54** | 90.683,65 | 1.337,82 |

«Respaldo» = mientras la migración de la v3 no corra: la v2 más el contado desde `switch_facturas`, con ±$0,01 de redondeo por documento del reporte de utilidad, **que la pantalla dice** («le sobran $0,01 contra el Resumen»). El Resumen no se tocó. Boston y Multifashion no entran a estas dos pestañas (decisión previa de Daniel; sus reglas propias siguen).

### Lo que se hizo

- 🔴 **LA DEFINICIÓN ES UNA** (`lib/ventas/una-sola-venta.ts`, puro): Factura · Tiquete · Transacción · Nota de Débito SUMAN, Nota de Crédito RESTA — la lista de `tipos-comprobante.ts`. De ella se **GENERA** el CASE de SQL (`sqlVentaFirmada`), y el candado compara el texto generado contra las migraciones que lo llevan: `switch_ventas_unificado_vw` (el Resumen), `ventas_dashboard_summary_v2` y la nueva `utilidad_por_cliente_v3`.
- 🔴 **Clientes › Utilidad**: la venta por cliente sale de `switch_facturas` firmada (el contado incluido), el costo y la utilidad del reporte de utilidad, y el margen sobre la parte CON costo (el contado no lo diluye: no se le conoce el costo). RPC **`utilidad_por_cliente_v3(p_anio, p_empresas)`**, migración **`20261217120000_utilidad_por_cliente_v3_una_sola_venta.sql`, ESCRITA y NO aplicada** (la aplica Daniel con `npm run migrar`); aditiva, año de Panamá, clave (empresa, `cliente_switch_id`) con el respaldo por nombre de la v2, FULL JOIN para no perder nada en silencio, y devuelve el `codigo` para marcar el mostrador. **Mientras no corra**, la ruta cae a la v2 y le suma el contado desde `switch_facturas` (`contadoDelAnio`, paginado). La línea del pie dice el cuadre: «incluye $65.640,85 de ventas de contado, que Switch reporta sin costo · el mismo total que el Resumen» (o cuánto falta y por qué). El mostrador va marcado «mostrador · ventas de contado» y **SUMA** (el Resumen lo suma). Para 2022–2025 no se llama a la RPC: «Utilidad tiene datos desde enero 2026» (el reporte arranca ahí).
- 🔴 **Productos**: `totales.venta` es la venta del Resumen para la ventana, leída con la **MISMA función** (`leerDashboardSummary`, suma de meses: la ventana siempre arranca el 1 de un mes y termina hoy o el 31-dic); el costo suma el de las notas de débito de `switch_factura_utilidad` (la única fuente con costo de ND, como el Resumen). Debajo del total: «El total es el del Resumen e incluye $4.871,48 de 12 notas de débito sin artículo y $353,93 de renglones que el reporte por artículo de Switch no trae». La ventana previa (`previo=1`, la del Δ) no se cuadra. El Excel de Productos lleva esas dos filas de cierre al final, para que sume su TOTAL. **Falla ABIERTA**: si el Resumen no se lee, el total es el del listado, como antes. Un año sin datos: «Productos de Fashion Wear tiene datos desde febrero 2023».
- 🔴 **2022 y 2023** (`anioValido`, piso `PRIMER_ANIO_VENTAS = 2022`) en las cuatro rutas: Productos, códigos, por cliente y Utilidad.
- 🔴 **Excel de Clientes**: la fila del mostrador dice «VENTAS LOCAL · mostrador · ventas de contado · fuera del ranking» y el TOTAL dice «TOTAL · 91 clientes, sin el mostrador». El número del total no cambia.
- 🔴 **La matriz**: `deltaCeldaDe` (en el módulo puro) pasa la base previa a `deltaCelda`; las 6 llamadas (`ResumenView` ×4, `ResumenViewMobile` ×2) van por ahí. Con Joystep en marzo: «no vendiste».
- **Interruptor** `src/lib/ventas/una-sola-venta-interruptor.ts` › `UNA_SOLA_VENTA` (hoy `true`): en `false` las rutas rechazan 2022–2023 como antes, Productos no cuadra, Utilidad llama a la v2 sin contado ni cuadre, los Excel salen como antes y la matriz vuelve a «n/a». Nada de lo que se guarda cambia.

### «Actualizar ahora» NO se corta: eran dos botones distintos

Medido en `switch_sync_log` (solo lectura): las 87 tandas «manual» de 6 empresas en el orden vistana → joystep son el **refresco de facturas de HOY de Guías** (`POST /api/guias/facturas-hoy`, `B2B_EMPRESA_KEYS`, `triggeredBy: "manual"`), que se dispara al entrar al módulo Guías y con «Buscar otra vez». El botón de Ventas (8 empresas + «Vistas de ventas») se apretó **una sola vez**, el 6-sep-2026 a las 00:00 UTC, y corrió **completo**: `american_classic` → las 6 → `confecciones_boston` → `sync-now-refresh-vistas` (heartbeat 00:01:59). No hay nada roto ni nada que arreglar en código; la auditoría tomó el proxy equivocado. ⚠️ Los dos caminos escriben el mismo `triggered_by = 'manual'`, así que en el log no se distinguen: separarlos es una decisión de Daniel (no se tocó lo que se guarda).

### Candado y mutaciones

`src/__tests__/lib/ventas-una-sola-venta.test.ts` (30 pruebas): la definición generada contra las tres migraciones; Productos cuadra con el Resumen (la ruta real con dobles: total = suma de meses del Resumen, ND por tipo, costo de ND, `previo=1` sin cuadre, falla abierta); Utilidad con la v3 y sin ella (v2 + contado por tipo, mostrador por código, cuadre 0 / −0,01); 2022 y 2023 aceptados, 2021 no, `datosDesde` en las dos rutas y en las rutas hijas; el Excel de Clientes (marca, rótulo del TOTAL, número intacto) y el de Productos (filas de cierre que suman); `basePrevia`/`deltaCeldaDe` y el barrido de las seis llamadas; y con el interruptor en `false` todo como antes. Se adaptaron `ventas-utilidad-joystep` (la cadena v3 → v2 → v1) y `ventas-productos-descripcion-reciente` (las lecturas del cuadre se permiten; `depurador_descripciones` sigue prohibida). **5 mutaciones a mano, 5 cazadas**: la v3 pierde «Transacción» en su CASE · Productos vuelve a `ventaListado` · una llamada de la matriz vuelve a `deltaCelda` · el TOTAL del Excel vuelve a «TOTAL» · el piso vuelve a 2024.
