...
>
> ⚠️ **La regla tachada la quitó DANIEL el 4-sep-2026 — no se aplique al revés nunca más.** Textual: *«"la dirección no se escribe sola" me refería a que el usuario no lo haga para no escribirlo mal como lo vimos, quita esa regla. Que se autollene como lo discutimos antes.»* El motivo ORIGINAL de la regla era que la PERSONA no tecleara el destino (para que no naciera otro «Pasocanoas»/«Wesland»), no prohibir que el sistema lo llene. Hoy: **con UN solo destino —definido por Daniel o único en la historia agrupada— el campo se llena solo al elegir el cliente** (`destinoParaAutollenar`); con varios, salen los botones. Lo que SÍ sigue prohibido es el **pareo por parecido** (`N2` ≠ `N3`, «Wesland» ≠ «Westland»). Ver la sección del 4-sep abajo.
>
...
> - ⚠️ ~~**El disparo automático NO va en la lista de `/guias`, y es a propósito.**~~ **Superado el 4-sep-2026** — Daniel, al probarlo: *«¿por qué no se puede hacer al apretar guías? Prefiero eso.»* El candado `guias-eliminar-en-la-fila` no se borró: **cambió de dirección** hacia lo que siempre quiso decir — **la lista solo LEE, nunca escribe una guía** — y hoy permite exactamente UNA salida además de las lecturas: el refresco de facturas de hoy, que no toca una sola guía. Ver la sección del 4-sep abajo. El disparo de `/guias/nueva` se queda para quien entra directo por URL; el acelerador de 10 min evita el doble disparo.
...
>
> El encargo contemplaba dejar **pre-marcado** el destino de un cliente definido con UNO solo («es una definición de Daniel, no un parecido»), con la instrucción explícita de no hacerlo si chocaba con el postmortem. **Choca**, y por eso no se hizo: la decisión del 14-ago-2026 sobre este MISMO campo decía que la dirección **«NO se escribe sola en el campo»**. El costo era UN toque para 8 clientes; quedó anotado, no construido.
>
> ⚠️ **Superado ese MISMO día (4-sep-2026, al probarlo Daniel):** *«sí quiero que se llene sola, ¿ese no era el propósito de todo esto? ¿Cómo que no pre-llenaste la dirección?»* — y sobre la regla del 14-ago, que él mismo había tomado: *«quita esa regla. Que se autollene como lo discutimos antes.»* Hoy el destino ÚNICO se llena solo al elegir el cliente (`destinoParaAutollenar`). Ver la sección de abajo; los botones para clientes con VARIOS destinos siguen exactamente como dice esta sección.
>
...
## 🔴 Guías — LOS AJUSTES DE DANIEL AL PROBARLO, y el resumen mensual al canal privado (4-sep-2026)

> Daniel probó «el cliente se elige una vez» y «los destinos como botones» el mismo día que salieron, y pidió cuatro correcciones. Tres cambian decisiones ANTERIORES — y en los tres casos quien las cambia es quien las había tomado.
>
> ### 1 · La búsqueda en Switch se dispara al TOCAR Guías
>
> Daniel, textual: *«¿por qué no se puede hacer al apretar guías? Prefiero eso.»* El disparo del refresco de facturas de hoy vivía solo en `/guias/nueva` porque el candado `guias-eliminar-en-la-fila` («abrir la lista es 100% lectura») se ponía rojo.
>
> 🔑 **El candado NO se borró: cambió de dirección hacia lo que siempre quiso decir** — la invariante es **«la lista solo LEE, nunca escribe una guía»**, no «la lista nunca habla con Switch». Hoy exige (a) que la ÚNICA salida que no es lectura al abrir `/guias` sea EXACTAMENTE el refresco (`/api/guias/facturas-hoy`, que dispara una lectura corta de Switch y no toca una sola guía) — **cualquier otra que alguien cuele mañana lo pone en rojo** —, (b) que ninguna escritura (PUT/PATCH/DELETE) salga a ningún lado, y (c) que a `/api/guias/**` no llegue nada que no sea lectura, salvo el refresco.
> - **No dispara** para quien no puede crear guías (vendedor), ni en modo solo lectura (`fg_guias_readonly`), ni con `GUIAS_ATAJOS_NUEVOS` apagado — cada uno con su caso en el candado.
> - **`/guias/nueva` conserva su disparo** para quien entra directo por URL; el acelerador de 10 min (sessionStorage) + el cooldown del server evitan el doble disparo. Fail-open y `logoutAllSwitchSessions()` en el `finally` del route, intactos.
> - 🩸 **Lección de VOCABULARIO, mía:** Daniel leyó «abrir la lista no manda ningún pedido de escritura» y entendió *pedido de mercancía* (*«guía no es para mandar pedidos»*). En texto que Daniel pueda leer, HTTP nunca se dice «pedido» ni «request»: se dice **leer / escribir**.
>
> ### 2 · 🔴 EL DESTINO SE AUTOLLENA — Daniel quitó su propia regla del 14-ago
>
> Primero: *«sí quiero que se llene sola, ¿ese no era el propósito de todo esto? ¿Cómo que no pre-llenaste la dirección?»*. Y al mostrarle la decisión del 14-ago-2026 («la dirección no se escribe sola»): *«"la dirección no se escribe sola" me refería a que el usuario no lo haga para no escribirlo mal como lo vimos, quita esa regla. Que se autollene como lo discutimos antes.»*
>
> La regla del 14-ago la escribió él **para que la persona no tecleara el destino** (y no naciera otro «Pasocanoas» / «Wesland» / «PENONOME») — no para prohibir que el sistema lo llene. La regla nueva, en `destinoParaAutollenar` (`destinos-clientes.ts`):
> - **Con UN solo destino — definido en su tabla O único en la historia agrupada — el campo Dirección se llena solo al ELEGIR el cliente.** Medido: **40 de 48** clientes con guías usan siempre el mismo destino (el ejemplo que preguntó Daniel: Bouti, S.A. D-14 → «David», 4 guías, siempre igual). Aplica en el selector de cada renglón Y al marcar la primera factura del cliente en el panel (las dos formas de elegirlo).
> - **Con VARIOS destinos no se llena nada**: salen los botones y elige la persona (Sporting Shoes y los pocos así).
> - **Lo escrito no se pisa** (el campo con algo adentro queda como está), **se borra y se escribe encima** como cualquier texto, y **el RENDER de una fila ya cargada no escribe nada** — el autollenado vive en el acto de elegir, no en dibujar un borrador.
> - Al desmarcar la última factura, una fila cuyo único contenido es la dirección AUTOLLENADA (idéntica, sin editar) **se retira**: no era trabajo de nadie. Editada una letra, se conserva.
> - 🔴 **Lo que SIGUE prohibido es el pareo POR PARECIDO**: `Outlet Duty Free N2` ≠ `N3`, «Wesland» (typo) ≠ «Westland». La historia entra ya agrupada por `claveDestino` (regla exacta), jamás por distancia de edición.
>
> ### 3 · 🔴 LA FAMILIA CITY MODA: cada «tienda» es OTRO CLIENTE — el campo tienda NO va
>
> Daniel preguntó *«¿qué es la X en entrega SportCorner?»* y después *«¿hay varios city moda no?»* — y tiene razón: son **ONCE clientes distintos, cada uno con su código** (D-26 Chorrera · D-27 Calidonia · D-28 Albrook 2 · D-29 Central · D-30 Chorrera · D-31 Del Este · D-32 Los Andes · D-33 Santa Ana · D-34 Westland · D-42 Del Norte · D-78 Ismora). Medido el 4-sep: las 8 direcciones del histórico de D-26 son `ALBROOK 2 (ENTREGA EN SPORTCORNER)` · `CALIDONIA (…)` · `CENTRAL (…)` · `LOS ANDES (…)` · `WESTLAND (…)` · `Z15 (…)` · `SPORTCORNER` · `5 DE MAYO`, 1× cada una — o sea **envíos cargados al cliente EQUIVOCADO**: iban a esos otros códigos y quien hizo la guía eligió D-26. **No es un dato que preservar: es el error que este cambio viene a evitar**, y la definición lo ignora.
> - Destino definido de la familia: **«Sport Corner Calidonia»** (la grafía mayoritaria de sus propias guías) para D-27, D-28, D-29, D-31, D-32, D-34 y D-42 — cada uno con UN destino, así que **autollenan**.
> - **D-26 City Moda Chorrera** queda con su único destino propio: **«5 de Mayo»**.
> - **D-30, D-33 y D-78 no tienen guías todavía** y por eso no tienen destino definido.
> - La idea de darle a D-26 un campo «tienda» como Sporting Shoes **se descartó**: las «tiendas» de City Moda tienen código propio; un campo tienda habría perpetuado cargarle a D-26 los envíos de los demás.
>
> ### 4 · La Frontera Duty Free = GUABITO, con candado propio
>
> Daniel, textual: *«en Frontera Duty Free es Guabito, hazme caso.»* Ya estaba así en la tabla; se verificó que la definición gana sobre el histórico real (que dice Changinola 7 / Guabito 4) y quedó **candado dedicado** con la cita (`guias-destinos-cliente.test.ts`) + mutación (`D-87 → Changuinola` muere en rojo).
>
> ### 5 · 🔒 El resumen mensual del grupo va al canal PRIVADO — y corre el DÍA 1
>
> Daniel, textual: *«este mensaje también lo quiero en alertas de Telegram, no en negocio.»* — el mismo motivo del resumen diario de ACS (2-sep): 📊 NEGOCIO es un grupo de tres con el celular de la empresa, y ahí no van los números del grupo. `grupo-resumen-mensual` pasó a **`enviarNegocioPrivado`** en sus DOS salidas (el cron y la recuperación de `switch-reconciliacion`), sin el prefijo `🔧 SISTEMA ·` y sin tocar una coma del texto. Candado: `acs-resumen-canal-privado.test.ts` (extendido).
>
> Y sobre cuándo llega, Daniel: *«sí, lo quiero lo antes posible»* → **del día 3 al día 1** (13:00 UTC = 08:00 Panamá). 🩸 **El «día 3» se sostenía en un dato de OTRO sync**: el comentario citaba `mesesCronDiario` («los syncs cubren el mes anterior los días 1-5»), que vive en `sync-utilidad.ts` y es un margen de la UTILIDAD. Las ventas cierran la misma noche — medido sobre los últimos 4 meses cerrados: la última factura de julio entró el 31-jul 19:15 UTC y la de agosto el 31-ago 19:15 UTC, **cero facturas después** (marzo y abril salen «tarde» solo por un backfill del 29-may) — y el rollup se refresca a las 07:35 UTC y en cada pasada de reconciliación.
> - 🔴 **Guardia nueva del cierre** (adelantar dos días quitó colchón): el mensaje **no sale** si el sync de facturas no corrió con `status='success'` para las **8 empresas** después de que terminó el mes (`switch_sync_log`), o si el rollup no tiene el mes ($0). En vez de mandar un número corto, **lanza error** (logCronError → Telegram interno). ⚠️ **La señal es la CORRIDA del sync, nunca la fecha de la última factura**: Fashion Wear no facturó después del 28-ago y Active Wear del 27-ago estando sanas — negocio, no avería.
> - **El reintento ya existía y solo cambió de días**: la recuperación de `switch-reconciliacion` aplica ahora los días **1-2** (pasadas 14:00/18:00 del día 1 y las tres del día 2), no-op si el día 1 ya mandó (`successSinceIso` desde el día 1).
>
...
- **La lista NO despacha ni edita: solo LEE, nunca escribe una guía.** Ni por swipe ni desplegando un formulario; sus botones solo navegan (editar y despachar viven en `/guias/[id]`). Al tocar Guías se dispara el refresco de facturas de hoy (4-sep-2026, Daniel: «¿por qué no se puede hacer al apretar guías? Prefiero eso.») — una lectura corta de Switch que no toca una sola guía; el candado exige que sea la ÚNICA salida que no es lectura.
...
- 🔴 **El DESTINO se autollena con «EL DE SIEMPRE»** (4-sep-2026, mockup final: «dale aprobado»; Daniel: «sí correcto, con entrega Sport Corner como default, que elija si quiere el otro sino»): cada cliente puede tener UN destino marcado así — en `guias_destino_cliente.el_de_siempre` y en la constante — y ESE se llena solo al elegir el cliente, **aunque tenga varios** (D-26: Sport Corner Calidonia solo; «Chorrera» de botón); sin ninguno marcado, nada (D-142). Un cliente SIN definición autollena su destino **único** de la historia agrupada (Daniel: «no quiero definir cliente por cliente…»). Lo escrito no se pisa y el pareo por parecido sigue prohibido. Detrás de `GUIAS_ATAJOS_NUEVOS`.
- El formulario **no guarda si nada cambió** (`cambios-form.ts`): cargar la guía no puede producir una diferencia contra sí misma.
- **Al crear, el cliente se elige UNA vez y se MARCAN sus facturas** (4-sep-2026, «va»): puente por CÓDIGO (`switch_clientes`, nunca por nombre), solo las 6 del grupo por inclusión, solo tipo `Factura`; «ya salió en otra guía» es **aviso, nunca bloqueo**; elegir cliente **sigue sin ser obligatorio** y el payload que se guarda **no cambia**. La lista abre con **los últimos 3 días CON factura** (no de calendario), cada día con su encabezado en palabras y «Ver más días» +3 — medido: 77% de las facturas usadas en guías salen del último día facturado, 95% de los últimos 3. Todo cuelga de **`GUIAS_ATAJOS_NUEVOS`** (`src/lib/guias/atajos-facturas.ts`): en `false`, la pantalla es exactamente la de antes.
- 🔴 **Son DOS caminos y nada más: factura o «Traslado»** (4-sep-2026, Daniel: «que en factura salga traslado»; descartó «Factura pendiente» y «Sin factura»). El Traslado es **del ENVÍO, no del cliente**, escribe el TEXTO `Traslado` en el mismo campo `facturas` — y así sale en el papel, el PDF y el Excel — y **la empresa se elige a mano** (no hay factura que la diga). Los 58 renglones viejos con `0000` **no se tocan**.
- **La dirección de un renglón es el DESTINO del envío, no la dirección del cliente** (4-sep-2026): `clientes_master` no tiene esa columna y el campo sigue siendo **texto libre**. Bajo el campo hay **botones con los destinos de ese cliente** — los **definidos** primero (ver el bullet de abajo; D-142 Sporting Shoes con 8 y tienda opcional «Westland · tienda 6»), el resto por historia (agrupado **exacto**, grafía más usada, máx. 6). **Los botones se tocan, nunca se aplican solos**; qué se autollena lo decide «el de siempre» (bullet de arriba). Cuelga del MISMO `GUIAS_ATAJOS_NUEVOS` y no cambia lo que se guarda.
- 🔴 **Los destinos definidos viven en la tabla `guias_destino_cliente` y se administran en Guías › Configuración** (4-sep-2026) — pestaña a pantalla completa en `/guias`, **admin Y secretaria** (Daniel: «configuraciones también deja a secretaria»); bodega y vendedor, 403. Nació de dos correcciones textuales: *«city shoes → Calle 19 Central, al lado de la joyería Super Oro. Y Nine Sport en Calle 19 Central.»* — antes cada corrección necesitaba un despliegue. **Precedencia en UNA función** (`destinosDefinidosPara`, `destinos-clientes.ts`): **tabla → constante `DESTINOS_DEFINIDOS` (la red mientras la migración `20260918120000` no corra; las dos correcciones también están ahí) → histórico agrupado**. Soft delete **firmado, NUNCA DELETE**; única entre activas; RLS service_role. Cada fila lleva su marca **«el de siempre»** (columna `el_de_siempre`, a lo sumo UNA activa por cliente — índice parcial; marcar una apaga las demás en el servidor). La pantalla **no toca `guia_items`**: el histórico solo se LEE, como ayuda para promover un destino **de un toque — nunca solo**. Candados: `guias-destinos-precedencia.test.ts` · `guias-destinos-config-route.test.ts` · `guias-configuracion-pantalla.test.tsx`.

**La limpieza del 5-sep-2026** (13 puntos aprobados uno por uno; mapeo en `docs/mapas/guias.md`).

- 🔴 **El número de factura se GUARDA COMPLETO y se MUESTRA CORTO.** Daniel: *«¿Sugieres agregar la factura completa pero que solo se refleje los últimos 4 dígitos?»* → sí. El atajo guarda `11-000002534`; el papel, el PDF, el Excel y la pantalla dicen `2534` (`facturasParaMostrar`). 🔴 **El aviso «ya salió en otra guía» compara los ÚLTIMOS 4 DÍGITOS DENTRO DE LA MISMA EMPRESA** (`claveDeFactura`, `src/lib/guias/numero-factura.ts`): medido sobre las **10.279** facturas de 2026, acotando por empresa dan 10.279 claves distintas (**ni un choque**) y sin acotar 7.830 (**2.449 choques**). Con eso el aviso parea por fin con los renglones viejos escritos a mano. ⚠️ Solo se recorta lo que TIENE forma de secuencial de Switch (`NN-NNNNNN…`): las **12** facturas de 5 dígitos escritas a mano salen tal cual. ⚠️ `Traslado` y el `0000` viejo (**67 renglones**) dan clave VACÍA: si se indexaran, dos traslados de la misma empresa se acusarían entre sí. 🩸 Se retiró `normalizarNumeroFactura` — dos normalizaciones para lo mismo es cómo nace un pareo que no parea.
- 🔴 **`D-201 American Classics` DEJA DE OFRECERSE al armar una guía** (`CODIGOS_RETIRADOS_DE_GUIAS`, `american-classics.ts`), en las tres puertas: el renglón, el panel de facturas y «Atar cliente» — más la búsqueda, «Más usados» y la red de seguridad del selector (`codigosOcultos`). Medido: D-201 **no existe en `switch_clientes` en ninguna de las 6**, tiene **0 facturas** y solo **2 renglones** de agosto; `D-108 Multi Fashion Holding` está en las 6 con **49**. **No se borra nada** y **las 2 guías viejas no se tocan**. ⚠️ El nombre ya estaba resuelto desde el 9-ago-2026: el alias D-108 → «American Classics Store» vive en `lib/clientes/nombre-display.ts` (uno solo, y buscable), con candado que prohíbe repetirlo.
- 🔴 **Bodega corrige los BULTOS al despachar, y queda registro.** Daniel: *«porque bodega si al despachar cuentan más bultos de lo que puso la secretaria, quiero que lo pueda cambiar»* y *«¿queda registro?»* → sí. Una caja por renglón al lado de la del N° del transportista, **solo con la guía PENDIENTE y solo para quien puede despachar** — 🔴 **ya firmada, los bultos NO se tocan** («es lo que el transportista firmó»), y el servidor lo decide por `previous.estado`. Escribe **UNA columna de UNA línea** (`items_bultos`, acotado con `.eq("guia_id", id)`), **nunca por `items`** del PUT. El rastro: «↑ 7 → 8, bodega» en vivo y «Bultos corregidos por … : 7 → 8» después, en `guia_items.bultos_original/_corregido_por/_corregido_en` (migración `20261004120000`, **pendiente**; sin ella el número se guarda igual y la línea no se muestra) + `activity_logs`. **En el papel, el PDF y el Excel sale el número FINAL, sin la historia.**
- 🔴 **Guardar deja de reescribir la guía entera.** Medido: de **549** guardados, **407 (74%)** borraron y recrearon todos los renglones sin que nada cambiara — la guía 85 pasó por eso **45 veces en 3 h 38 min**. Con nada cambiado el botón **no llama al servidor** (`hayCambios`), y los renglones siguen viajando solo cuando cambiaron (`renglonesCambiaron`). ⚠️ **Nadie pierde acceso**: bodega sigue pudiendo cambiar el cliente de un renglón y agregar renglones (Daniel: *«quiero que bodega también pueda agregar algo si sale a último segundo»*).
- 🔴 **La línea «Cerrada en bloque el 3-ago-2026…» deja de MOSTRARSE y NO se borra de la base** (`observacionesVisibles`). Medido: **54 guías vivas** la traen, **51** solas y **3** con una observación real debajo — esas tres se conservan y se siguen viendo. Se compara la LÍNEA entera, no un pedazo. ⚠️ El campo que se EDITA sigue trayendo el texto guardado.
- 🔴 **El ámbar «Salió incompleta» solo marca lo que se puede arreglar**: guías del **10-ago-2026 en adelante** (`FECHA_BLOQUEO_DESPACHO`). Las **65** marcadas son TODAS anteriores y desde entonces son **0 de 45**; esos tres campos siguen cerrados, así que la marca acusaba sin dar salida — y el mismo ámbar marca las **22** a las que sí les falta el N° del transportista. ⚠️ Sin fecha legible se MARCA (ante la duda, se dice). ⚠️ `guiaSinNumeroTransp` **no lleva corte**: ese número sí se anota tarde.
- 🔴 **Cinco columnas RETIRADAS de `guia_transporte`** (sin lectores ni escritores): `firma_transportista` · `nombre_entregador` · `cedula_entregador` (**242 de 242 vacías**), `motivo_rechazo` (**0 filas**) y `monto_total` (**0.00 en las 242**, y viajaba al navegador en cada carga de la lista). **La columna NO se dropea** (patrón `mayor_lineas` / `cxc_favorites`): queda con su `COMMENT` (migración `20261006120000`, **pendiente**) y candado que pone el build ROJO si una migración la borra o si el código vuelve a tocarla. ⚠️ Las DOS firmas que SÍ se usan (`firma_base64`, `firma_entregador_base64`, 70% llenas) **no se tocan**.
- 🔴 **El estado «Rechazada» se retiró** (Daniel: *«quitarlo»*): **0 de 242** guías en toda la historia. `guiaYaDespachada` solo reconoce «Completada», el PATCH ya no acepta `motivo_rechazo` y los juegos frecuentes piden `.eq("estado", "Completada")`.
- 🔴 **«Compartir» manda IMAGEN hasta 6 renglones y PDF de ahí para arriba** (`formatoParaCompartir`, `MAX_RENGLONES_PNG = 6`). Daniel: *«en el grupo de WhatsApp siempre ponen compartir cuando terminan (llega en pdf)»*. Medido: **60% de las guías tienen 1 renglón, 79% tres o menos, 94% seis o menos**. El botón se sigue llamando «Compartir» y **decide solo**; **imprimir no cambió**. 🩸 La imagen se dibuja con el API 2D **sin un solo `await`** (`toDataURL`, no `toBlob`) y las firmas se **precargan al abrir la guía**: en iOS un `await` en el medio le quita el gesto a la hoja de compartir. Una firma que no esté decodificada **no se inventa**. ⚠️ `png-guia.ts` **no arrastra jsPDF**.
- **Un solo «Guardar Guía»** (el de abajo, donde vive el aviso «Falta: …»; la barra pegajosa conserva «Sin guardar / Listo, guardado») y **la caja de BULTOS arranca vacía**, sin el `0` gris que había que borrar.
- 🔴 **«Changuinola» con «u»** (Daniel: *«es changuinola»*). La lista que el formulario OFRECE (`DEFAULT_DIRECCIONES`) decía «Changinola», así que la grafía mala se tocaba y se repetía: medido, **26 renglones vivos** mal escritos contra **1** bien, mientras los dos destinos definidos de ese pueblo (D-156 y D-147) ya decían «Changuinola» — el mismo pueblo contaba como DOS destinos. Migración `20261005120000` (**pendiente**), acotada al **valor exacto**, nunca un `LIKE` suelto.
- Candados del 5-sep: `guias-numero-factura.test.ts` · `guias-american-classics.test.ts` · `guias-bultos-de-bodega.test.ts` · `guias-restos-y-ambar.test.ts` · `guias-compartir-png.test.ts` · `guias-bultos-y-guardar.test.tsx`; **50 mutaciones, 50 cazadas** (`scripts/_mutar-candados-guias-rediseno.sh`, con 2 controles).

**Cuatro defectos del 11-sep-2026** (auditoría del grupo 3; ninguno cambia lo que se guarda).

- 🔴 **«Solo pendientes» se VE y se puede QUITAR.** 🩸 El rediseño del 5/6-sep se llevó el botón «Ver pendientes / Ver todas» y dejó vivos el estado, el filtro y el enlace de ⌘K: desde `/guias?pendientes=1` la lista quedaba en **1 de 229** sin chip ni botón que lo dijera, y recargar lo volvía a aplicar — la única salida era escribir `/guias` a mano. El chip lo apaga y **limpia la dirección** (`replace`, mismo nivel): con el `?pendientes=1` puesto, recargar volvería a filtrar. Módulo puro `lib/guias/filtro-pendientes.ts`.
- 🔴 **El aviso «N guías sin despachar» LLEVA a la guía, esté dibujada o no.** 🩸 Se calcula sobre TODAS y la lista dibuja el último mes y lo que pasa el buscador: con algo escrito, tocarlo marcaba `expandedId` de una fila que no existe y **no pasaba nada**; una pendiente de +30 días quedaba detrás de «Ver guías más viejas», contra el invariante de que la pendiente sube arriba. Ahora se abre el camino primero —se limpia el buscador y/o se abren las viejas— y recién ahí se expande; si la guía **no está en la lista cargada, se navega a su página**. La decisión vive en `lib/guias/llegar-a-la-guia.ts` (puro), no en la pantalla.
- 🔴 **`/guias/nueva` rebota al vendedor en el SERVIDOR.** 🩸 La página lo aceptaba y el POST lo rechazaba: se armaba la guía entera —cliente, renglones, facturas, bultos— y el único aviso llegaba al tocar «Guardar Guía» (el «＋ Agregar destino» ya le había contestado 403). La lista de roles de ESCRITURA estaba escrita **seis veces** (la página y cinco `route.ts`) y ahora vive en **`src/lib/guias/roles-escritura.ts`**; hay barrido que prohíbe volver a escribirla a mano. ⚠️ El vendedor **no pierde Guías**: las sigue viendo en solo lectura.
- 🩸 **En una guía despachada el aviso del botón gris salía VACÍO.** Con `soloCorregible` la lista de faltantes es `[]` y `textoFalta([])` devuelve `""`: botón gris, `title` vacío y un `<p>` ámbar sin una palabra adentro. Ahora dice **«Ya se despachó: solo se corrigen N° del transportista, cliente y facturas»**, 🔑 **derivado de `CAMPOS_DESPACHADA`** (`textoYaSeDespacho`), no escrito a mano: el día que se abra o se cierre un campo, la frase se corrige sola.
- Candados del 11-sep: `guias-filtro-y-aviso.test.ts` (22) · `guias-filtro-y-aviso.test.tsx` (8, de conducta). Dos candados cambiaron de dirección con nota fechada, ninguno se borró: `guias-numero-transp-tarde-route` (los roles se comprueban sobre el módulo único) y `guias-papel-y-marcas` (monta `NuevaGuiaClient`, que es donde vive el formulario).


**El panel y el teléfono (5-sep-2026)** — cuatro cambios de cómo se VE una guía (Daniel: *«van todos»*) y seis del PANEL (*«no, mándalo así»*). Ninguno cambia lo que se guarda. Medido: **222 guías vivas · 532 renglones · 7.564 bultos**, idénticos antes y después.

- 🔴 **En el teléfono los renglones son FICHA, no tabla.** La tabla de 6 columnas del acordeón pide 600 px dentro de un iPhone de 390 —**210 px de arrastre lateral**— y medido, **127 de 222 guías (57%) llevan UN solo renglón** y **172 (77%) tres o menos**. De `lg:` para arriba la tabla **se queda**. 🔴 No se inventó un formato: es el de `ListaEnvios`, sacado a `ResumenEnvio.tsx` para que las dos pantallas dibujen lo MISMO — y `ListaEnvios` no cambió un píxel. La celda «Cliente» vive una sola vez en el archivo (`celdaCliente`), con el chip y «Atar cliente» intactos; en la ficha sube de 12 a **14 px**.
- 🔴 **Las firmas, plegadas al MIRAR — y ENTERAS al firmar.** Una línea «✓ Firmada por las dos partes» con «Ver firmas» (`FirmasPlegadas`, `lib/guias/firmas-resumen.ts`), en las DOS pantallas de lectura (el acordeón y `/guias/[id]`). Medido: de 221 despachadas, **156 tienen las dos firmas y 65 no tienen ninguna** — con ninguna **no se dice nada** (el ámbar «Salió incompleta» ya las marca); con una sola se dice **cuál falta**. 🔴 `SignatureCanvas` **no se tocó**: al despachar siguen los dos cuadros de 150 px. El papel, el PDF y la imagen siguen con la firma entera.
- 🔴 **La cédula se muestra con guiones, y lo guardado no se toca** (`lib/guias/cedula.ts`, función pura aplicada al LEER). Medido sobre las 156 guías con cédula: **54 cambian de forma en pantalla, 102 no**. 🩸 **No hay regla general y por eso se MIDIÓ**: `810102403` es `8-1010-2403` (tomo de CUATRO) y la regla ingenua «provincia, 3, el resto» habría dado `8-101-02403`, que es otra cédula. Los **14 pares** en que la misma persona quedó escrita de las dos formas dicen la regla entera: **tomo de 3, salvo que eso deje un folio de más de 4 — ahí 4**. 🔴 **Lo que no parece una cédula se muestra TAL CUAL** (`Co272797`, `Bx4289`, `8-930`, `8-9302142`, 10 dígitos). ⚠️ **No se aplica en `DespachoForm`**: ahí la cédula se ESCRIBE y se guarda. ⚠️ La imagen de WhatsApp **no muestra la cédula** (nunca la mostró).
- 🔴 **«Lista plana / Agrupar por fecha» se RETIRÓ: la lista va SIEMPRE agrupada por fecha.** Daniel: *«el chip por fecha y todos quítalo. Siempre ordenado por fecha»*. El botón nombraba el DESTINO, no dónde estabas. Lo agrupado no cambió.
- 🔴 **El chip verde «despachada» se fue** (salía en **221 de 222**): *«un color que sale siempre deja de avisar»*. Solo se pinta lo que ESPERA. El borde izquierdo esmeralda **no se tocó** — decisión pendiente de Daniel.
- 🔴 **Arriba va UNA línea con lo que falta** y LLEVA a esa guía: «1 guía sin despachar — hace 4 días» (`lib/guias/pendientes-arriba.ts`, mismo motor de días que el aviso de Telegram). Sin pendientes **no aparece**. 🩸 El banner viejo salía solo con `role === "bodega"`: Angela y andrea, que crean el 99% de las guías, nunca lo vieron.
- 🔴 **La pendiente sube ARRIBA de la lista, fuera de los grupos de fecha, con «Despachar» a la vista.** Hoy es **1 de 222** (GT-239, del 1-sep).
- 🔴 **«Imprimir» y «Compartir» pasaron a la FILA, sin desplegar; «Editar» y «Eliminar guía» al «···».** Se MOVIERON, no se duplicaron: el bloque de acciones del acordeón se retiró. 🩸 El «···» tenía UNA sola opción y solo lo veía quien puede borrar, así que **bodega no tenía menú**; ahora tiene el suyo, y «Eliminar guía» sigue siendo solo de admin y secretaria. 🩸 **El papel se pide COMPLETO**: `GET /api/guias` deja las firmas afuera a propósito (**7,3 MB** medidos en las 156 firmadas, ≈47 kB por guía), así que imprimir la guía de la lista tal cual habría sacado un papel **SIN FIRMAS**. La fila pide `/api/guias/[id]` una sola vez por guía y **arranca la lectura en el `pointerdown`**, porque en iOS la hoja de compartir y el visor de PDF tienen que abrirse dentro del gesto del toque.
- 🔴 **La lista abre con el ÚLTIMO MES** (`lib/guias/ventana-lista.ts`, 30 días, reusando `partirPorVentana` de los comprobantes) y el resto detrás de **«Ver guías más viejas»**. Medido: **46 de 222**. 🩸 Antes traía las 222 y mostraba 15 con un «Ver más» que había que tocar **14 veces** para llegar a la primera. El corte se normaliza al **día de Panamá** (con la hora cruda la ventana valía 29 días a la tarde). ⚠️ La ruta sigue sin `.limit()`: esto decide qué se DIBUJA, no qué se pide.
- 🔴 **El orden de lo que se lee, en las dos pantallas.** 🩸 Lo más grande y en negrita era el **transportista**, que en pantalla tiene **7 etiquetas distintas** (6 del catálogo + «Entrega directa»), y el **cliente** —lo que distingue una guía, **49 códigos**— era la línea gris. Escritorio: `Guía · Cliente · Destino · Bultos · Transportista`. Teléfono: cliente arriba en negrita, debajo en gris `destino · N bultos · transportista`, y el número y la fecha chicos al final. **La fecha sale de la fila** (el encabezado del día ya la dice) y **el estado también**.
- Candados: `guias-cedula-con-guiones.test.ts` · `guias-firmas-plegadas.test.ts` · `guias-ventana-y-pendientes.test.ts` · `guias-en-el-telefono.test.tsx` · `guias-panel-que-se-lee.test.tsx`. Seis candados **cambiaron de dirección con nota fechada**, ninguno se borró: `guias-numero-factura` (la ficha reemplaza a `ListaEnvios` como superficie), `guias-anotar-numero-tarde` (las firmas, después de un toque), `guias-eliminar-en-la-fila` (el «···» lleva dos ítems y bodega tiene menú), `guias-entrega-directa` y `guias-sin-rechazo` (los botones se leen en la fila), `guias-despacho-una-sola-puerta` (la tira de acciones) y `guias-lista-834-columnas` (una vez por LAYOUT, no tres en el archivo).

**La lista de destinos, el papel y el ＋ (7-sep-2026)** — cinco cambios aprobados por Daniel. Medido antes y después: **225 guías vivas · 536 renglones · 7.599 bultos**, idénticos (`scripts/_medir-guias-destinos-compartidos.mjs`).

- 🔴 **LA LISTA DE DESTINOS DEL CAMPO DIRECCIÓN ES DEL EQUIPO, NO DE UN NAVEGADOR.** Daniel: *«lo de solo ver en mi pantalla no tiene lógica, el sistema debe de trabajar todo igual, que sea para todo»* y *«quítame hola»*. 🩸 Vivía en `localStorage` (`fg_direcciones`, `loadList`/`saveList`): lo que agregaba Angela no lo veía nadie más y **no se podía quitar desde ninguna pantalla** — así quedó vivo para siempre un destino de prueba llamado «hola» en un solo navegador. Ahora vive en **`guias_destino_lista`** (migración `20261014120000`, **pendiente**) y se administra en **Guías › Configuración**, en su propia tarjeta. ⚠️ **NO se fusiona con `guias_destino_cliente`**: aquélla son los destinos DE UN CLIENTE (botones y autollenado), ésta la lista general sin dueño. **Agregar** es de quien arma la guía (admin · secretaria · bodega, el ＋ del campo); **quitar**, de quien administra (admin · secretaria). 🔴 **Soft delete firmado, NUNCA DELETE.** El repetido se rechaza por `claveDestino` (regla exacta, jamás por parecido): «DAVID» no entra dos veces. **El código no degrada sin la DDL**: la lectura falla ABIERTA a `DESTINOS_BASE` (los cinco de siempre) y el GET contesta 200 con lista vacía.
- 🔴 **La semilla sale de lo que de VERDAD se usa, nunca del `localStorage` de nadie** — por eso «hola» no entra. Medido: 77 grafías vivas → **55 destinos** agrupados por `claveDestino`; entran los de **3+ usos = 17**, que cubren **490 de 536 renglones (91,4%)**. Se siembra la **grafía más usada**, salvo cuando Daniel ya definió esa grafía en `guias_destino_cliente` (ahí manda la suya: «Penonomé», no «Penonome»; «Chorrera», no «CHORRERA»). ⚠️ Quedan sembrados «CALLE 19» (13 usos) y «Wesland» (4, el typo de Westland) porque son uso REAL — la diferencia es que ahora se quitan de un toque.
- 🔴 **El destino repetido no se dibuja.** Daniel: *«si el cliente tiene un solo destino y ya está escrito en el campo, no dibujar el botón. No ofrece nada.»* Con **varios** se dibujan todos, incluido el que coincide. La regla vive en `botonesQueSeDibujan` (módulo puro); ⚠️ la fila de TIENDAS se conserva — es otra pregunta.
- 🔴 **UN SOLO BOTÓN DE IMPRIMIR, Y ES EL PDF.** 🩸 `GuiaDetail` hacía `window.print()` sobre la PANTALLA, que vive dentro de `HojaEscalada` con un `transform: scale(…)`: Daniel imprimió y le salió *«una foto impresa de la guía y no como estaba antes en pdf bien bonito»*. Ahora los tres botones del módulo llaman a `imprimirGuia`. ⚠️ El bloque `@media print` de `HojaEscalada` **no se toca**: cubre el Ctrl+P del navegador.
- 🔴 **Compartir mira el APARATO: celular → imagen hasta 6 renglones (lo de siempre); computadora → SIEMPRE el PDF.** La imagen existe para WhatsApp, que la muestra dentro del chat. El aparato se reconoce **por el dedo** (`pointer: coarse`), no por el nombre — el iPad se anuncia como «Macintosh» (`src/lib/aparato.ts`, con `esIOS()` de plan B). ⚠️ **Imprimir no cambia**: el papel es el PDF en los dos mundos.
- 🔴 **El «＋» del campo lleva rótulo VISIBLE** («＋ Agregar destino»). Antes solo lo explicaba el `title`, que **aparece pasando el mouse por encima — y en el iPad no hay mouse**. ⚠️ El ＋ de «quién despacha» sigue pelado (hay control).
- Candados: `guias-destinos-compartidos.test.ts` · `guias-papel-uno-solo.test.ts` · `guias-destinos-compartidos-pantalla.test.tsx`; **39 mutaciones, 39 cazadas** con 2 controles (`scripts/_mutar-candados-guias-destinos-compartidos.sh`). Dos candados **cambiaron de dirección con nota fechada**, ninguno se borró: `guias-consistencia-despachada` (el ＋ va con rótulo, con el control al revés en «quién despacha») y `guia-pdf-compartir` (Imprimir manda el PDF, con el control de que el papel no cambió).

**Varios clientes en una guía, y los días plegados (10-sep-2026)** — tres cambios de PANTALLA en el panel «Facturas del cliente» de Nueva guía. 🔴 **Nada de lo que se guarda cambia** (`instantaneaRenglones` compara byte a byte la guía de DOS clientes armada con el atajo contra la misma escrita a mano) y todo cuelga del MISMO `GUIAS_ATAJOS_NUEVOS`: en `false`, la pantalla es la de antes.

- 🔴 **UNA GUÍA LLEVA FACTURAS DE VARIOS CLIENTES, DE A UN CLIENTE A LA VEZ.** Daniel: *«Una guía lleva facturas de varios clientes en un mismo despacho»*, *«un cliente a la vez»*, *«se quedan abajo»*, *«un renglón por cliente-empresa, pueden haber más de una factura en ese renglón»*. El botón **«+ Otro cliente»** limpia **el BUSCADOR y nada más** — los renglones ya marcados no se tocan (`items` nunca se limpiaba al cambiar de cliente; lo que faltaba era la invitación). **Reusa el MISMO `ClientePicker`**, no nace un segundo selector. Se dibuja solo cuando ese cliente ya dejó algo en la guía. 🩸 El foco vuelve al buscador **por un efecto, no en el acto**: enfocando antes del re-render, el `onFocus` del selector copia al campo el nombre del cliente ANTERIOR y el campo se ve «limpio» con el nombre viejo escrito adentro.
- 🔴 **SOLO EL DÍA MÁS RECIENTE VIENE ABIERTO.** Daniel: *«al menos déjame poder desplegar más días y que no me llene la pantalla»*, *«que ya venga plegado solo el último día desplegado by default»*. Caso real: Sporting Shoes N 4 (D-142), 3 días de 12-17 facturas tapaban la pantalla entera — y justo debajo está «Detalle de Envío», que es donde se ve lo marcado. Los demás días muestran **el título y cuántas facturas tienen** («Jueves 23 jul · 12 facturas») y se abren de un toque; **«Ver más días» los trae PLEGADOS**. 🔑 **Qué está abierto se DERIVA, no se guarda**: el estado son los días que la persona ALTERNÓ, y abierto = (es el más reciente) XOR (lo alternó) — así los días nuevos nacen plegados sin ningún efecto de inicialización, y un día abierto a mano sigue abierto cuando llegan más. Cambiar de cliente reinicia. ⚠️ Un día plegado **DICE cuántas de sus facturas ya están marcadas**; abierto no lo repite (las casillas se ven).
- **Las filas se aprietan en la COMPUTADORA** (`lg:[@media(pointer:fine)]`), nunca por ancho a secas: el iPad tiene 1024 px y se toca con el dedo. Con el dedo siguen siendo **44 px**. La fila **envuelve** (`flex-wrap`): todo en ella es `shrink-0` menos el nombre de la empresa, así que en 390 px la etiqueta «Ya salió en GT-XXX» baja un renglón en vez de arrastrar la página de lado.
- 🔴 **«Ya salió en GT-XXX» se queda COMPLETA** (Daniel: *«el ya salió no me molesta»*), sigue siendo AVISO y nunca bloqueo, y el buscador del panel sigue **sin salida a mano** (las facturas viven amarradas al CÓDIGO).
