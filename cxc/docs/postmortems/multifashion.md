...
- ✅ **Maher (resuelto el 23-sep-2026, después de este encargo).** Daniel: *«métel[o] para no hacer excepciones por solo una persona»*. `fuera-de-seguimiento.ts` quedó **VACÍO** (el mecanismo se conserva, por CÓDIGO y nunca por nombre) y VENTAS MAHER es un cliente como cualquiera en TODO el módulo. Medido contra producción ese día: «No vuelven» **721 → 723**, «Nuevos» **52 → 52**, «Todos» **983 → 986** —son sus TRES códigos (47 con 76 visitas y última compra el 17-sep; 48 y 49 con una compra cada uno del 5-jun-2024, que caen en «No vuelven»)— y las CUATRO tarjetas quietas (96 · 47 · 723 · 862), porque el universo de fidelización siempre lo contó. Ninguno de los tres tiene teléfono en Switch: salen sin botón de WhatsApp. La Frontera (324) sigue fuera del ranking por código: es mayoreo, no una excepción por persona. Candado: `multifashion-clientes-seguimiento.test.ts` § 4-bis, con tres mutaciones en rojo (devolver el 47 a la lista → 3 rojos; comparar por nombre con `toLowerCase().includes("maher")` → 1 rojo; una exclusión sin motivo escrito → 2 rojos).
...
- 🔴 **EL MÓDULO SE CIERRA A QUIEN NO LO TIENE (6-sep-2026).** 🩸 `src/app/multifashion/page.tsx` **no comprobaba ningún rol**: `/ventas` manda a casa a quien no es admin en dos líneas y ésta no tenía ninguna, mientras el middleware solo valida que la sesión EXISTA — o sea, **cualquiera con sesión** (bodega, un vendedor, contabilidad) abría esa dirección y el resumen de la tienda le llegaba **ya escrito en el HTML**. Y **10 de las 11 rutas** de `/api/multifashion/*` dejaban entrar a `secretaria`, dos además a `contabilidad`, aunque **ninguno de los dos roles tiene el módulo** en `src/lib/modules.ts`. Daniel, al preguntarle si lo cerraba igual aunque las secretarias perdieran el avance de las metas: **«A»** — ciérralo igual; con la pestaña «Multifashion» de Comisiones ven lo que necesitan. Ahora hay guard SSR en la página (antes de cargar la data) y **UNA sola lista de roles**, `src/lib/multifashion/acceso.ts`, **derivada de `modules.ts`** (`ROLES_MULTIFASHION` = admin + `gerente_acs`). `ROLES_LECTURA_METAS` pierde `secretaria`; `ROLES_ADMIN_METAS` no cambia (solo admin). ⚠️ **La ÚNICA excepción son las DOS rutas que alimentan la pestaña espejo de Comisiones** —`vendedoras` y `bonos`, con `ROLES_VENDEDORAS_ESPEJO` = los del módulo ∪ los de `/comisiones`—: es lo que esa pestaña muestra, no una puerta al módulo. ⚠️ **Jennifer no pierde nada** (sigue en las 11 rutas y en las metas) y **los crons tampoco**: el resumen diario de ACS, la fidelización y el ritmo de la meta leen la BASE directo y entran con `CRON_SECRET`, nunca por HTTP. Candado: `multifashion-cerrado-y-espejo.test.ts` (pone el build ROJO si nace una ruta del grupo con la lista escrita a mano, o si la página se queda sin guard).
...
**El rediseño del módulo (6-sep-2026).** Daniel revisó las SEIS pantallas del escritorio y DIEZ del teléfono y decidió punto por punto. 🔴 **Ningún número se movió**: septiembre sigue valiendo **$10.867,09**, el año **$369.153,24**, los tiquetes **213** y la proyección **$65.202,51**. Lo único que puede cambiar de valor es el ranking de Vendedoras cuando corra la migración del amarre — y ahí cambian las FILAS, no la suma.

- 🔴 **SEIS PESTAÑAS → CUATRO: Resumen · Vendedoras · Productos · Clientes** (`src/lib/multifashion/pestanas.ts`). **Metas se mudó ENTERA adentro de Vendedoras** —la tarjeta de avance, «Nueva meta», «Cambiar», el premio, el rango de fechas, la historia y el aporte de cada una— Daniel: *«de acuerdo, ponerlo en vendedoras, pero el tab de metas no es idéntico, tiene más cosas útiles»*. **Caja se retiró de la navegación**: se abrió **8 días en toda su historia** (3-jul → 14-ago-2026), 3 de ellos en cero. ⚠️ **La razón es que nadie la usa, NO que abra una sesión en Switch** —eso era falso: tiene caché por día—; su ruta, su caché y su componente **no se borran** (patrón `mayor_lineas`). Todo `?subtab=` viejo redirige, y 🩸 **la basura ya no deja la pantalla en blanco**: cae en Resumen.
- 🔴 **«Multifashion» en TODOS LADOS** (Daniel: *«multifashion en todos lados»*). El título de la pantalla decía **«American Classics»** (`app_settings.multifashion_tienda`) y ahora dice Multifashion. La ruta `/multifashion` y la clave `american_classic` **no se tocan**.
- 🔴 **UN SOLO CONTROL DE TIEMPO** (`src/lib/multifashion/periodo.ts`, `?mfPeriodo=`). 🩸 Había **TRES a la vez**: el año arriba a la derecha, el mes con flechas ‹ › más abajo, y píldoras propias en Vendedoras (**seis**, tres filas en el teléfono) y en Clientes (**cuatro**, que decían el LARGO de la ventana y nunca el mes de corte — gobernado por un selector que en esa pestaña **ni se dibujaba**). Queda un desplegable `Septiembre 2026 ⌄` con los meses (el año adentro), el año completo y últimos **3 · 6 · 12** meses. 🔴 **Cada pestaña ofrece SOLO lo que sabe servir** —el Resumen es el detalle de UN mes; Productos consulta `periodo=mes|12m`— y un período que la pestaña no sirve **cae a SU MES**, nunca a otra cosa en silencio. El corte es el mes de **Panamá**.
- 🔴 **EL BONO DEJÓ DE SER UNA BARRA: es una COLUMNA que dice «al cierre»** mientras el mes no termine. ⚠️ Ni el monto ni la regla del bono se tocaron. ⚠️ **La pestaña espejo de Comisiones NO se toca**: monta el MISMO componente sin `periodo` ni `conMetas`, así que conserva sus seis píldoras y no pide metas (le contestarían 403).
- 🔴 **Clientes abre con la COBERTURA en una línea** (`clientes-cobertura.ts`), recalculada para el período: *«19% de los tiquetes con nombre — el 27% de la venta»* (agosto 2026 medido: 224 de 1.152 tiquetes · $14.287,83 de $53.148,61). **Sin tiquetes se abstiene** — nunca «0% — 0%». La lista abre con **10 filas** y ofrece **«Ver los 33»** (dibujaba las 33 de una, bajando hasta $20,72), y el **bloque de mayoreo vacío NO aparece** (5 facturas en todo 2026, en 3 meses de 9).
- **Los nombres se capitalizan** en pantalla (convivían «Martin Montenegro» y «MARIA APARICIO»), reusando `nombreVendedorEnPantalla` de Comisiones. 🔴 **Solo cambia cómo se MUESTRA**: la clave de agrupación sigue en mayúsculas y no se mueve un número.
- 🔴 **EL AÑO SUBIÓ A LAS TARJETAS.** Cuatro: **Ventas del mes · Tickets · Cierra en · Año 2026**. El desplegable **«Panorama del año» se retiró** y lo que traía (venta del año, proyección de cierre vs el año pasado, margen de la tienda y la nota de mayoreo) vive en la tarjeta del año. **Nada se perdió; dejó de estar guardado.**
- 🩸 **«CUÁNDO VENDE LA TIENDA»: UNA sección, y cada línea dice DE QUÉ PERÍODO HABLA.** Medido el 6-sep-2026: «mejor día de semana **Sáb, $3.364,19 promedio**» y «mejor día del mes **$3.364,19, el 5 de septiembre**» eran **EL MISMO NÚMERO** — ese «promedio» era UN solo sábado, porque la tarjeta miraba nada más el mes. **«Día más fuerte» y «Hora pico» pasan a los ÚLTIMOS 3 MESES** (`patrones.ts`): sábado **$2.640,52 sobre 10 sábados** y la hora pico de las 4 pm ($1.624,40 de un mes) a las **3 pm ($15.209,52)**. **«Mejor / peor día» sigue siendo del MES.** 🔑 El promedio de N meses **no es el promedio de sus promedios**: se reconstruye la suma (`promedio × días`), se suman sumas y días, y recién ahí se divide. **Sin RPC nueva ni DDL**: se piden los mismos dos procedimientos, uno por mes de la ventana (16 ms y 6,5 ms medidos).
- 🔴 **La proyección dice sobre cuántos días está hecha: «con 5 días».** Medido: $65.202,51 = $10.867,09 ÷ 5 × 30, al centavo. ⚠️ **La fórmula NO se tocó** —el dato (`dia_corte`, `dias_mes`) ya viajaba en `proyeccion_mensual_retail_v1` y solo no se mostraba— y el número **no está inflado**: con la forma real de septiembre de 2025 daría $74.077. Es la misma regla que el módulo ya aplica en Metas.
- 🔴 **El encabezado del teléfono, de SEIS bloques a TRES**: título + período · la línea de hoy · las pestañas. «Sincronizado …» y «Actualizar ahora» se fueron al **menú ☰** (`AppHeader acciones=`, aditivo: ningún otro módulo lo pasa); en el escritorio siguen a la vista. Y **«Hoy» pasó de una franja de CUATRO renglones a UNA línea**: `Hoy · domingo 6 sep · sin ventas todavía · 2:00 p. m.` — 🔴 sigue sin escribir «$0» cuando el día no arrancó y la frescura sigue viajando SIEMPRE.
- 🔴 **LAS VENDEDORAS CON DOS CÓDIGOS SE JUNTAN** (migración `20261009120000_multifashion_vendedora_alias.sql`, **aplicada** — verificado contra producción el 18-sep-2026: 3 amarres activos desde el 7-sep, `multifashion_vendedora_canonica` viva y la vista con `vendedor_canonico`; esta línea decía «pendiente de aplicar» y era falso). El 20-jun-2026 alguien **creó tres vendedoras nuevas en Switch** en vez de usar las existentes: `ANA TREJOS` (12→3) · `CINDY DE GRACIA` (13→8) · `YEISIBETH MUÑOZ` (14→10), **$5.436,96 en 121 documentos** que en la vista de 12 meses aparecen al fondo del ranking como tres personas más. 🔑 **La identidad es el CÓDIGO**, así que el amarre es código → código; tabla firmada, **soft delete, nunca DELETE**, única solo entre las activas, RLS service_role. Se resuelve en **UNA función** (`multifashion_vendedora_canonica`) que la vista `_multifashion_sf_vw` expone como `vendedor_canonico`, y de ahí la usan **todas** las superficies: `multifashion_vendedoras_v4`, `multifashion_vendedoras_range_v2` y `multifashion_bonos_v4` (= las v3 con esa columna y **nada más**). Las rutas llaman a la v4 y **caen a la v3** mientras la DDL no corra. Medido: 13 filas → **10**, con la venta y la comisión **idénticas al centavo** ($671.180,81 y $3.252,21 en 12 meses).
- ⚠️ **JUNTAR LOS CÓDIGOS NO ARREGLA LA DIFERENCIA ENTRE EL TOTAL DE VENDEDORAS Y EL DEL MES**, y el encargo suponía que sí. Medido el 6-sep-2026: el mes vale **$10.867,09** y las vendedoras suman **$10.570,66** — los **$296,43** que faltan son **`DEFAULT`**, el marcador de Switch para «sin vendedor», que las RPC excluyen a propósito. Reagrupar filas reparte la misma suma en menos renglones: no puede cambiar un total.
- Candados: `multifashion-rediseno.test.ts` (60) · `multifashion-rediseno-pantalla.test.tsx` (8); **45 mutaciones, 45 cazadas** con 2 controles (`scripts/_mutar-candados-multifashion.sh`); medición `scripts/_medir-multifashion-rediseno.mjs`.

**Los defectos del 11-sep-2026** (auditoría del módulo, aprobados por Daniel; **ningún cálculo cambió**).

- 🩸 **El año se decía DOS veces con números distintos.** La tarjeta «Año» usa `overview.retail.ytdVentas` (los 12 meses) y la fila «YTD» de «Mes a mes» sumaba SOLO los meses con base del año anterior. Medido: **2025 → $652.420,19 arriba y $509.291,64 abajo** (ene–abr 2025 no tienen 2024). Ahora la fila se llama **«Año»**, su total ES el de la tarjeta (`fila-anio.ts`, módulo puro), el Δ se sigue midiendo sobre los meses comparables y, cuando no son todos, la tabla lo dice: *«Δ sobre los 8 meses que tienen 2024: $509,291.64 vs $477,843.10»*. ⚠️ En el año en curso el total de la tarjeta puede diferir de la suma de los meses por las ventas del día de corte (los meses van «al mismo corte», la tarjeta al día): medido el 11-sep, 2026 dice $375.573,44 en la tarjeta contra $374.793,11 sumando meses — la fila dice ahora lo de la tarjeta.
- 🩸 **Jennifer (`gerente_acs`) no tenía «Actualizar ahora»**: el shell montaba `SyncNowButton` sin `roles` y el default es admin + secretaria. Ahora `roles={ROLES_MULTIFASHION}`, y la lista del `useAuth` del shell —la última copia a mano— también se lee de `acceso.ts`.
- 🩸 **El rol se mostraba crudo** («gerente_acs», «gerente_boston») debajo del nombre: `AppHeader` y `Sidebar` tenían cada uno su `ROLE_LABELS`, sin esos dos y con un `cliente` que no existe. Ahora hay UN lugar, `src/lib/roles-etiquetas.ts`, DERIVADO de `SYSTEM_ROLES`: «Gerente Multifashion» (*«Multifashion en todos lados»*) y «Gerente Boston»; el candado exige que cubra los siete roles y nada más.
- El desplegable de período lleva `placeholder` («Elige el período»): el día 1, antes del primer sync, el mes de corte no tiene opción y quedaba en blanco.
- Candados: `multifashion-anio-una-vez.test.ts` · `roles-etiquetas.test.ts`. Medición: `scripts/_medir-catalogos-multifashion-defectos.mjs`.

**Ventas por redes: «REDES Sheynee» es Sheynee (18-sep-2026).** 🔴 Toca la comisión de Multifashion.

- **Lo que Daniel decidió, textual:** *«tendría que sumarse ambos vendedores para lo de la comisión ya que sigue siendo la misma persona»* · *«quiero el sistema limpio y minimalista»*. Quiere medir lo que la tienda vende por WhatsApp e Instagram; creó en Switch el vendedor **`REDES Sheynee`** (id interno **15**, por la dirección `…/vendedores/editar/15`) y Sheynee Batista es la única que vende por ahí.
- 🔑 **El id interno NO es la columna «Código» de la pantalla de Switch.** Medido: Sheynee Batista se ve con Código 10 y está guardada como **`vendedor_switch_id = 11`** en `switch_facturas` (1.942 documentos en 12 meses); la 10 es Yeisibeth Muñoz. En el 15 coinciden por casualidad. Por eso el amarre es **15 → 11**, no «15 → 10».
- 🔴 **El canal es una columna del MISMO amarre, escrita a mano.** `multifashion_vendedora_alias.canal` (`NULL` = mostrador, `'redes'`; CHECK cerrado), resuelto en `multifashion_vendedora_canal(codigo)`, que lee la tabla y **nada más**. **Nunca se busca «REDES» en el nombre**: sería parear por parecido. La vista expone `vendedor_canal` al final.
- 🔴 **UNA sola fila en el ranking, con el desglose adentro.** `multifashion_vendedoras_v5` y `..._range_v3` son las v4/v2 **byte a byte** (candado que lo compara) salvo que el CTE `actual` agrupa primero por (persona, canal) y después por persona, y la fila lleva `por_canal` (`{"redes": 1717.73}`) **solo si un código con canal vendió en el período**; para las demás va `NULL`. Ventas, tickets y comisión salen JUNTOS. La pantalla (fila y tarjeta) escribe debajo del nombre **«Sheynee $11,674.57 · Redes $375.30»** (`lib/multifashion/canales.ts`, puro; la tienda se DERIVA: total − canales) y a las demás no les agrega nada.
- 🔴 **LA LÍNEA DICE EL NOMBRE DE LA VENDEDORA, Y «REDES» CON MAYÚSCULA (24-sep-2026).** Decía «tienda $11,674.57 · redes $375.30» — «tienda» no es nadie. Ahora el rótulo de la parte derivada es el **PRIMER nombre** capitalizado (el completo ya está en negrita arriba, en la misma fila): computadora **«Sheynee $11,674.57 · Redes $375.30»**, celular **«Sheynee $11,675 · Redes $375 · 275 tiquetes»** (Sheynee Batista, septiembre 2026, medido contra producción). 🔴 **Es un RÓTULO: ningún número ni cálculo se movió** —las partes siguen sumando la fila, $11,674.57 + $375.30 = $12,049.87—, el canal se sigue decidiendo por CÓDIGO y sin el nombre cae a «tienda» (falla ABIERTA). La CLAVE de `CANALES` sigue siendo el valor de la base; lo que cambió es su rótulo. Candado `multifashion/vendedoras-linea-redes.test.tsx`.
- 🔴 **El bono usa el total junto y no se tocó**: `multifashion_bonos_v4` ya agrupa por `vendedor_canonico`; esta migración no lo redefine.
- 🔴 **Metas usa el canónico.** 🩸 `multifashion_meta_ventas_v1` devolvía el nombre **CRUDO** y quien juntaba era `claveVendedora` por NOMBRE normalizado: junta «ANA TREJOS» con «Ana Trejos» de casualidad, pero jamás «REDES Sheynee» con «Sheynee Batista». En la meta grupal viva («Viaje playa», 1-sep → 31-dic-2026, $420.000, 4 participantes) lo de redes habría caído en *«ventas de alguien que no está en esta lista»*. Ahora `multifashion_meta_ventas_v2` devuelve `vendedor_canonico`; la lectura pide la v2 y **cae a la v1 solo si no existe** (`esFuncionAusente`; un timeout se lanza). El camino paginado también lee `vendedor_canonico`. `claveVendedora` se queda: es la clave con la que se GUARDARON los participantes.
- 🔴 **La base de la comisión de Multifashion es `subtotal_comision`: SOLO lo de CONTADO, notas de crédito restando** (`20260603000000_multifashion_comision_solo_contado.sql`), no «toda la venta» como decía `CLAUDE.md`. Medido en Sheynee, sep-2026: contado $9.225,47 · crédito $131,64 → base 0 · NC −$239,38 → comisión **$44,93** (sobre toda la venta daría $45,59).
- **Medido contra producción antes y después (solo lectura, 18-sep-2026), sin correr la migración:** sep-2026 → Sheynee $9.117,73 / $44,93 · Jailine $5.651,06 / $26,83 · Milagros $5.561,03 / $27,32 · Jennifer $5.508,22 / $26,08 (4 vendedoras, total $25.838,04, corte 18-sep); bono de agosto → Sheynee (gerente Jennifer $100); 12 meses a sep → 10 filas. **Cero ventas con el 15 todavía**, así que **nada se mueve** hasta que las haya; el candado exige que la v5 sea la v4 más el desglose y nada más.
...
- 🔴 **«REDES Sheynee» (15) ES Sheynee (11)**: columna `canal` del MISMO amarre, NUNCA por nombre; UNA fila (comisión y bono juntos) con «Sheynee $X · Redes $Y» (`canales.ts`, v5); Metas lee el canónico (`meta_ventas_v2`). Migración `20261209120000` **aplicada** (verificado contra producción el 24-sep-2026: la fila del código 15 con canal `redes` existe desde el 19-sep-2026).
