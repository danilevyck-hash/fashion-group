...
4. 🔶 **MEDIO CERRADO el 5-sep-2026.** ✅ *Reclamos* ya va por **(empresa, código)** — `reclamos.proveedor_codigo` lleno en las **34 filas**, migración `20260922120000` aplicada, candado `reclamos-proveedor-por-codigo.test.ts`. ✅ **CERRADO el 6-sep-2026 también en Proveedores**: la identidad sale de `proveedor_amarre` (lista escrita a mano, grano `(empresa_key, proveedor_switch_id)`, migración `20261010120000` **pendiente**) y la resuelve UNA función, `aplicarAmarre`. Confecciones Boston pasó de **3 filas a UNA** ($4.165,96, 5 empresas) y la lista de 47 a 43, con el total intacto en $5.199.705,82. 🔴 **El RUC sigue sin usarse, y a propósito**: tres pares que lo comparten son empresas distintas (Daniel: *«fashion wear no es boston»*) y el proveedor más grande tiene DOS RUC por un guion. ⚠️ Los **7 «GENERAL»** siguen fundidos en una fila (no es un proveedor: es el comodín de Switch) — se dejó como estaba. → [08](08-amarres.md) · [04](04-operacion.md)
...
- ✅ **RESUELTO el 10-sep-2026 — Packing Lists se retiró.** Daniel: *«packing list no se usa, eliminar»*. Era esto: 0 filas desde el 22-abr, un cron limpiándolo a diario, y un texto que prometía 7 días cuando la regla eran 90. Se fueron la ficha, la pantalla, las 3 rutas, el lector de PDF y el cron; **las tablas quedan** (0 filas). Ver [superado.md](../historico/superado.md).
