...
- 🔴 **Comisiones — CLIENTES QUE NO COMISIONAN para un vendedor + pestaña Configuración** (3-sep, tarde). Daniel: *«crea configuración en comisiones para desactivar cálculos de clientes»*, grano *«cliente vendedor»*, *«también venta»*; *«no lo llames así [exclusiones] y ponlo en Configuración»*; *«¿por qué en card y no como tab en toda la pantalla normal?»*; *«pon a Reinaldo 1 y 1»*. Código listo y sin commitear; **migración `20260912120000_comision_exclusion_v7.sql` pendiente de que Daniel la aplique** (tabla `comision_exclusion` + 17 filas de Daniel + `comision_b2b_v7` + detalle v4 + Reinaldo 1 %/1 %, que ya estaba así). Hasta entonces la pantalla cae sola a la v6 y lo dice (`exclusiones_aplicadas: false`), y la pestaña Configuración muestra «Falta correr la migración». Medido ene–sep 2026 con el SQL real: **solo Reinaldo se mueve**, Active Shoes −447,67 · Active Wear −151,19 · grupo −598,86; nadie sube. 29 mutaciones, 29 cazadas. Detalle en `docs/postmortems/ventas-referencia.md`. ⚠️ Sin tabla de alias para REINALDO/REYNALDO (una fila por grafía) — si Daniel prefiere una sola fila por cliente, es una decisión pendiente.
- 🔴 **Comisiones — el cobro se paga a QUIEN REGISTRÓ el recibo, y DEFAULT y Daniel no se pagan** (3-sep). Código listo y sin commitear; **migración `20260911120000_comision_b2b_v6_cobro_quien_registro.sql` pendiente de que Daniel la aplique** — hasta entonces la pantalla cae sola a la v5 (cobro por cartera) y lo dice en la respuesta (`regla_cobro: "cartera"`). Medido ene–ago 2026 con el SQL real: grupo +1.253,58 · Reinaldo +2.507,14 · Daniel +1.943,86 · Edwin −2.640,50; lo que de verdad se paga (sin DEFAULT ni Daniel) **48.491,64 → 48.064,15**. Detalle en `docs/postmortems/ventas-referencia.md`.
