...
## 11-sep-2026 (noche) — Auditoría grupo 3: trece defectos arreglados sin rediseñar (Guías · Reclamos · Marketing · Caja Menuda)

Encargo de Daniel sobre `grupo-3.md`. **Arreglar, no rediseñar.** Trece puntos, aprobados uno por uno; dos de ellos son decisiones nuevas suyas (el orden de Reclamos y las dos listas de «Eliminados»).

| # | Dónde | Antes | Ahora |
|---|---|---|---|
| 1 | **Guías** — ⌘K «guías pendientes» | `/guias?pendientes=1` dejaba la lista en **1 de 229** sin chip ni botón, y recargar lo volvía a aplicar: la única salida era escribir `/guias` a mano | chip **«Solo pendientes ×»** que lo quita y limpia la dirección (`replace`). El filtro y el enlace se quedan |
| 2 | **Guías** — aviso «N guías sin despachar» | marcaba `expandedId` de una fila que podía no estar dibujada: con algo en el buscador no pasaba **nada**, y una pendiente de +30 días quedaba detrás de «Ver guías más viejas» | se abre el camino primero (limpia el buscador · abre las viejas) y recién ahí expande; si la guía no está en la lista cargada, **va a su página**. La decisión vive en `lib/guias/llegar-a-la-guia.ts` |
| 3 | **Guías** — `/guias/nueva` | el vendedor entraba, armaba la guía entera y el POST le decía «Sin permiso» al final (el «＋ Agregar destino» ya le había contestado 403) | **guard SSR** con la MISMA lista del POST. La lista estaba escrita **seis veces** y ahora vive en `lib/guias/roles-escritura.ts` |
| 4 | **Guías** — guía despachada | «Guardar Cambios» gris con el renglón de explicación **VACÍO** (`textoFalta([]) === ""`) | «Ya se despachó: solo se corrigen N° del transportista, cliente y facturas», **derivado** de `campos-editables.ts` |
| 5 | **Reclamos** — enlace directo o F5 | el SSR tenía su propio `select`: sin `reclamo_settlements` y sin firmar un solo archivo → fotos rotas, sin «Factura del proveedor», sin comprobante y **«Recuperación 0%»**. Entrando por la lista se veía bien | las dos puertas leen por `lib/reclamos/leer-detalle.ts`. Falla **abierta**: si el detalle se cae, la lista igual se dibuja |
| 6 | **Reclamos** — pestaña «Cobrados» | cada fila seguía ofreciendo «Correo» = cobrarle **dos veces** al proveedor (medido: $5.306,62 en 5 reclamos). El lote y el detalle sí lo frenaban | se retira de las filas cobradas **y el servidor lo rechaza** con su motivo |
| 7 | **Reclamos** — el orden | la factura más **vieja** primero, fijo | 🔴 decisión nueva de Daniel: *«reclamo debe ir sort el más nuevo arriba para verlo, pero con opción de sort en todas las columnas: más plata, más días, menos días, menos plata»*. Abre con la factura más **reciente** arriba; las 5 columnas ordenan tocando el encabezado, con flecha y orden en la URL |
| 8 | **Reclamos** — «Nuevo reclamo» | ofrecía **Joystep**, que no tiene tarjeta: ese reclamo sumaba arriba y no había dónde abrirlo | el formulario y la edición leen `EMPRESAS_CON_RECLAMOS`; un reclamo que YA esté en Joystep conserva su opción |
| 9 | **Reclamos** — «Fecha de factura \*» | marcada obligatoria en las dos pantallas y **nadie la validaba**: de ella salen los días y el orden entero | se exige en pantalla y en el servidor, con la MISMA frase de la lista |
| 10 | **Marketing** — proyecto eliminado | no se recuperaba desde ninguna pantalla: la única puerta a `papelera/restaurar` era el «Deshacer» en `useState`, y bastaba un F5 | 🔴 Daniel: *«a) una lista "Eliminados" con "Restaurar", como en Guías»*. Al pie de la lista de proyectos, plegada; sin ninguno **no se dibuja** |
| 11 | **Marketing** — «Otro gasto» | prometía leer el PDF con IA y nunca lo corría (sin proyecto, la ruta que firma la subida cortaba): spinner que se enciende, se apaga, y los seis campos vacíos — sin éxito ni error | el PDF se sube **una vez** a `sin-dueno/` (`paraLeerConIA`), la IA lo lee, y al guardar se cuelga de la factura con ese mismo `path` |
| 12 | **Caja** — período cerrado | el «···» que abre la foto del recibo **no se dibujaba**: el único archivo de comprobantes quedaba inalcanzable (hoy 2 de 3 períodos están cerrados) | el menú existe siempre; cerrado lleva **una sola cosa**, «Foto del recibo», y la zona abre en solo lectura |
| 13 | **Caja** — aviso de eliminar | prometía «Podrás restaurarlo desde Gastos eliminados» y esa pantalla era de **solo lectura** desde el 7-sep: la promesa fue falsa cuatro días | 🔴 Daniel: *«a) vuelve Restaurar»*. Botón por fila y su rama en el servidor, **solo con el período abierto** |

**Medido contra producción (solo lectura, 11-sep-2026):** 33 reclamos vivos, **0 sin fecha de factura** (por eso exigirla no bloquea a nadie) y **0 de Joystep** (Fashion Wear 21 · Vistana 6 · Fashion Shoes 5 · Active Shoes 1); **0 proyectos de Marketing anulados**.

**Candados nuevos:** `guias-filtro-y-aviso.test.ts` (22) · `guias-filtro-y-aviso.test.tsx` (8, de conducta) · `reclamos-defectos.test.ts` (23) · `caja-y-marketing-defectos.test.ts` (20) · `caja-periodo-cerrado-fotos.test.tsx` (3, de conducta).
**Cambiaron de dirección o de ancla con nota fechada, ninguno se borró:** `guias-numero-transp-tarde-route` (los roles se comprueban sobre el módulo único) · `guias-papel-y-marcas` (monta `NuevaGuiaClient`) · `reclamos-rediseno` (el orden nuevo y el firmado por una sola puerta) · `reclamos-tax` (la cabecera gana un obligatorio) · `reclamos-papel` (el `conLinks` que ya no existe) · `marketing-pdf-en-la-puerta` («Otro gasto» ahora sí lee, con el control de que el PDF se sube una sola vez) · `caja-columnas-retiradas` («Restaurar» sale de la lista de lo retirado, con el control de que vuelve SOLO en la lista de eliminados).

### ⚠️ Dejado a propósito / pendiente de Daniel
- **El PDF que la IA lee se sube antes de que la factura exista y nadie lo borra si se cancela** (bucket `marketing`, sin cron de limpieza). Ya pasaba en el camino Factura; ahora también en «Otro gasto», bajo `sin-dueno/`. Es un hallazgo aparte y no estaba en el encargo.
