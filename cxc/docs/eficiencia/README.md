...
1. 🔴 **Depurador — el divisor no se valida en la pantalla.** `70` en vez de `0.70` descarga un Excel con precios **100× mal**, y ese Excel se sube a Switch. `validarDivisor` existe pero solo corre al guardar fórmulas por API. 50-60 corridas/mes. Arreglo **chico**. → [03](03-operacion-diaria.md)
2. 🔴 **Un cliente que Switch ya borró se sigue ofreciendo.** `switch_clientes` lo marca ausente; `clientes_master` —el directorio que lee el selector— es un upsert puro y **nunca marca lo que dejó de llegar**. Hoy son 2 (D-30 «City Moda Chorrera» y D-135 «Rey Store (Aguas)»); crece solo. → [01](01-ventas-y-clientes.md)
