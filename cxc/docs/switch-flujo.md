...
### 4. Costo y venta del día (el fantasma)

**Sale de Switch por:** API — `GET /apireporte/totalventas?tipo=03` (`client.ts:1197`, `getReporteMesActual`): el mes en curso por día, sin parámetro de fecha.
**Lo trae:** viaja dentro de `switch-sync tipo=all` (05:30/05:35/05:40/06:30), `syncCostoDiario` (`sync-empresa.ts:1108`; disparo en `switch-sync/route.ts:196`).
...
**Se ve en:** 🔴 **En ninguna pantalla.** No hay una sola lectura en `src/`, y la vista que la usaba (`switch_costo_unificado_vw`) se reescribió el 6-jun-2026 sobre `switch_articulo_diario` (`supabase/migrations/20260606080000_switch_costo_unificado_vw_solo_articulo.sql:2-33`). Se escribe en las 8 empresas todos los días para nadie. Sigue siendo útil como **cuadre** (es lo que la alerta A vigila bajo `costo`, `silencio-de-datos.ts:135`).
**Empresas:** las 8.
**Se rompe si:** Switch manda una cifra imposible (pasó: Boston 14-jul-2026, $1.000.000.049,22 de costo contra $493 de venta — de ahí nació el guard de montos).
**Para consultarlo al momento:** `/apireporte/totalventas?tipo=04` da el año por mes con costo y utilidad — no se usa (`switch-referencia.md` §1.2).
...
**Se ve en:** Multifashion › Productos (`api/multifashion/productos/route.ts:236-249`, paginado a mano) · Ventas › Productos (RPC `switch_top_descripciones_reciente`, `api/ventas/productos/route.ts:121`) · Ventas › Referencia como «Vendí» (`src/lib/ventas/referencia.ts:30,57`) · el margen del Resumen vía `switch_costo_unificado_vw`.
...
| 3 | `switch-referencia.md:7` y #16; `switch-panel.md:8`; `client.ts:574` | El PDF del API «no está versionado en el repo», vive en `~/Downloads`; se cita como `docs/api-switch.pdf` | **Sí está**: `docs/switch/api-documentacion.pdf` (commit `d648f4fd`). La ruta citada es la que no existe | ⚠️ pendiente en los dos docs y en el comentario |
| 4 | `client.ts:401-403`, `sync-log.ts:64`, `cron-telemetry.ts:113, 549, 755`, `types.ts` (nota del 0006), `switch-sync/route.ts:22` | «Switch admite UNA sesión **por empresa**» | Es **por usuario** (PDF p. 6; `client.ts:19-23, 266-280`; medido 3-sep). Coincide en la práctica porque el usuario es `daniel` en 7 de 8 | ⚠️ comentarios viejos; la regla operativa de 15 min sigue siendo correcta |
| 5 | `switch-panel.md` §13 | «Sesión única — NO está documentada» | Documentada en el PDF del API, p. 6 (`switch-referencia.md` Parte 3, #1) | ⚠️ pendiente en `switch-panel.md` |
| 6 | `empresas.ts:281, 287`; `sync-recibos.ts:46` | Recibos = «6 B2B + Multifashion, **excluye Boston**» | `confecciones_boston` tiene `recibos: true` (`empresas.ts:138`): son las 8 | ⚠️ comentarios viejos (el doc ya se corrigió el 31-ago) |
| 7 | `sync-acs-fidelizacion.ts:16-18` | «corre en su propio cron (08:15 UTC), espaciado de multifashion-sync (05:00)» | Corre 11:30 y 16:30; `multifashion-sync` se retiró el 26-jul | ⚠️ comentario viejo |
| 8 | `cron-telemetry.ts:494` | `sync-factura-lineas` «02:30 UTC = 9:30 p.m. Panamá» | `vercel.json:45`: **03:30** (`30 3 * * *`) | ⚠️ comentario viejo |
| 9 | `sync-empresa.ts:297`; `types.ts:163` | «El signo contable lo aplica `switch_ventas_netas_vw`» | Esa vista se **borró** el 26-jul (`20260726210100`); el signo vive en `tipos-comprobante.ts` y en `switch_ventas_unificado_vw` | ⚠️ comentarios viejos |
...
| 11 | `CLAUDE.md` › Dónde vive › costo | `switch_costo_unificado_vw` «vista muerta: cero lecturas en `src/`»; `switch_costo_diario` sin advertencia | La vista **sí** la lee la RPC `ventas_dashboard_summary` (SQL, no TS) y desde el 6-jun se arma sobre `switch_articulo_diario`. La que nadie lee es **`switch_costo_diario`**, que se sigue escribiendo 4×/día en las 8 | ✅ corregido en `CLAUDE.md` |
...
| 14 | skill `traer-reporte-switch` › Paso 2 | «revisar el cuerpo con `esError()`» como si fuera de `web-client.ts` | `esError()` es una copia local de los scripts (`_bajar-acs-*.ts:39, 51`); en `web-client.ts` es `jsonDeSwitch` (`:405-425`) | ⚠️ detalle menor en la skill |
