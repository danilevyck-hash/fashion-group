...
| `GET /apicliente/estadocuenta?clienteId=` (§5.15, p. 22-24) | `estadocuenta.elements[]` (`ccteId, secuencial, numeroFiscal, plazoCredito, total, saldo, tipoComprobante, abrev, tiporecibo, fechaCreacion DD-MM-YYYY, dias, saldoConsecutivo, debito, credito`) **+ `Saldos[]` (aging `0-30`…) + `saldoTotal`** | Sí — `sync-empresa.ts` (`mapEstadoCuenta`) → `switch_estadocuenta`; la llamada es `client.getEstadoCuenta` (`client.ts:1188`, endpoint en `:1193`) | 🔄 **9-sep-2026: `saldoTotal` y `Saldos[]` YA NO SE DESCARTAN.** Hasta ese día decía «🔑 Descartamos `Saldos[]` y `saldoTotal`», y por eso el sistema sumaba los documentos **sin nada con qué comprobarse**: si uno no llegaba, el estado de cuenta salía al cliente con un número que Switch no reconoce y nada lo decía. Ahora caen en `switch_estadocuenta_saldo` (migración `20261023120000`, **pendiente**) — `saldo_total` a columna, `Saldos[]` crudo en jsonb porque su forma no está documentada — y alimentan el **cuadre** que el cajón y la hoja «Cobrar» dicen en pantalla antes de mandar el papel. ⚠️ Los tramos que se MUESTRAN siguen siendo los TRES de la pantalla, no los ocho de Switch (Daniel: *«solo los 3 de mi lista»*). Es **uno por cliente** — por eso Boston (4.912 clientes) va por reporte web (`CLAUDE.md:426`). |
...
11. ⚠️ **Descartamos el aging que Switch calcula** *(vigente hasta el 9-sep-2026; desde el 18-sep se guardan de verdad en `switch_estadocuenta_saldo`, leídos de ADENTRO de `data.estadocuenta` por `estadocuenta-cuadre.ts` — entre el 9 y el 18 se leían del nivel de afuera y quedaban en NULL)*. `/apicliente/estadocuenta` devuelve `Saldos[]` por bucket y `saldoTotal` (p. 23) **anidados en `estadocuenta`**; `switch_estadocuenta_aging` sigue recalculando los tramos que se ven. Para proveedores sí se guardan los buckets de Switch, y ahí se puede ver la forma exacta sin llamar a nadie: `select aging from switch_proveedor_estadocuenta limit 1` devuelve **ocho** tramos — `0-30 · 31-60 · 61-90 · 91-120 · 121-180 · 181-270 · 271-365 · Mas de 365`. Nuestra vista de clientes arma **siete** (`d0_30 … mas_365`), así que el cuadre gratis existe pero no es tramo a tramo: sería contra el `saldoTotal` y contra los tramos que coinciden.
