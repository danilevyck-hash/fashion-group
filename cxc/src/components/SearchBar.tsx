    ...
    return (
      <div className="flex flex-col h-full">
        <div className="flex items-center gap-3 px-4 sm:px-6 py-3 border-b border-gray-200">
          ...
          {query && <button onClick={() => setQuery("")} aria-label="Borrar búsqueda" className="text-gray-400 w-11 h-11 flex items-center justify-center"><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M18 6 6 18M6 6l12 12" /></svg></button>}
    ...
    <div ref={wrapperRef} className={`relative ${compact ? "w-56" : "w-full max-w-xl mx-auto mb-6"}`}>
      <div className={`relative flex items-center rounded-lg border ${darkMode ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-gray-50"} transition focus-within:border-gray-400 focus-within:shadow-sm`}>
        ...
        <input
          ...
          /* 356×37 medidos en /home. `text-base sm:text-sm`: por debajo de 16px
             Safari hace zoom al enfocar y te deja la pantalla corrida. */
          className={`w-full pl-10 pr-10 py-2 min-h-[44px] text-base sm:text-sm rounded-lg outline-none ${inputBg} bg-transparent`}
          autoFocus={compact}
        />
