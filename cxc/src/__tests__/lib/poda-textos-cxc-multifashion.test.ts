...
const SE_FUE_DE_LA_VISTA: { archivo: string; que: string; texto: string }[] = [
  ...
  // 10-sep-2026 · NOTA FECHADA — la fila de Packing Lists (el h1 del detalle,
  // el último de los 27 títulos de la auditoría) se fue con la PANTALLA: el
  // módulo se retiró (Daniel: «packing list no se usa, eliminar»;
  // `packing_lists` con 0 filas). La regla —lo que se poda de la vista sigue
  // teniendo nombre para un lector de pantalla— no cambió.
];
...
// ─────────────────────────────────────────────────────────────────────────────
//
// 10-sep-2026 · NOTA FECHADA — este bloque entero se retiró porque se retiró el
// MÓDULO: Daniel, textual, *«packing list no se usa, eliminar»* (`packing_lists`
// y `pl_items` con 0 filas desde el 14-may-2026). Probaba el h1 de
// `app/packing-lists/[id]/page.tsx`, un archivo que ya no existe.
//
// 🔴 LA REGLA QUE FIJABA NO SE FUE y no se puede perder: el identificador —CUÁL
// cosa estoy mirando— NO es el nombre de la pantalla y nunca se poda a sr-only.
// Es la regla 2 de la poda del #510, la misma que le dejó su título a
// Nueva/Editar guía, y sigue probada en los demás módulos de este archivo.
