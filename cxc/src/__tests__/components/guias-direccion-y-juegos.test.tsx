 ...
 *  · que tocar un juego llene los TRES campos y los deje editables.
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
const JUEGOS: JuegoDespacho[] = [
  { receptor: "Jocsan", cedula: "8918246", placa: "DG7115" },
  { receptor: "Aníbal arauz", cedula: "810102403", placa: "EK0700" },
];
...
describe("🔴 un toque llena los TRES campos", () => {
  ...
  it("los últimos despachos de ese transportista se ofrecen con los tres datos", () => {
    render(<Despacho tipo="externo" juegos={JUEGOS} />);
    expect(screen.getByText(/Últimos despachos con este transportista/i)).toBeTruthy();
    const boton = screen.getByRole("button", { name: /Jocsan/ });
    expect(boton.textContent).toContain("8918246");
    expect(boton.textContent).toContain("DG7115");
  });

  it("tocarlo llena recibido por + cédula + placa de una vez", () => {
    render(<Despacho tipo="externo" juegos={JUEGOS} />);
    fireEvent.click(screen.getByRole("button", { name: /Jocsan/ }));
    expect(val("despacho-receptor")).toBe("Jocsan");
    expect(val("despacho-cedula")).toBe("8918246");
  ...
  it("se puede cambiar de juego: el segundo pisa al primero, entero", () => {
    render(<Despacho tipo="externo" juegos={JUEGOS} />);
    fireEvent.click(screen.getByRole("button", { name: /Jocsan/ }));
    fireEvent.click(screen.getByRole("button", { name: /Aníbal/ }));
    expect(val("despacho-receptor")).toBe("Aníbal arauz");
    expect(val("despacho-cedula")).toBe("810102403");
  ...
  it("⚠️ EN ENTREGA DIRECTA NO APARECE: no hay transportista ni placa", () => {
    ...
    expect(screen.queryByText(/Últimos despachos con este transportista/i)).toBeNull();
    expect(screen.queryByRole("button", { name: /Jocsan/ })).toBeNull();
  });
