// Validación server-side del POST público de pedidos Calvin Klein
// (/api/catalogo/calvin/pedido-publico). Espejo de
// tommy-pedido-publico-validate: NADA del body se confía. Funciones PURAS
// (sin I/O) para testear con vitest.
//
// Igual que Tommy:
//   * Calvin es 100% calzado → el bulto sale del producto (`bulto_pzas`,
//     default 12) y no de la category. applyDbPrices solo reemplaza el precio
//     y el total usa calculateCalvinOrderTotal.
//   * Sin is_preorder (Calvin no tiene preventa — feature flag off).
//
// Dos fases:
...
//      reemplaza por el real de `calvin_products` y el total se calcula
...
import { calculateCalvinOrderTotal } from "./calvin-order-total";
...
// El nombre del cliente lo valida la regla ÚNICA de las marcas
 ...
 *  /pedido-calvin/[id]: foto, nombre, sku, cantidad, precio). Sin category ni
 *  is_preorder (Calvin es todo calzado, sin preventa). */
...
/** Precio real según la tabla `calvin_products` (una fila por product_id). */
 ...
 * el total server-side (bulto por producto, default 12). `adjusted=true` si algún
 * precio del cliente difería del real (para loguearlo). Pura: recibe el Map ya
 * consultado.
 */
export function applyDbPrices(
  ...
  const total = Math.round(calculateCalvinOrderTotal(priced) * 100) / 100;
