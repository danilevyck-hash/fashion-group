// Rate-limit anti-spam del POST público de pedidos Calvin (pedido-publico).
// Espejo de tommy-pedido-rate-limit: cuenta los pedidos YA GUARDADOS en
// calvin_pedidos_publicos por ip_hash en los últimos 10 min. Máximo PEDIDOS_MAX
// por IP por ventana.
//
// Requiere la tabla/columna (migración 20260812150000_calvin_catalogo.sql).
...
export function hashPedidoIp(ip: string): string {
  return createHash("sha256").update(`calvin-pedido:${ip}`).digest("hex").slice(0, 32);
}
