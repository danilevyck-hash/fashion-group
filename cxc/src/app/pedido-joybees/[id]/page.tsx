...
export default function PedidoJoybeesPage() {
  ...
  // Confirmar el pedido: entra directo al proceso (se convierte en JBP-###).
  ...
  async function handleConfirm(aceptar: boolean) {
    ...
    try {
      const res = await fetch(`/api/catalogo/joybees/pedido-publico/${order.short_id}/confirmar`, {
        method: "POST",
  ...
  const totalPedido = calculateJoybeesOrderTotal(order.items);
  const whatsappUrl = confirmado
    ? buildPedidoWhatsappUrl({
        ...
        bultos: totalBultos,
        total: totalPedido,
        link: `https://www.fashiongr.com/pedido-joybees/${order.short_id}`,
      })
    : "";
  ...
  return (
    <div className="min-h-screen bg-[#FFFEF5]">
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {/* Items */}
        <div className="bg-white border-x border-[#1A2656]/10 divide-y divide-gray-100">
          {order.items.map((item, idx) => {
            const bs = getBultoSize();
            const lineTotal = item.quantity * bs * item.unit_price;
            return (
              <div key={idx} className="flex items-center gap-3 px-4 py-3">
                <div className="w-14 h-14 rounded-lg bg-[#FFFEF5] flex-shrink-0 overflow-hidden">
                  {item.image_url ? (
                    <Image
                      src={item.image_url}
                      alt={item.name}
