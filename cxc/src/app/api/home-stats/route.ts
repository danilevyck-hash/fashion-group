...
export async function GET() {
  ...
  const { count: reclamosPendientes } = await supabaseServer.from("reclamos").select("*", { count: "exact", head: true }).neq("estado", "Aplicada");

  const { data: cheques } = await supabaseServer.from("cheques").select("fecha_deposito, monto").eq("estado", "pendiente");
  ...
  const { data: periodoAbierto } = await supabaseServer.from("caja_periodos").select("fondo_inicial, id").eq("estado", "abierto").order("created_at", { ascending: false }).limit(1).maybeSingle();
  ...
  const { count: guiasEsteMes } = await supabaseServer.from("guia_transporte").select("*", { count: "exact", head: true }).gte("created_at", monthStart);
  const { count: totalClientes } = await supabaseServer.from("directorio_clientes").select("*", { count: "exact", head: true });
