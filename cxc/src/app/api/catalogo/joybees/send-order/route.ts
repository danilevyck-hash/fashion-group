...
export async function POST(req: NextRequest) {
  ...
  type EmailItem = { sku: string; name: string; quantity: number; unit_price: number; image_url: string };
  ...
  if (body.orderId) {
    const { data: order, error } = await joybeesServer
      .from("joybees_orders")
      .select(
        "client_name, order_number, comment, created_at, joybees_order_items(product_id, sku, name, quantity, unit_price, image_url)",
      )
      .eq("id", body.orderId)
      .single();
    ...
    const rawItems = (order.joybees_order_items || []) as { sku: string; name: string; quantity: number; unit_price: number; image_url?: string }[];
    items = rawItems.map((i) => ({
      sku: i.sku || "", name: i.name || "", quantity: i.quantity, unit_price: i.unit_price,
      image_url: i.image_url || "",
    }));
    totalBultos = items.reduce((s, i) => s + i.quantity, 0);
    totalPiezas = items.reduce((s, i) => s + i.quantity * getBultoSize(), 0);
    total = items.reduce((s, i) => s + i.quantity * getBultoSize() * Number(i.unit_price), 0);
  } else {
  ...
  // Keyed by image_url so we can look them up by item regardless of order.
  ...
  await Promise.all(items.map(async (item) => {
    ...
    try {
      ...
      imgs[item.image_url] = `data:${contentType};base64,${b64}`;
  ...
  // Marca Joybees: navy #1A2656 con acento amarillo #FFE443.
  ...
  autoTable(doc, {
    ...
    headStyles: { fillColor: [26, 38, 86], textColor: [255, 255, 255] },
    ...
    didDrawCell: (data: { row: { index: number; section: string }; column: { index: number }; cell: { x: number; y: number; height: number; width: number } }) => {
      if (data.column.index === 0 && data.row.section === "body") {
        ...
        if (b64) {
          ...
          try { doc.addImage(b64, "JPEG", xOffset, yOffset, imgSize, imgSize); } catch { /* skip */ }
        }
      }
    },
  });
  ...
  doc.text("Fashion Group Panama · Joybees", 14, fy + 10);
    ...
    <div style="font-family:Arial,sans-serif;max-width:650px;margin:0 auto">
      ...
      <div style="padding:20px;border:1px solid #eee;border-top:none;border-radius:0 0 8px 8px">
        <p style="color:#333;font-size:14px;line-height:1.5;margin:0 0 16px">
          ...
          Se ha recibido un nuevo pedido del catalogo Joybees. A continuacion el detalle:
        </p>
        ${comment ? `<p style="color:#666;font-size:13px;margin:0 0 12px"><strong>Nota:</strong> ${comment}</p>` : ""}
        <table style="width:100%;border-collapse:collapse;margin:8px 0 16px">
          <thead><tr style="background:#1a2656;color:white">
