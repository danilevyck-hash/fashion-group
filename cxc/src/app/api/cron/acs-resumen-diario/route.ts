...
import { calcularResumenDiario, buildMensaje, hoyPanama } from "@/lib/acs-resumen-diario";
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const resumen = await calcularResumenDiario(fecha);
    ...
    return NextResponse.json({ ok: true, fecha, mensaje, resumen });
  } catch (err) {
