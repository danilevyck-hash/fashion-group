...
// tienda_marca_formulas tiene RLS `to service_role`: sin la service role key, el rol anon
// la lee VACÍA sin error (fallo silencioso). Fallamos ruidosamente para que se vea.
const MISCONFIG = NextResponse.json(
  { error: "Falta SUPABASE_SERVICE_ROLE_KEY en este entorno: las fórmulas de tienda no se pueden leer ni guardar (RLS las bloquea para el rol anon)." },
  { status: 503 }
);

const COLS = "id, marca, empresa, divisor, extra, redondeo, updated_at, updated_by";

/** Lista todas las fórmulas de tienda guardadas, ordenadas por empresa y marca. */
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabaseServer
    .from("tienda_marca_formulas")
    .select(COLS)
    .order("empresa", { ascending: true, nullsFirst: true })
    .order("marca", { ascending: true });

  if (error) {
    return NextResponse.json({ error: "No se pudieron cargar las fórmulas de tienda." }, { status: 500 });
  }
  return NextResponse.json({ rows: data ?? [] });
}

/** Upsert de una fórmula de tienda por marca (case-insensitive). */
export async function PUT(req: NextRequest) {
  ...
  if (findErr) {
    return NextResponse.json({ error: "No se pudo guardar la fórmula de tienda." }, { status: 500 });
  }

  if (existing?.id) {
    const { error } = await supabaseServer.from("tienda_marca_formulas").update(fields).eq("id", existing.id);
    if (error) return NextResponse.json({ error: "No se pudo guardar la fórmula de tienda." }, { status: 500 });
  } else {
    const { error } = await supabaseServer.from("tienda_marca_formulas").insert(fields);
    if (error) return NextResponse.json({ error: "No se pudo guardar la fórmula de tienda." }, { status: 500 });
  }
