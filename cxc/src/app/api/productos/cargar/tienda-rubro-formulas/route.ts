...
const MISCONFIG = NextResponse.json(
  { error: "Falta SUPABASE_SERVICE_ROLE_KEY en este entorno: las excepciones de tienda no se pueden leer ni guardar." },
  { status: 503 }
);

/** Lista todas las excepciones de tienda marca+rubro. */
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabaseServer
    .from("tienda_rubro_formulas")
    .select(COLS)
    .order("marca", { ascending: true })
    .order("rubro", { ascending: true });

  if (error) {
    return NextResponse.json({ error: "No se pudieron cargar las excepciones de tienda." }, { status: 500 });
  }
  return NextResponse.json({ rows: data ?? [] });
}

/** Upsert de una excepción de tienda por marca+rubro (case-insensitive). */
export async function PUT(req: NextRequest) {
  ...
  if (findErr) {
    return NextResponse.json({ error: "No se pudo guardar la excepción de tienda." }, { status: 500 });
  }

  if (existing?.id) {
    const { error } = await supabaseServer.from("tienda_rubro_formulas").update(fields).eq("id", existing.id);
    if (error) return NextResponse.json({ error: "No se pudo guardar la excepción de tienda." }, { status: 500 });
  } else {
    const { error } = await supabaseServer.from("tienda_rubro_formulas").insert(fields);
    if (error) return NextResponse.json({ error: "No se pudo guardar la excepción de tienda." }, { status: 500 });
  }
...
/** Borra una excepción de tienda por id. */
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await supabaseServer.from("tienda_rubro_formulas").delete().eq("id", id);
  if (error) {
    return NextResponse.json({ error: "No se pudo borrar la excepción de tienda." }, { status: 500 });
  }
  return NextResponse.json({ ok: true });
}
