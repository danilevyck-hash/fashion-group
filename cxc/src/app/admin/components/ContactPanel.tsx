      ...
      <div className="flex flex-col sm:flex-row gap-6">
        ...
        {!editing && (
          <div className="flex sm:flex-col gap-2">
            <button
              onClick={(e) => { e.stopPropagation(); setWaSending(true); onOpenWhatsApp(client); setTimeout(() => setWaSending(false), 3000); }}
            ...
            >
              ...
              {waSending ? "Enviado" : "WhatsApp"}
            </button>
            {waSending && (
              <button
                onClick={(e) => { e.stopPropagation(); onMarkContacted(client.nombre_normalized, "whatsapp"); setWaSending(false); }}
                className="flex items-center gap-2 text-[11px] border border-emerald-300 text-emerald-700 bg-emerald-50 px-2.5 py-1.5 rounded-lg hover:bg-emerald-100 transition animate-pulse"
              >
                Contactaste?
              </button>
            )}
