...
export default function ContactPanel({
  ...
  return (
    <div className="bg-gray-50/80 px-6 py-4 border-b border-gray-200 space-y-3">
      ...
      <div>
        ...
        {lastContact ? (
          <div className="flex flex-wrap items-center gap-3">
            <div className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-md ${daysSince !== null && daysSince <= 7 ? "bg-emerald-50 text-emerald-700" : daysSince !== null && daysSince <= 30 ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"}`}>
              Hace {daysSince} dias via {lastContact.method}
            </div>
            {client.resultado_contacto && (
              <span className="text-xs text-gray-600">Resultado: {client.resultado_contacto}</span>
            )}
            {client.proximo_seguimiento && (() => {
              ...
              const colorClass = isOverdue ? "text-red-600 bg-red-50" : isToday ? "text-amber-600 bg-amber-50" : "text-gray-600 bg-gray-100";
              const label = isOverdue ? "Seguimiento vencido" : isToday ? "Seguimiento hoy" : "Prox. seguimiento";
              return (
                <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-md ${colorClass}`}>
                  ...
                  {label}: {new Date(client.proximo_seguimiento + "T00:00:00").toLocaleDateString("es-PA", { day: "numeric", month: "short", year: "numeric" }).replace(".", "")}
                </span>
              );
            })()}
          </div>
        ) : (
          <div className="text-xs text-gray-400 italic">Sin contacto registrado</div>
        )}
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 text-xs border border-gray-200 text-gray-600 px-3 py-2 rounded-lg hover:bg-gray-100 transition font-medium min-h-[36px]"
          >
      ...
      {/* ── Section 3: Contacto (info + edicion) ──────────── */}
      <div>
