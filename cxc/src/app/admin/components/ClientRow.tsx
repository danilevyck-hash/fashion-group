...
export default function ClientRow({ client, isExpanded, onToggle, userRole, contactLog, selectionMode, isSelected, onQuickWA, onQuickEmail, onRegisterContact, isFavorite, onToggleFavorite }: Props) {
  ...
  const followUpBadge = followUpOverdue
    ...
    : followUpSoon
    ? { label: `${Math.ceil((new Date(client.proximo_seguimiento + "T00:00:00").getTime() - new Date(today + "T00:00:00").getTime()) / 86400000)}d`, bg: "bg-amber-50", text: "text-amber-600" }
    : null;
          ...
          <>
            ...
            <div className={`${selectionMode ? "col-span-3" : "col-span-4"} font-medium truncate flex items-center gap-1.5`}>
              ...
              {!selectionMode && followUpBadge && (
                <span className={`inline-flex items-center gap-0.5 text-[10px] px-1.5 py-0.5 rounded-full ${followUpBadge.bg} ${followUpBadge.text} flex-shrink-0 font-medium`} title={`Seguimiento: ${new Date(client.proximo_seguimiento + "T00:00:00").toLocaleDateString("es-PA", { day: "2-digit", month: "short" })}`}>
