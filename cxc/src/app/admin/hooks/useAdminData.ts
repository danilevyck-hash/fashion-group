...
export default function useAdminData() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Último pago (recibo más reciente) por empresa+cliente, de la vista
      // read-only switch_ultimo_pago_cliente. Join al aging por (company_key,
      // codigo). Cobertura parcial: clientes sin recibo no tienen entrada.
      const { data: pagos } = await supabase.from("switch_ultimo_pago_cliente").select("*");
      const pagoMap = new Map<string, { fecha: string; monto: number }>();
      if (pagos) {
        for (const p of pagos) {
          if (!p.cliente_codigo) continue; // join de la UI es por codigo
