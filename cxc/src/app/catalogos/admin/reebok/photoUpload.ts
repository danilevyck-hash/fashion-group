// Subida de foto de producto Reebok con compresión cliente. Reusa el endpoint
// existente /api/catalogo/reebok/upload (file + sku → path determinístico) y luego
// PUT products.image_url. Manejo de error robusto: valida, comprime para no exceder
// el límite de body de Vercel (~4.5MB → fotos de celular daban 413), timeout, y
 ...
 */
export async function uploadProductPhoto(productId: string, sku: string, file: File): Promise<string> {
  ...
  try {
    ...
    const upRes = await fetch("/api/catalogo/reebok/upload", { method: "POST", body: fd, signal: controller.signal });
    if (!upRes.ok) {
      const body = await upRes.json().catch(() => null);
      throw new Error(body?.error || `No se pudo subir la foto (error ${upRes.status}).`);
    }
    const { url } = await upRes.json();
    const putRes = await fetch("/api/catalogo/reebok/products", {
      method: "PUT",
