  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <button onClick={() => router.push("/admin")} className="w-full text-left border border-gray-100 rounded-2xl p-5 mb-4 hover:border-gray-300 transition">
          ...
          {cxc.lastUpload && (
            <div className="mt-2 flex items-center gap-2 flex-wrap">
              ...
              <span className="text-[10px] text-gray-500 font-medium">{new Date(cxc.lastUpload).toLocaleDateString("es-PA")}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {/* Guías */}
        <button onClick={() => router.push("/guias")} className="relative text-left border border-gray-100 rounded-2xl p-5 hover:border-gray-300 hover:shadow-sm transition bg-white group w-full">
          <div className="bg-gray-50 rounded-xl p-2.5 w-10 h-10 flex items-center justify-center mb-3 text-gray-600"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M1 3h15v13H1z"/><path d="M16 8h4l3 3v5h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg></div>
          ...
          {stats && <p className="text-[11px] text-gray-400 mt-2">{stats.guiasEsteMes} guías este mes</p>}
          <span className="absolute bottom-5 right-5 text-gray-300 group-hover:text-gray-500 transition text-base">→</span>
        </button>

        {/* Carga */}
        <button onClick={() => router.push("/upload")} className="relative text-left border border-gray-100 rounded-2xl p-5 hover:border-gray-300 hover:shadow-sm transition bg-white group w-full">
        ...
        {/* Caja */}
        <button onClick={() => router.push("/caja")} className="relative text-left border border-gray-100 rounded-2xl p-5 hover:border-gray-300 hover:shadow-sm transition bg-white group w-full">
        ...
        {/* Reclamos */}
        <button onClick={() => router.push("/reclamos")} className="relative text-left border border-gray-100 rounded-2xl p-5 hover:border-gray-300 hover:shadow-sm transition bg-white group w-full">
        ...
        {/* Cheques */}
        <button onClick={() => router.push("/cheques")} className="relative text-left border border-gray-100 rounded-2xl p-5 hover:border-gray-300 hover:shadow-sm transition bg-white group w-full">
          <div className="bg-gray-50 rounded-xl p-2.5 w-10 h-10 flex items-center justify-center mb-3 text-gray-600"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 12l2 2 4-4"/></svg></div>
        ...
        {/* Directorio */}
        <button onClick={() => router.push("/directorio")} className="relative text-left border border-gray-100 rounded-2xl p-5 hover:border-gray-300 hover:shadow-sm transition bg-white group w-full">
