...
export interface DirectorioInitialData {
  clientes: Cliente[];
  total: number;
  empresas: string[];
  cxcNombres: string[];
...
export default function DirectorioClient({ initialData }: { initialData: DirectorioInitialData }) {
  ...
  // Load on search/page/empresa change. Skipear el primer mount cuando ya
  // tenemos initialData del SSR (estado por defecto: search="", page=1, empresa="")
  // que matchea la query del server. Refetch solo cuando el usuario interactúa.
  const initialLoadRef = useRef(true);
  useEffect(() => {
    if (!authChecked) return;
    if (initialLoadRef.current) {
      initialLoadRef.current = false;
      return; // initialData ya pobló clientes/total
    }
