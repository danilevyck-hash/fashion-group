...
export default function PedidosPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const r = await fetch("/api/catalogo/joybees/orders"); if (r.ok) setOrders(await r.json()); } catch { /* */ }
    setLoading(false);
  }, []);
  ...
  async function deleteOrder() {
    ...
    setDeleting(true);
    await fetch(`/api/catalogo/joybees/orders/${deleteTarget.id}`, { method: "DELETE" });
    setDeleting(false);
  ...
  async function duplicateOrder(order: Order) {
    ...
    try {
      const res = await fetch(`/api/catalogo/joybees/orders/${order.id}`);
      if (!res.ok) { toast("Error al cargar pedido", "error"); return; }
      const full = await res.json();
      const items = (full.joybees_order_items || []).map((i: { product_id: string; sku: string; name: string; image_url: string; quantity: number; unit_price: number }) => ({
        product_id: i.product_id, sku: i.sku, name: i.name, image_url: i.image_url, quantity: i.quantity, unit_price: i.unit_price,
      }));
      const createRes = await fetch("/api/catalogo/joybees/orders", {
      ...
      if (createRes.ok) {
        ...
        router.push(`/catalogo/joybees/pedido/${newOrder.id}`);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <Link href="/catalogo/joybees" className="text-xs text-gray-400 hover:text-gray-600 transition">← Catálogo</Link>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filtered.map(o => (
            <div key={o.id} onClick={() => router.push(`/catalogo/joybees/pedido/${o.id}`)}
