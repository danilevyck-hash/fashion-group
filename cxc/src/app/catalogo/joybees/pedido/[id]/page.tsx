...
// Joybees es footwear puro: orden simple por SKU ascendente.
...
export default function OrderDetailPage() {
  ...
  // Refs con los ultimos valores para que el guardado nunca mande datos viejos
  // por culpa de closures de un render anterior.
  ...
  async function confirmOrder() {
    ...
    try {
      const confirmRes = await fetch(`/api/catalogo/joybees/orders/${id}`, {
        method: "PUT", headers: { "Content-Type": "application/json" },
    ...
    sessionStorage.removeItem("joybees_draft_id");

    // 3. Try to send email (non-blocking — order is already confirmed)
    try {
      const emailRes = await fetch("/api/catalogo/joybees/send-order", {
  ...
  async function deleteOrder() {
    ...
    try {
      const res = await fetch(`/api/catalogo/joybees/orders/${id}`, { method: "DELETE" });
    ...
    router.push("/catalogo/joybees/pedidos");
  }
  ...
  async function sendToClient() {
    ...
    try {
      const res = await fetch("/api/catalogo/joybees/send-order", {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const totalPiezas = items.reduce((s, i) => s + (i.quantity || 0) * BULTO, 0);
  const totalMoney = items.reduce((s, i) => s + (i.quantity || 0) * BULTO * Number(i.unit_price || 0), 0);

  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      {/* Back button */}
      <button onClick={() => router.push("/catalogo/joybees/pedidos")} className="text-sm text-gray-400 hover:text-black transition mb-4 inline-block">
        ← Volver a Pedidos
      </button>
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          {/* Add more products */}
          {canEdit && (
            <Link href="/catalogo/joybees" className="text-xs bg-gray-100 text-gray-600 px-3 py-1.5 rounded-full hover:bg-gray-200 transition">
              + Agregar productos
            </Link>
          )}
        </div>
      ...
      {items.length > 0 ? (
            ...
            <tbody>
              {items.map((item, idx) => (
                <tr key={idx} className="border-b border-gray-50">
                  ...
                  <td className="py-2 text-center text-xs text-gray-400 tabular-nums">{item.quantity * BULTO}</td>
                  ...
                  <td className="py-2 text-right tabular-nums text-sm">${fmt(item.quantity * BULTO * Number(item.unit_price))}</td>
                  {canEdit && (
                    <td className="py-2 text-center">
        ...
        <div className="py-12 text-center text-sm text-gray-400 border border-dashed border-gray-200 rounded-lg mb-4">
          No hay productos. <Link href="/catalogo/joybees" className="text-black underline">Agregar desde el catalogo</Link>
        </div>
      )}
