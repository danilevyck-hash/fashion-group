      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        <div className="flex items-end justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            {allSelectableIds.length > 0 && <button onClick={() => sendBulkWA(allSelectableIds)} className="text-sm text-gray-400 hover:text-black transition" title="Enviar todos los reclamos pendientes por WhatsApp">WhatsApp todo</button>}
        ...
        {sortedRecs.length === 0 ? <p className="text-center text-gray-300 text-sm py-20">Sin reclamos</p> : (
          <table className="w-full text-sm">
            ...
            <tbody>{sortedRecs.map((r) => { const days = daysSince(r.fecha_reclamo); const total = calcSub(r.reclamo_items ?? []) * 1.177; const isOpen = r.estado !== "Aplicada"; return (
              <tr key={r.id} onClick={() => loadDetail(r.id)} className="border-b border-gray-100 hover:bg-gray-50/80 transition cursor-pointer">
