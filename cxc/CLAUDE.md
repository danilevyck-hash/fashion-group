...
- 🔴 **Destinos definidos en `guias_destino_cliente`**, administrados en Guías › Configuración (**admin y secretaria**; bodega y vendedor 403). **Precedencia en UNA función** (`destinosDefinidosPara`): **tabla → `DESTINOS_DEFINIDOS` (red mientras `20260918120000` no corra) → histórico**. Soft delete **firmado, NUNCA DELETE**; única entre activas; RLS service_role; **una sola** `el_de_siempre` por cliente. No toca `guia_items`.
...
- 🔴 **Bodega corrige los BULTOS al despachar, solo con la guía PENDIENTE**; **firmada, no se tocan** (por `previous.estado`). **UNA columna de UNA línea** (`items_bultos`), **nunca por `items`** del PUT; rastro en `guia_items.bultos_original/_corregido_por/_corregido_en` (`20261004120000`, **pendiente**). En papel, PDF y Excel sale el número **FINAL**.
...
- 🔴 **LOS OCHO NÚMEROS DEL HUB LOS SUMA LA BASE, Y LA REGLA SIGUE SIENDO UNA (14-sep-2026).** Daniel: *«5. ok va»*. 🩸 `/catalogos/marcas` bajaba el **catálogo entero de las 4 marcas + el `inventory` de Reebok** —**462,8 KB** medidos, **24.384 ms de p95**— para escribir «N a la venta · N sin foto». Ahora es **UNA** petición a `/api/catalogo/contadores`: **181 bytes**. 🔴 **No hay dos definiciones de «a la venta»**: el SQL **se GENERA** desde `catalogo/contadores.ts`, cláusula por cláusula de `estaALaVenta`, y la migración `20261123120000` (⚠️ **pendiente de aplicar**) es su **salida impresa**, comparada **byte a byte** por el candado. 🔴 **Falla ABIERTA**: sin la migración se cuenta leyendo filas con `productosALaVenta` y el hub se ve igual. ⚠️ Ninguna ruta vieja cambió. **Medido antes y después**: Reebok 178·0 · Joybees 81·0 · Tommy 453·0 · Calvin 82·0, idénticos. Candado: `catalogo-contadores-una-regla.test.ts`; **13 mutaciones, 13 cazadas** (`scripts/_mutar-candados-contadores-hub.sh`). Detalle en el postmortem.
- Candados: `catalogo-publico-como-el-catalogo` · `catalogo-publico-revisar`.
...
- 🔴 **REEBOK ENTRA POR DOS ARCHIVOS, Y LA PANTALLA DICE CUÁL SE SUBIÓ (17-sep-2026):** la **confirmación de compra** (lo que va a llegar, para cotizar — **no cambió**) y el **despacho** (lo que llegó), que arregla tres números que se inventaban: el **costo se LEE** (`Precio after Disc`; el archivo real trae 20 %, 25 % **y 30 %** — medido: +$1.379,51 en ropa y −$329,94 en calzado contra el descuento asumido), el **código de barra sale del `EAN` y, sin él, del `UPC`** (era el SKU de Reebok; el orden está medido: `EAN` ≠ `UPC` en las 1.579 filas y **94 de los 183 artículos ya cargados en Switch son EAN-13**, 66 con el prefijo `120` del despacho. ⚠️ En el despacho VIEJO de calzado el `SKU` ES el EAN, así que ese defecto era solo de ropa. 🔴 **Pendiente: pedirle a Reebok el `EAN` de vuelta** — el formato nuevo lo perdió; la `Composición` que también perdió Daniel no la quiere) y la **cantidad es `Quantity`** (era la proyección del mes). 🔴 **El MISMO archivo con o sin las columnas nuevas**: la regla de respaldo es un DATO (`COLUMNAS_DESPACHO`, `depurador/reebok-despacho.ts`) —`PO NAME` → `BP Reference No.` → `Orden`; `Category` → `SHOES` solo en calzado; `Department` derivado de `Segmento de negocio` por **palabra entera, nunca `includes`**— y **ninguna columna que hoy falta es obligatoria**. El despacho produce los MISMOS `ReebokItem`: no hay un segundo generador de las 25 columnas ni una segunda cuenta de costo. Candado `reebok-despacho.test.ts` (**16 mutaciones, 16 cazadas**). Detalle y las tres decisiones abiertas de Daniel, en el postmortem.
...
- 🔴 **EL DÍA LIBRE DE LA EMPRESA: SE PAGA COMPLETO Y QUEDA DEBIENDO 8 HORAS EN DÓLARES, QUE SOLO PAGAN LAS HORAS EXTRA (17-sep-2026).** Daniel, textual: *«en las fiestas judías hay días libres… que son libres para el colaborador, pero se pagan con el tiempo de horas extra»* · *«se le paga ese día pero deben las horas laborales (8 horas para todos)»* · *«debe de ser el dólares pienso, porque no todas las horas valen igual»* · *«queda debiendo para la próxima quincena hasta cancelar la deuda»* · *«arrastra para siempre hasta que haga horas extra»* · *«[se le descuenta de la liquidación] no»*. 🔑 **No se inventó: la contadora lo lleva a mano desde mayo** («DESC. POR FIESTA JUDÍA (22 DE MAYO) 18.50 … HORAS PENDIENTES A DESCONTAR 12.8375»), con dos saldos vivos medidos el 17-sep (Kenny Vargas 28: $53,69 · Samir Polo 42: $12,84, a cargar cuando Daniel confirme). Séptimo motivo (`MOTIVO_DIA_LIBRE_EMPRESA`, último de la lista y **lejos de «Compensatorio», que es lo CONTRARIO**: aquél es un libre que se le DEBÍA y no cuesta nada). Regla en `lib/asistencia/dia-libre-empresa.ts`: deuda = `8 × rata` de `rata.ts`, **congelada** al cargarse; se cobra consumiendo las CINCO columnas del extra (diurna · nocturna · excedente · domingo · feriado), el bruto baja con ellas y los seguros se recalculan sobre el bruto nuevo (mismo trato que `aplicarAjusteEnLinea`). 🔴 **El tope es el EXTRA, nunca el neto**: sin horas extra no se cobra un centavo y la deuda arrastra sin caducar. Se carga por RANGO para toda una empresa (`POST /api/asistencia/dia-libre`) o persona por persona desde Justificaciones —**una sola puerta**, `cargarDeudasDiaLibre`—, solo **admin y contabilidad** (`diaLibreRoles()`), solo días **hábiles**; el cierre anota el pago (único `(colaborador, quincena)`) y **reabrir lo revierte** con soft delete. Se dice en la celda, en «Antes de cerrar», en el Excel, en el PDF y en su sección de la ficha. Migración **`20261203120000` (pendiente de aplicar)**, aditiva y tolerada: sin ella no hay deudas y el cuadro es el de ayer. **Medido 1–15 sep con corte 10-sep**: sin deuda cargada, las 46 líneas dan el MISMO neto ($11.314,29) hasta el centavo; con un día libre a Samir (rata $3,02) su extra de $6,28 se va a cero, queda debiendo $17,88 y **nadie más se mueve**. Candado `dia-libre-empresa.test.ts` (37); **15 mutaciones, 15 cazadas** con 2 controles (`scripts/_mutar-candados-dia-libre.sh`); cambiaron de dirección con nota fechada y CONTROL: `asistencia-motivo-trabajo-fuera` · `planilla-tres-descuentos` (H) · `dias-afuera-y-compensatorio` (4).
- 🔴 **LAS HORAS EXTRA LAS APAGA SOLO LA CASILLA «¿Cobra horas extra?» DE LA FICHA — SER SERVICIO PROFESIONAL YA NO LAS APAGA (14-sep-2026).** Daniel, textual: *«los servicios profesionales de fashion wear sí llevan horas extras»* y *«solo yulissa no cobra, todos los demás sí. Ella es la única excepción hoy y siempre»*. 🩸 Hasta hoy `armarLinea` decía `sinRecargos = fueraDePlanilla || noCobraExtra`: lo que Daniel dijo de Yulissa el 3-sep (*«es solo para ver sus tardanzas y ausencias»*) se había escrito como regla del motor para TODO servicio profesional. Sigue siendo cierto **para ella**, y por eso su ficha (26) tiene la casilla en NO: la excepción vive en la ficha. Con la casilla en SÍ, un servicio profesional **mide** sus extras, sale en Aprobaciones y en el aviso ámbar — pero **sigue sin `dinero`** (`fueraDePlanilla` no se tocó): `extraMedido.monto` es `null`, no se inventa una rata; cuánto vale la hora lo decide quien le paga por fuera. El Reporte (`cuentaHorasExtra`) y su ruta preguntan por la MISMA casilla. **Medido contra producción antes y después (1–15 sep y 16–31 ago): 0 cambios de neto** — los únicos servicios profesionales son Yulissa (casilla en NO) y Daniel Levy (52, inactivo y sin marcas desde el 7-ago). ⚠️ **Marcar a alguien de Fashion Wear como servicio profesional lo SACA de la planilla entera** (sin quincenal, sin seguros, sin neto): los 7 activos suman **$1.888,81** en 1–15 sep. Es otra pregunta, no ésta. Candado: `servicio-profesional-cobra-extra.test.ts`; cambiaron de dirección con nota fechada: `reporte-servicio-profesional-sin-extras` · `aprobaciones-no-lista-servicio-profesional` · `planilla-aviso-extras-sin-aprobar` (g) · `asistencia-servicio-profesional`.
...
- 🔴 **«TRABAJA AFUERA» ES UNA CASILLA DE LA FICHA, Y NADIE CARGA NADA (14-sep-2026).** Daniel, textual: *«a ellas cuando están afuera se les paga el día regular como si hubiesen trabajado las 8 horas, en horario de 9-6, con una hora de almuerzo»* · *«cuando marcan, que es cuando trabajan en la tienda, trabajan de 10 a 7»*. 🩸 Ana Trejos (2) y Cindy De Gracia (3) son impulsadoras: **1 día con marca de 22 hábiles** (16-ago → 15-sep) y cada día sin marca era una AUSENCIA descontada. Con 21 días afuera al mes, cargar el rango a mano cada quincena es la solución equivocada. Regla en **`lib/asistencia/trabaja-afuera.ts`** (`motivoAutomaticoDelDiaSinMarca`): a quien tiene la casilla, **un día hábil ya pasado, sin marca, sin feriado y sin justificación** —exactamente la condición que lo habría hecho ausencia— se le pone «Trabajo de vendedor» solo; el reporte le pregunta en su rama «sin marcas» y la planilla deja de pedirle «no marcó ni un día». **El día que SÍ marca se mide del reloj igual que hoy.** Feriado, fin de semana, vacación (la «ya pagada» se sigue descontando), justificación cargada y día en curso **siguen mandando**. ⚠️ **NO es `no_marca_reloj`** (ésa apaga el reloj SIEMPRE; con las dos, gana). El motivo manual por rango se conserva para el caso suelto. Columna `asistencia_personas.trabaja_afuera` (`DEFAULT false`), migración **`20261124120000` SIN APLICAR** — Daniel la corre cuando la prenda (nombró a 2, 3, 13 y a Yeisibeth Muñoz, que **no tiene ficha**). 🔴 **Se lee APARTE de las fichas** (`leerTrabajaAfuera`, tolerante: sin la columna, «nadie») porque `leerPersonas` ya no tolera una columna ausente; el PUT la escribe en un `update` propio **solo si cambió**. Chip «Trabaja afuera» en ficha, lista y planilla; se edita en Editar › Excepciones al lado de «¿Cobra horas extra?». **Medido 1–15 sep (`scripts/_medir-trabaja-afuera.ts`, solo lectura)**: sin prender, **0 diferencias en 45** ($10.864,30 = $10.864,30); prendiendo 2, 3 y 13: Ana **9 ausencias → 0, +$184,55** (deja de descontar $207,36; el resto son seguros sobre el bruto que sube) · Cindy **9 → 0, +$169,17** · Rodrigo **1 → 0, +$36,96**; nadie más se mueve. Candado `planilla-trabaja-afuera.test.ts` (25); **14 mutaciones, 14 cazadas** con 2 controles (`scripts/_mutar-candados-trabaja-afuera.sh`).
> Detalle completo: [el postmortem](docs/postmortems/asistencia-planilla.md) › «Lo que decía CLAUDE.md hasta el 14-sep-2026».
> ⚠️ Las reglas de PANTALLA de este módulo (qué se dibuja, rótulos, tamaños) viven SOLO en ese postmortem: léelo antes de tocar una pantalla suya.
...
- 🔴 **LOS DÍAS DE VACACIONES SE CALCULAN SOLOS, Y NO SON UN SALDO (17-sep-2026).** Daniel, textual: *«las vacaciones no funciona por día, hay que cambiar eso, funciona que por cada 11 meses trabajado, 1 mes de vacaciones»* · *«1. Un mes son 30 días corridos. 2. La fecha de ingreso que tiene la ficha. 3. [las ya tomadas] lo vemos después»* · *«Quita lo del saldo vacaciones»*. Regla en `lib/asistencia/vacaciones-corresponden.ts`: **30 días corridos por cada 11 MESES** desde `fecha_ingreso`, el período en curso **truncado** a día entero, menos las vacaciones REGISTRADAS (las «ya pagadas» también restan, y se cuentan aparte); días de CALENDARIO, domingos adentro. 🔴 **Se lee «Le corresponden N días», NUNCA «le quedan»**, y va SIEMPRE con la línea gris «No incluye vacaciones tomadas antes del 17 de septiembre de 2026»: nadie sabe qué se tomó antes. 🔴 **NO entra a ningún cálculo de plata** —ni planilla ni liquidación—, con barrido que lo exige: leerlo como saldo y pagarle 45 días a quien ya tomó 30 es plata de verdad. 🔴 **Sin `fecha_ingreso` no sale un número**, ni cero (9 de 49 fichas, medido); esa persona aparece igual en la lista diciendo qué le falta. 🩸 **El saldo escrito a mano se RETIRÓ**: `saldo_vacaciones_dias` y `saldo_vacaciones_corte` quedan en la base sin lectores ni escritores, con `COMMENT` (migración `20261204120000`, **pendiente de aplicar**) y candado que pone el build ROJO si una migración las dropea o si el código vuelve a nombrarlas. Medido el 17-sep-2026: de las **49** fichas **ninguna** tenía un número (47 vacías, 2 con un 0), o sea que la pantalla decía «Falta el saldo» para todo el mundo. **0 diferencias de neto** (1–15 sep, corte 10-sep: $11.314,29 = $11.314,29). Candado `vacaciones-le-corresponden.test.ts` (28); **15 mutaciones, 15 cazadas** con 2 controles (`scripts/_mutar-candados-vacaciones-corresponden.sh`); cambiaron de dirección con nota fechada y CONTROL: `asistencia-falta-configurar` (B) · `asistencia-lista-que-falta` (7 y 9) · `tolerancia-ddl-retirada-asistencia` · `persona-en-el-centro` (I) · `asistencia-vacaciones-sin-saldo-a-mano` · `asistencia-vacaciones-corresponden`.
...
- 🔴 **MULTI FASHION HOLDING SE EXCLUYE POR CÓDIGO (D-108), CON COMODÍN `*` = TODOS LOS VENDEDORES**: enumerar nombres deja entrar al nuevo. La v9 = la v8 **byte a byte** salvo eso. Migración `20261008120000` (**pendiente**).
...
- 🔴 **UN DESCUENTO TIENE FECHAS**: `desde` / `hasta`, el «hasta» **INCLUSIVE** y **OPCIONAL**, grano **MES**; sin `desde`, como siempre. `lib/comisiones/vigencia.ts`, aplicada en `leerDescuentosEfectivos` **antes** de la excepción del mes. Migración `20261007120000` (**pendiente**).
