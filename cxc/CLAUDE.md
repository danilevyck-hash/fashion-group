...
| Qué **clientes NO comisionan** para un vendedor | `comision_exclusion` | 1 fila por (empresa, código de cliente, vendedor normalizado) · 17 activas al cargar (11 pares empresa–cliente, Active Wear con las dos grafías de Reinaldo) | ⚠️ **Migración `20260912120000` pendiente de aplicar.** Soft delete: `activa = false` + `desactivado_por/en`; **nunca DELETE**, es historial. La leen `comision_b2b_v7` y `comision_b2b_detalle`; la escribe solo `/api/ventas/comisiones/exclusiones` (admin). Sin `motivo`: Daniel no lo pidió. |
| Cuánto comisiona cada **vendedor** (tasa de venta y de cobro) | `comision_vendedor_tasa` | 1 fila por nombre de vendedor · 9 | ⚠️ Global (no por empresa). Hay filas con el nombre mal escrito (`REINDALDO ESPINOSA`, una con espacio final) que no cruzan con nadie. Reinaldo = **1 % / 1 %** en sus dos grafías (Daniel: *«pon a Reinaldo 1 y 1»*). Se edita en Comisiones › Configuración › Tasas por vendedor. |
