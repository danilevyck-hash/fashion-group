...
- 🩸 **La basura de las pruebas SÍ se borra de verdad, una vez y con lista explícita.** 16 pedidos de Calvin y 37 de Joybees de las corridas de verificación del 12-13 de agosto (Daniel: *«borro de verdad de la base»*) salen por la migración `20260924120000_borrar_pedidos_de_prueba.sql` (**pendiente de aplicar**). Es la excepción, no la regla, y trae sus frenos adentro: lista de **ids**, nunca un `LIKE`; el que tenga un **envío vivo** a Switch (`estado <> 'error'`) se saca de la lista; y solo se borra lo que ya estaba `deleted`. Candado: `borrar-pedidos-de-prueba.test.ts`.
...
- 🔴 **El Historial guarda EL MISMO Excel que se descargó (bytes idénticos), 90 días**, en el bucket privado `depurador-plantillas` (migración `20260921120000`, **pendiente de aplicar**). **SOLO los Excel de Switch** — el pedido para cliente de Reebok, Tallas y Fotos a mi Excel no se guardan. El cron `cleanup-depurador-archivos` (03:20 UTC) borra el archivo vencido y **la fila con los totales se queda para siempre** (solo pierde el botón). Todos ven y bajan todo. Filtro por compañía (Todas + las 6); Facturas Tienda se anota como «Multifashion».
...
- Crons que tocan la **MISMA empresa** en Switch van **≥15 min** separados (`SEPARACION_MINIMA_MIN`): Switch admite **un solo token válido por USUARIO** (`docs/switch/api-documentacion.pdf`, p. 6), y como cada empresa entra con un único usuario de API, en la práctica es una sesión por empresa.
...
- 🩸 **Dos fuentes de Switch tienen que decir lo mismo** (3-sep-2026): el **cuadre mensual de costo** (`src/lib/alertas/cuadre-costo.ts`, misma pasada de la reconciliación, cero crons nuevos) compara, por (empresa, mes cerrado), el costo del Resumen contra `switch_costo_diario` —el reporte «Total de ventas», que trae las ND y que durante tres meses nadie leyó mientras Active Wear agosto mostraba costo negativo—. **Tampoco es una regla nueva**: es la regla 2 sobre el dato. Solo días comparables (nunca el último del mes, que en esa tabla vale $0; ni los sin fila; ni los leídos antes de cerrar el día), **>2 % Y >$100**, ≥10 días, anti-loop de 7 días por (empresa, mes), un mensaje por pasada. 🔴 **Telegram, no Data Health** — Daniel: «yo no uso Data Health, nunca lo veo». Medido sobre 32 pares may–ago 2026: 0 disparos (peor 0,75 %).
...
- 🔴 **NADA QUE NO SE PUEDA VOLVER A CONSEGUIR SE QUEDA SIN COPIA** (5-sep-2026). Medido: el respaldo tenía **56 tablas de las 136** y afuera quedaban **63 que escriben personas** — el módulo **Asistencia entero**, con las **6.081 marcaciones del reloj** (append-only, el reloj no reenvía el pasado: si se perdían, se perdía la asistencia de todos), los saldos de banco que escribe contabilidad a mano, la configuración de comisiones, los tres catálogos nuevos y hasta el de **Reebok**, que la documentación daba por respaldado y **no lo estaba**. 🔑 El hueco no fue descuido: fue que **nada avisaba** cuando nacía una tabla. Ahora la base entera está clasificada en `src/lib/backup/tablas.ts` por **qué se pierde si se pierde** (`personas` · `congelada` · `switch` · `bitacora` · `retirada` · `vista`) y `backup-nada-sin-copia.test.ts` pone el build ROJO si una tabla nueva de una migración queda sin clasificar, o si alguien saca del respaldo algo que no vuelve. 🩸 Y cierra un segundo hueco silencioso: **una tabla cuya PK no es `id` y no está en `ORDER_BY` deja un respaldo incompleto que parece completo** — la llave real de producción vive medida en `PK_QUE_NO_ES_ID` y el candado exige que el `ORDER_BY` la cubra columna por columna. El respaldo diario pasó de **33,6 MB a ~37,2 MB** (+11%). Lo re-derivable de Switch se decide por costo, no por riesgo: `switch_factura_lineas` (97 MB crudos) queda **afuera** a propósito.
...
- **El resumen diario de ACS por Telegram dice si van arriba o abajo del ritmo de la meta** (3-sep-2026). Daniel: *«el mensaje de telegram igual que hoy en día solo que diciéndome si están qué porcentaje arriba o abajo para la meta»* y, al confirmar la cuenta, *«es calcular 23% arriba del mismo día año anterior sumando todos los días pasados?»* → sí. UNA línea al final (`🎯 Meta  ▲ +13% arriba del ritmo`), nada más cambia: `factor` = objetivo ÷ venta del rango completo un año antes (420.000 ÷ 340.698,55 = 1,2328); `ritmo` = venta del año pasado desde el inicio hasta corte−1 año × factor; % = vendido ÷ ritmo − 1, con el MISMO corte que Mes/Año y el mismo «vendido» que la pestaña Metas. Sin meta grupal activa que cubra el día, o sin comparable, la línea no sale (en enero desaparece sola); falla abierto. Módulo puro `src/lib/multifashion/meta-ritmo.ts`, lectura `meta-ritmo-lectura.ts`. Medido 3-sep-2026: vendido $4.599,07 · ritmo $4.061,12 · **+13%**. Candado `acs-resumen-meta-ritmo.test.ts` (22 mutaciones, 22 cazadas, `scripts/_mutar-candados-meta-ritmo-telegram.sh`).
...
| Qué se descargó del Depurador y su Excel | `carga_history` + bucket privado `depurador-plantillas` | 1 fila por descarga de plantilla Switch · ~140 | La fila (totales) vive para siempre; el ARCHIVO (los mismos bytes que bajaron) solo 90 días (`archivo_path`/`archivo_nombre`, migración `20260921120000` **pendiente**; cron `cleanup-depurador-archivos`). ⚠️ El bucket NO entra a la réplica R2 a propósito: archivos generados, re-derivables, con vencimiento. Solo Excel de Switch — el pedido de Reebok, Tallas y Fotos a mi Excel no se guardan. |
...
- **Smart suggestions:** 💡 proactivas inline (contactar cliente $10K+, depositar vencidos, escalar reclamo +45d). La de «cerrar período +30 días» de Caja se retiró el 4-sep-2026: el criterio real de cierre es «queda poca plata» (la secretaria), no los días abiertos
