...
- **Vistas / Materialized views:** Convención de nombres: sufijo `_mv` = materialized view, `_vw` = view. (No verificado contra catálogo pg — vía REST no se distingue MV de view; confirmar con acceso a catálogo si se necesita certeza.)
  ...
  - **`switch_recibos` — escritura selectiva (26-jul-2026).** El sync ya NO reescribe la ventana entera. Antes: DELETE de los 3 meses + INSERT de todo, en cada una de las 4 corridas diarias = **3.416 filas borradas + 3.416 insertadas por corrida** (13.664 filas muertas/día) para reflejar **~10 recibos nuevos** (medido: 41,4 recibos/día de alta real sobre 18 días). Ahora `leerMesGuardado()` trae el mes tal como está, `diffRecibos()` (`src/lib/switch-api/recibos-diff.ts`) lo compara contra lo que devolvió Switch y solo se escriben las diferencias. Medido contra producción el 26-jul: **3.416/3.416 filas pareadas, 0 escrituras, equivalencia exacta fila por fila** (`scripts/_diag-recibos-churn.ts`, solo lectura).
    ...
    - 🔴 **Efecto colateral MEDIDO del paginado — decisión de negocio PENDIENTE.** Al ver las 3.904 facturas en vez de 1.000, la heurística marca **20 recibos más** como retención en `american_classic`, **todos del cliente `cli=1` "CONTADO"**. Son FALSOS POSITIVOS, no el bug corrigiéndose: CONTADO es el cliente mostrador genérico (25.800 de las ~26.500 facturas de la empresa), y con 3.455 facturas suyas en la ventana la coincidencia `total ≈ impuesto/2 ± 0.01` deja de significar nada — un recibo de $2.00 "cuadra" con **6** facturas distintas y uno de $0.01 con 4. Un cliente de mostrador que paga en efectivo no es agente de retención de ITBMS (eso es B2B). Los 6 `es_retencion=true` históricos de esa empresa son también todos CONTADO, o sea la misma colisión. **Impacto hoy: NINGUNO** — el "último pago" no cambia para ninguno de los 786 clientes (CONTADO tiene miles de recibos y el último no es ninguno de los 20), y `american_classic` no entra en comisiones (no está en `B2B_EMPRESA_KEYS`). **Guard propuesto, sin aplicar:** no clasificar como retención los recibos de un cliente tipo CONTADO. Medido: cambiaría **cero** filas hoy en las 6 empresas (las 5 B2B tienen 0 retenciones de CONTADO en toda su historia; en `american_classic` las 6 quedan fuera de la ventana, que el sync no reescribe) y evitaría exactamente los 20 falsos positivos. Evidencia reproducible: `scripts/_diag-impuesto-truncado.ts` (solo lectura, no toca Switch).
  - ⚠️ **`db-max-rows` = 1000 — barrido del repo (26-jul-2026), lo que queda SIN arreglar.** Se auditaron todas las lecturas de PostgREST de `src/` y `scripts/` buscando el mismo defecto y se **midió el tamaño real** de cada tabla antes de opinar. Ya paginados: `leerMesGuardado` y `loadImpuestoMap` (sync-recibos) y `buildSwitchIdMap` (sync-utilidad — no truncaba: máx. 162 filas en la ventana del cron y 710 en un backfill de año entero, porque utilidad corre solo para las 5 de `B2B_COMISION_KEYS`). **Confirmados truncando HOY, no tocados** (fuera del alcance de ese PR, cada uno necesita su propio cambio):
    - `switch_estadocuenta` = **1.511 filas** → `src/app/api/upload/route.ts:13` y `src/app/api/notification-badges/route.ts:54` leen `(empresa_key, synced_at)` sin tope y ordenado por `synced_at` desc. Como todas las filas de una corrida comparten el mismo sello, las primeras 1.000 son de UNA empresa → la frescura de CXC y el badge `cxcStale` ven una sola empresa. Es un número en pantalla, mal.
    - `clientes_empresa_12m_vw` = **1.563 filas** → `src/lib/ventas/queries.ts:340-353` usa `.limit(5000)`, que no protege de nada (el tope real es 1.000). De ahí sale la fila sintética "Otros clientes (N)" de `/ventas`, o sea **plata sumada sobre una lectura corta**.
    - `switch_clientes` = **1.710 filas** → `src/app/api/catalogo/switch-clientes/route.ts:26` (`.order("nombre")`, sin tope): los clientes alfabéticamente posteriores al corte no aparecen en el selector del checkout de catálogos. `sync-clientes-master.ts` ya pagina esta misma tabla.
    - Reebok/catálogo público: `inventory` y `products` sin tope en `api/catalogo/[marca]/public/route.ts:55,65`, `api/catalogo/reebok/stats/route.ts:12,29,39` (otro `.limit(5000)`) y `api/catalogo/reebok/inventory/route.ts:14`. Truncado ⇒ productos que salen **Agotado** en la tienda pública, y encima el resultado se cachea con `unstable_cache`.
    ...
    - **Dos reglas que salieron del barrido:** (1) `.limit(N)` con N > 1000 es el marcador de "alguien creyó estar cubierto" y no cubre nada — el propio repo ya lo dice en `api/cxc/ultimo-pago/route.ts:20`; (2) **paginar sin `.order()` tampoco alcanza**: con offset y sin orden estable PostgREST puede repetir o saltear filas entre páginas. `api/multifashion/fidelizacion/route.ts:67,80,93` pagina pero sin ordenar, y agrega.
...
## Crons (vercel.json)
53 entradas configuradas (eran 54 hasta el 26-jul-2026, cuando se retiró `multifashion-sync` — ver `multifashion_tickets` arriba). **Una entrada = una ocurrencia al día**: para frecuencia sub-diaria se agregan entradas separadas del mismo path, NUNCA una lista de horas (`0 15,19,23 * * *`), que Vercel Pro sí acepta — ver la nota de slots más abajo. Límite Vercel Pro: 100 cron jobs/proyecto.
