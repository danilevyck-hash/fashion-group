...
> 📄 Las piezas A–D, los remates, el freno por tienda, el celular y las citas de Daniel: el postmortem › §§ 1–14 y «Lo que decía CLAUDE.md hasta el 23-sep-2026». Léelo antes de tocar una pantalla.

- 🔴 **DOS PUERTAS: TIENDAS (dónde se gasta: se registra, edita y anula ahí) y MARCAS (a quién se le pasa: se cierra y se manda el ZIP).** Abre en **Tiendas** (nombre del DIRECTORIO por código, lo reportado como único monto, cada fila un `<a>` a su ficha), con pestañas Tiendas · Marcas · Impulsadoras · Mobiliario; la ficha es **UNA tabla por fecha** (facturas + muebles + impulsadora; «···» edita y anula por las rutas de siempre); la marca muestra su período abierto con **UNA línea por tienda** y **sin proyectos** (**Reportes se fue**). Interruptor `MARKETING_TIENDAS_Y_MARCAS` (hoy `true`). Candado `marketing-tiendas-y-marcas`.
- 🔴 **MULTIFASHION ES UNA TIENDA (D-108) Y NO APARECE EN NINGUNA MARCA — UNA regla** (`gastoEsDeMultifashion`) en portada, marca, reporte y ZIP; un gasto es factura · mueble · impulsadora con **UNA marca**, su tienda o «General», `se_reporta` prendido (apagado no suma ni va al ZIP); duplicado = proveedor normalizado + monto + fecha + **tienda**, salvo números distintos. Migraciones `20261216120000/120100`, `20261218120000` y `20261218120100`, **aplicadas** (📄 cifras y detalle: postmortem § 12).
...
- 🔴 **EL PERÍODO MANDA (23-sep-2026): arriba se elige el período, abajo se ve lo de ese período.** La ficha y la lista de Tiendas llevan la barra **Abierto · cada cierre («mid 2026 · PVH») · Todos** y abren en **Abierto** (todo gasto sin sello a un período CERRADO); los chips salen de los períodos REALES, en «Todos» se agrupa por período con subtotales que suman, y **ningún número cambia** (D-118: $1.771,27 · $4.701,26 · $6.472,53). 🩸 **Lo anulado desaparece de todas las pantallas**, anular pide escribir **ELIMINAR** y a los **90 días** el cron `cleanup-marketing-anulados` (03:40 UTC) lo borra con sus adjuntos y sellos. ⚠️ Los muebles no tienen `anulado_en`. Puro en `lib/marketing/periodo-manda.ts`; candado `marketing-el-periodo-manda`.

- 🔴 **EN EL CELULAR, UNA FILA ES UN NOMBRE Y UN MONTO (24-sep-2026)** — Daniel, textual: *«ya son datos que veré adentro, eso me ensucia la pantalla, no solo aquí sino en todo el sistema»*. Mockup aprobado letra por letra —**1a · 2a · 3a · 4c · 5b · 6b · 7b · 8a · 9a · 10a · 11a · 12a · 13b**, cada una en el postmortem § 14— (`MARKETING_CELULAR`, hoy `true`; `false` = el celular de antes, intacto): la portada es **Tiendas** con su total arriba y el número grande **más chico**; la ficha, **dos renglones por gasto con el monto a la vista** (🩸 en la tabla el monto empieza en el píxel 537 de 390), «General» pliega sus pagos de impulsadora en UN renglón y **sin el número de factura**, y con «Todos» el período es el **título del grupo con su subtotal**; la marca es un número con **ZIP y Cerrar anchos**; **Marcas NO lleva total de las tres**; una impulsadora es una fila con **«Pagar»**; Mobiliario, una fila por producto **con su foto** y **sin «entregadas 543 de 561»**; el cierre sube como **hoja de iOS** sin tocar una regla; el botón apagado **dice qué falta**; y la × de una foto sale solo al tocar **«Editar»**. 🔑 **UN SOLO ÁRBOL** (`useEsCelular`, hasta 639 px): con los dos montados cada nombre salía dos veces. 🔴 **Ningún número cambia**. Candado `marketing-celular`.
- 🔴 **ESCANEAR LA FACTURA CON LA CÁMARA (24-sep-2026, 4c)**: «＋ Gasto» en el celular abre con **tres puertas** —Escanear · Elegir el PDF · Escribirlo a mano—. 🔑 **El lector ya existía** (`/api/marketing/ia/leer-factura`, seis campos): faltaban `capture="environment"` y que aceptara una FOTO (`mediaType` del bucket o del nombre, **PDF por defecto**; el prompt no cambió). La foto se achica a **1600 px · JPEG 0,8**, se sube «sin dueño» y lo leído entra por el **`initial` que ya existía**. 🔴 **Nada de lo que se guarda cambia**: se cuelga como `foto_factura` por el camino de siempre y el POST no ganó un campo. **Falla ABIERTA**. Candado `marketing-escanear`.
...
- `mk_mobiliario_notas_proveedor` (los costos del proveedor) queda **SEPARADA del inventario. NO fusionar**. La **nota de entrega** usa un solo generador para compartir e imprimir, y el PDF se arma **antes del clic** (iOS bloquea la hoja de compartir con un `await` de red en el medio).
