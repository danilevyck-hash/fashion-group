...
- 🔴 **LA TARJETA DEL HUB DICE LO QUE EL CLIENTE VE, Y SU PULSO (22-sep-2026)**: el contador son **TARJETAS**, no filas (Joybees decía 81 y el catálogo mostraba 70; `catalogo/tarjetas.ts`, RPC `catalogos_contadores_hub`, `20261214120000` aplicada), y cada tarjeta lleva comprobantes · plata · «hace N días» de los últimos **90 días**, la MISMA ventana de la lista importada de `comprobantes-ventana.ts` (`pulso-pedidos.ts`, RPC `catalogos_pulso_pedidos`; «hoy» de Panamá por parámetro; sin comprobantes NO escribe «$0.00»). Candados: `catalogo-hub-pulso` · `catalogo-hub-tarjeta-completa`.
...
- 🔴 **NINGUNA HOJA DEL PAPEL DEL CLIENTE SE QUEDA SIN DECIR DE QUIÉN ES (22-sep-2026)**: la cabecera se repite en CADA hoja, cada una dice «Página N de M» y la tabla reserva el sitio del total (`order-pdf-core.ts`; arreglo de LUGAR, ningún número se movió). 🔴 **El Excel de Comprobantes se puede filtrar**: «Fecha» es FECHA, hay columna «Tipo», «No se ha mandado a Switch» es columna propia, «Items» se suma y «TOTAL» no cae encima. Candados: `pedido-pdf-hoja-con-identidad` · `comprobantes-excel-que-se-filtra`.
...
- 🔴 **LA FOTO DE UNA TIENDA SE PUEDE GUARDAR, Y SIGUE AL PERÍODO (24-sep-2026)**: 🩸 **ninguna se pudo guardar nunca** —`mk_adjuntos_destino_chk` exige proyecto y la puerta manda `foto_proyecto` sin él (23514): **0 de 160** filas y **4 huérfanos** hoy—; y las dos de D-118 («mid 2026 · PVH») salían en «Abierto». La migración `20261219130000` (**NO aplicada**) afloja la regla, agrega `mk_adjuntos.periodo_id` y sella las **52** viejas **por lista de ids**. La foto nace en el período abierto del **gasto más reciente** de su tienda, la cuadrícula sigue al chip, el ZIP la lleva sin duplicar y lo que no se pudo anotar **se borra del cajón**. Falla ABIERTA. `MARKETING_FOTOS_CON_PERIODO`; candado `marketing-fotos-de-tienda`.
- 🔴 **ESCANEAR LA FACTURA CON LA CÁMARA (24-sep-2026, 4c)**: «＋ Gasto» en el celular abre con **tres puertas** —Escanear · Elegir el PDF · Escribirlo a mano—. 🔑 **El lector ya existía** (`/api/marketing/ia/leer-factura`): faltaban `capture="environment"` y que aceptara una FOTO (**PDF por defecto**; el prompt no cambió). 🔴 **Nada de lo que se guarda cambia**: se cuelga como `foto_factura`. **Falla ABIERTA**. Candado `marketing-escanear` (📄 postmortem § 14).
...
- 🔴 **EN EL CELULAR, ☰ ABRE EL MENÚ ENTERO, A PANTALLA COMPLETA (24-sep-2026)**: título «Menú», buscador que solo acorta ESA lista (`coincideBusqueda`, subcadena normalizada, **nunca por parecido**), «Inicio», los TRES grupos como secciones con TODOS los módulos del ROL —ícono en su color, el de aquí marcado— y al pie nombre · rol · Contraseña · Salir. **12 sin desplazar, 2 toques a cualquiera.** 🩸 La hoja de abajo arrancaba a la MITAD y mostraba 6 de 20. 🔴 Los CUATRO sin acento ya tienen tono, **fuera de `getModuleKeyFromPath`**. `MODO_DEL_CAJON` en `cajon-por-grupos.ts` (`pantalla`; `hoja` devuelve la hoja, `CAJON_HOJA_ABAJO=false` el cajón lateral). Candados `menu-pantalla-completa` · `cajon-hoja-abajo`.
- 🔴 **Y LA BARRA DE ARRIBA SE ESCONDE AL BAJAR (24-sep-2026)**: hasta `sm` queda **FG · el módulo · ☰** —se fueron campana y lupa para TODOS los roles, Daniel: *«no uso ni notificaciones ni buscar»*— y se va al deslizar hacia abajo, vuelve al subir: **782 px de 844 en vez de 736**. 🔴 Manda el SENTIDO del dedo, no la posición; arriba del todo está SIEMPRE; **`--fg-altura-encabezado` pasa a 0** y las barras pegajosas suben con ella; sin animación con «reducir movimiento». En la computadora nada cambia. `lib/navegacion/barra-celular.ts`, interruptor `BARRA_QUE_SE_ESCONDE`. Candado `barra-celular`.
- **SPAs de un solo route**: el patrón de referencia es **Reclamos** (`reclamos/ReclamosClient.tsx`), que reconstruye el estado desde la URL. Los de routes reales (Caja, Préstamos, Guías, Clientes) ya son correctos.
