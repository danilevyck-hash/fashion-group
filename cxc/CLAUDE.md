...
| Qué **clientes NO comisionan** para un vendedor | `comision_exclusion` | 1 fila por (empresa, código de cliente, vendedor **canónico**) · 17 activas hoy (11 pares empresa–cliente, Active Wear con las dos grafías de Reinaldo) → **11 activas** al aplicar `20260913120000` (las 6 repetidas se apagan firmadas) | Soft delete: `activa = false` + `desactivado_por/en`; **nunca DELETE**, es historial. `excluye_venta` / `excluye_cobro` (desde `20260913120000`, **pendiente**; default las dos). La leen `comision_b2b_v8` y `comision_b2b_detalle`; la escribe solo `/api/ventas/comisiones/exclusiones` (admin). Sin `motivo`: Daniel no lo pidió. |
| Qué **grafías de Switch son la misma persona** | `comision_vendedor_alias` | 1 fila por grafía · 5 (3 de Reinaldo → `REYNALDO ESPINOSA`, 2 de Aguas → `REY STOUTE AGUAS`) | ⚠️ **Migración `20260913120000` pendiente de aplicar.** Se aplica SOLO por `comision_vendedor_canonico()`; sin alias el nombre sale recortado tal cual. Sin pantalla: se carga por migración. |
