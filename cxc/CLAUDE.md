...
> - 🔴 **PENDIENTE, decisión de Daniel — `confecciones_boston/estadocuenta` NO cabe en una función y sus saldos están congelados desde el 28-jul 04:36.** Cada intento quema ~13 min de función 4×/día y muere. Su universo real es **459 clientes con saldo abierto** (y 326 con factura desde el 1-may), no 4.912 → restringir el bucle a los clientes que pueden deber algo lo bajaría a ~300 s y entraría cómodo. **No se implementó a propósito:** es un cambio en la ruta del DINERO (toca el reconcile que pone `saldo=0` a los clientes que no se consultaron — restringir el set sin cuidado pondría en cero saldos buenos) y define qué clientes se refrescan, o sea lógica de negocio. Requiere aprobación explícita antes de tocarlo.
>
