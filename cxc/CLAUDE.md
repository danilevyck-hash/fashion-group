...
> 🔴 **MARCACIÓN «UN TOQUE» (24-sep-2026, `MARCACION_UN_TOQUE` en `lib/marcacion/un-toque.ts`, hoy `true`)**: UN botón fijo abajo que **nunca cambia de posición** (saltaba 202 → 344 px) y con las dos marcas queda gris en «Listo por hoy»; 🔴 **aceptar la foto ES marcar** —se fue «Enviar», de 4 toques a 3— con el «Deshacer» de 2 min DENTRO de la pastilla verde; **cámara normal** (`capture="environment"`, Daniel: *«sus fotos son del lugar, no de su cara»*); la **ubicación se pide AL ABRIR** y negada dice «Ajustes › Safari › Ubicación»; se van la nota y «Mis marcas» y queda una línea solo si **ayer** faltó la salida; el encabezado dice **«Marcación»** (decía `marcacion`) y a ese rol no se le dibujan campana, lupa ni menú; sin señal, una línea propia en vez de la franja naranja que tapaba el encabezado. 🔴 **Nada de lo que se guarda cambia** (candados `marcacion-un-toque` · `marcacion-payload-igual`). `false` = la pantalla de antes.
...
- 🔴 **LAS PESTAÑAS VISITADAS SE QUEDAN ARMADAS (24-sep-2026)**: se ESCONDEN, no se desarman (`pestanas-vivas.ts`, `ASISTENCIA_PESTANAS_VIVAS`), y la que nadie tocó **no se monta**. 🩸 Ir a Planilla y volver perdía la quincena, el cuadro, «Antes de cerrar» y **el corte volvía al propuesto (13/28)** —otra lectura del reloj, sin avisar—. La quincena y el corte viajan además en la dirección (`plQuincena` · `plCorte`, `replace`); 🔴 **el cuadro generado NO se guarda en ningún storage**. Candado `asistencia-pestanas-vivas`.
- 🔴 **GUARDAR UNA HORA NO BORRA LA TABLA NI SALTA ARRIBA (24-sep-2026)**: la recarga de después de guardar es SILENCIOSA —pastilla FIJA, la tabla se queda con lo viejo— y 🩸 antes desaparecía segundos y el teléfono quedaba arriba de todo. 🔴 **La fila recién corregida no se va** aunque «Solo a revisar» la saque (chip «listo»), hasta cambiar el filtro o recargar. El `POST …/correcciones/dia` **no cambió**. `ASISTENCIA_GUARDAR_SIN_SALTO`; candado `asistencia-guardar-sin-salto`.
- 🔴 **QUIEN NO MARCÓ EN EL PERÍODO APARECE IGUAL, EN GRIS (24-sep-2026)**: 🩸 Yeisibeth Muñoz (306) no existía en 1–15 sep ni María V. Bethancourth (49) en la quincena en curso, y no había cómo arreglarles las horas. Sale con sus días **en cero y NINGUNO ausencia** —medido: la planilla le da `HORAS_CERO` y la deja en «Tú decides», sin cobrarle una ausencia— y se corrige por la puerta de siempre (`correcciones/dia`). Excel y PDF la llevan igual. `sin-marcas.ts`, `ASISTENCIA_SIN_MARCAS_VISIBLE`; candado `asistencia-sin-marcas-visible`.
- 🔴 **GRACIA DEL ALMUERZO: 5 MIN SOBRE LA DURACIÓN, UNA PARA LAS CUATRO (24-sep-2026)**: Daniel: *«60 que dura 65 no descuenta nada; 66 descuenta los 6»* — PUERTA, no descuento (`reglas-nuevas.ts`, `asistencia_reglas.gracia_almuerzo_min`, editable en Reglas del cálculo). 🩸 Se descontaba desde el minuto uno. `GRACIA_ALMUERZO`; sin la columna, 0. Candado `gracia-almuerzo`.
- 🔴 **ENTRADA AUTORIZADA POR DÍA (24-sep-2026)**: en «Arreglar el día», «Hoy entraba a las __:__» con motivo y firma (tabla propia `asistencia_entradas_autorizadas`; se anula, nunca se borra); la extra de ENTRADA se mide desde esa hora —o la marca, si marcó después— hasta la entrada del horario (10:00 · 06:00 · marcó 05:58 → **240 min**), misma puerta del mínimo, va a Aprobaciones y paga al recargo por la hora del día. Sin autorización, cero como hoy. `ENTRADA_AUTORIZADA`; candado `entrada-autorizada`.
- 🔴 **AVISO «llegó N min antes · ¿entrada autorizada?» SOLO desde 30 min (24-sep-2026)**, configurable (`aviso_entrada_temprana_min`); no cuenta ni frena; abre «Arreglar el día». 🔴 **Ninguna quincena CERRADA cambia** (candado `quincena-cerrada-no-cambia`). Migración `20261219120000` **pendiente** (Daniel).
...
- 🔴 **LOS DÍAS Y LOS DOS HORARIOS SON CONFIGURABLES POR PERSONA (18-sep-2026):** «hábil» es la lista de cada quien (`asistencia_horarios.dias_laborables`; en NULL manda la EMPRESA: **Multifashion lunes a SÁBADO**), y cada persona tiene DOS horarios —el del reloj y el del teléfono (`entrada_afuera`/`salida_afuera`, vacío = el de adentro)—, **y cuál aplica lo decide la PRIMERA marca del día**. 🔴 **El domingo no se toca.** Regla en `horario-configurable.ts`, lectura ÚNICA en `horarios-server.ts`; migración `20261208120000` **aplicada** (falla ABIERTA sin ella).
...
> 📄 Las piezas A–D, los remates, el freno por tienda y las citas de Daniel: el postmortem › §§ 1–11 y «Lo que decía CLAUDE.md hasta el 23-sep-2026». Léelo antes de tocar una pantalla.

- 🔴 **DOS PUERTAS: TIENDAS (dónde se gasta: se registra, edita y anula ahí) y MARCAS (a quién se le pasa: se cierra y se manda el ZIP).** La portada abre en **Tiendas** (16 + «General», nombre del DIRECTORIO por código, lo reportado como único monto, cada fila un `<a>` a `/marketing/tienda/<código>`) con pestañas Tiendas · Marcas · Impulsadoras · Mobiliario; la ficha es **UNA tabla por fecha** (facturas + muebles + pagos de impulsadora, «···» edita y anula por las rutas de siempre, anulados plegados, «＋ Gasto» con la tienda puesta); la marca muestra su período abierto con **UNA línea por tienda** y **sin proyectos** (`?proyecto=` viejo redirige a la ficha; **Reportes se fue**, `?vista=reportes` cae en Tiendas). Interruptor `MARKETING_TIENDAS_Y_MARCAS` (`lib/marketing/tiendas-y-marcas.ts`, hoy `true`; `false` = todo lo de antes). Candado: `marketing-tiendas-y-marcas`.
- 🔴 **MULTIFASHION ES UNA TIENDA (D-108) Y NO APARECE EN NINGUNA MARCA — UNA regla** (`gastoEsDeMultifashion`: por `tienda_codigo` o por el proyecto) en portada, marca, reporte y ZIP; un gasto es factura · mueble · impulsadora con **UNA marca**, su tienda o «General», `se_reporta` prendido (apagado no suma ni va al ZIP); duplicado = proveedor normalizado + monto + fecha + **tienda**, salvo números de factura distintos. Medido 23-sep: Tommy **$116.675,66** · Calvin **$73.782,95** en TODAS las superficies (Reportes sumaba $1.319,25 y $2.477,58 de la tienda propia); tiendas $200.060,24 = marcas + Multifashion + no reportado. Migraciones `20261216120000/120100` **aplicadas**; `20261218120000` (la factura huérfana de $2.307,32 toma D-108, 1 fila) y `20261218120100` (`marketing` a contabilidad, 1 fila) **aplicadas el 23-sep-2026**.
- 🔴 **CONTABILIDAD ENTRA SOLO A MIRAR** (`lib/marketing/roles.ts`: `ROLES_MARKETING` lee, `ROLES_MARKETING_ESCRITURA` escribe; las rutas que escriben contestan 403; la ficha del menú se enciende por herencia de `gastos-contabilidad` antes de la migración). Se conservan tal cual: «＋ Gasto» con sus tres formularios, el cierre con nombre y nota de crédito, el ZIP y los links de 30 días, los ZIPs anotados, Impulsadoras (todos los meses sin pagar) y Mobiliario.

- 🔴 **EL PERÍODO MANDA (23-sep-2026): arriba se elige el período, abajo se ve lo de ese período.** La ficha de la tienda y la lista de Tiendas llevan la barra **Abierto · cada cierre («mid 2026 · PVH») · Todos**, abren en **Abierto** (todo gasto sin sello a un período CERRADO, de cualquier marca), los chips salen de los períodos REALES de los gastos, en «Todos» la tabla se agrupa por período con subtotales que suman, y **ningún número cambia** (D-118 medido contra los sellos y el ZIP real: Abierto $1.771,27 · mid 2026 $4.701,26 · Todos $6.472,53 — ⚠️ el mockup ponía el mueble de $4.630 en Abierto y la base y el ZIP lo tienen en «mid 2026»). 🩸 **Lo anulado desaparece de todas las pantallas** (ni chip, ni fila, ni conteo; el servidor no lo manda), anular pide escribir **ELIMINAR** y a los **90 días** el cron `cleanup-marketing-anulados` (03:40 UTC) lo borra de verdad con sus adjuntos y sellos (medido: 15 anuladas, 14 se borrarían en la primera corrida; los muebles no tienen `anulado_en`). Bajo `MARKETING_TIENDAS_Y_MARCAS`; `hoyPanama()`. Puro en `lib/marketing/periodo-manda.ts`; candado `marketing-el-periodo-manda`.
