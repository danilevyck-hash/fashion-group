...
- 🔴 **REEBOK ENTRA POR DOS ARCHIVOS, Y LA PANTALLA DICE CUÁL SE SUBIÓ (17-sep-2026):** la **confirmación de compra** (lo que va a llegar, para cotizar — **no cambió**) y el **despacho** (lo que llegó), que arregla tres números que se inventaban: el **costo se LEE** (`Precio after Disc`; el archivo real trae 20 %, 25 % **y 30 %** — medido: +$1.379,51 en ropa y −$329,94 en calzado contra el descuento asumido), el **código de barra es el `UPC`** (era el SKU de Reebok) y la **cantidad es `Quantity`** (era la proyección del mes). 🔴 **El MISMO archivo con o sin las columnas nuevas**: la regla de respaldo es un DATO (`COLUMNAS_DESPACHO`, `depurador/reebok-despacho.ts`) —`PO NAME` → `BP Reference No.` → `Orden`; `Category` → `SHOES` solo en calzado; `Department` derivado de `Segmento de negocio` por **palabra entera, nunca `includes`**— y **ninguna columna que hoy falta es obligatoria**. El despacho produce los MISMOS `ReebokItem`: no hay un segundo generador de las 25 columnas ni una segunda cuenta de costo. Candado `reebok-despacho.test.ts` (**15 mutaciones, 15 cazadas**). Detalle y las tres decisiones abiertas de Daniel, en el postmortem.
