...
- 🔴 **El DESTINO sí se autollena con UN solo destino** (4-sep-2026, Daniel: «quita esa regla. Que se autollene como lo discutimos antes.») — definido en su tabla o único en la historia AGRUPADA; con varios salen botones y elige la persona; lo escrito no se pisa. El pareo por parecido sigue prohibido. Detrás de `GUIAS_ATAJOS_NUEVOS`.
...
- **La dirección de un renglón es el DESTINO del envío, no la dirección del cliente** (4-sep-2026): `clientes_master` no tiene esa columna y el campo sigue siendo **texto libre**. Bajo el campo hay **botones con los destinos de ese cliente** — los **definidos** primero (ver el bullet de abajo; D-142 Sporting Shoes con 8 y tienda opcional «Westland · tienda 6»), el resto por historia (agrupado **exacto**, grafía más usada, máx. 6). **Con VARIOS destinos el botón se toca, nunca se aplica solo**; con UNO solo autollena (bullet de arriba). Cuelga del MISMO `GUIAS_ATAJOS_NUEVOS` y no cambia lo que se guarda.
- 🔴 **Los destinos definidos viven en la tabla `guias_destino_cliente` y se administran en Guías › Configuración** (4-sep-2026) — pestaña a pantalla completa en `/guias`, **admin Y secretaria** (Daniel: «configuraciones también deja a secretaria»); bodega y vendedor, 403. Nació de dos correcciones textuales: *«city shoes → Calle 19 Central, al lado de la joyería Super Oro. Y Nine Sport en Calle 19 Central.»* — antes cada corrección necesitaba un despliegue. **Precedencia en UNA función** (`destinosDefinidosPara`, `destinos-clientes.ts`): **tabla → constante `DESTINOS_DEFINIDOS` (la red mientras la migración `20260918120000` no corra; las dos correcciones también están ahí) → histórico agrupado**. Soft delete **firmado, NUNCA DELETE**; única entre activas; RLS service_role. La pantalla **no toca `guia_items`**: el histórico solo se LEE, como ayuda para promover un destino **de un toque — nunca solo**. Candados: `guias-destinos-precedencia.test.ts` · `guias-destinos-config-route.test.ts` · `guias-configuracion-pantalla.test.tsx`.
...
| Los **destinos definidos** de cada cliente (botones y autollenado del formulario) | `guias_destino_cliente` | 1 fila por (cliente, destino) · 24 al aplicar `20260918120000` (⚠️ **migración pendiente**; hasta entonces manda la constante `DESTINOS_DEFINIDOS`) | Se administra en **Guías › Configuración** (admin y secretaria). `tiendas text[]` en el mismo grano (solo D-142 las usa). **Soft delete firmado (`activo=false`), NUNCA DELETE**; única entre activas. Precedencia tabla → constante → histórico en `destinosDefinidosPara`. **No toca `guia_items`.** |
