...
- 🔴 **ASISTENCIA: UNA FILA DE MANDOS Y DIEZ COLUMNAS (24-sep-2026)**: selector · lupa · **«Descargar» (Excel · PDF)** · **UNA pastilla con los dos relojes**, y los avisos plegados en «N avisos del período». 🩸 La columna «Sale» se retiró (11 → 10): el código va DELANTE del nombre, alineado, y la salida en burbuja. En el día abierto «Arreglar el día» y «Justificar» salen al pasar el mouse —el teclado los alcanza— y **los días que no llegaron no los ofrecen**.
- 🔴 **EN EL CELULAR, EL MÓDULO ABRE EN UNA PORTADA Y ASISTENCIA SON TARJETAS (24-sep-2026)**: 🩸 888 px de tabla en 356. Las cinco pestañas como filas iOS con su conteo REAL, una tarjeta por colaborador con el MISMO pie, Colaboradores por empresa y el tablero de cierre **solo informa**: se cierra entrando a la empresa.
...
- 🔴 **REFERENCIA, EL REDISEÑO (25-sep-2026, `REFERENCIA_2026_09` en `lib/ventas/referencia-pantalla.ts`, hoy `true`; MISMA vista en celular y computadora)**: agrupar por modelo = quitar los **3 ÚLTIMOS CARACTERES, letra o número** (Fashion Wear: 5.130 códigos, de 4.820 «modelos» a **2.573**; Reebok no se mueve, Joybees agrupa de MENOS y **nunca de más**). Sale la tarjeta del **MODELO** —la suma de sus colores, que hoy NO existe— con **Compré · Vendí · Stock · % vendido**, «Llegadas N desde mes/año», las **dos últimas llegadas**, **FOB · Precio prom · Margen** en UNA línea y un renglón **PLANO** por trimestre (no se adivinan temporadas); debajo, un renglón por COLOR **ordenado por STOCK**, los sin mercancía **plegados**, y el resto tras **«Más info»**. Buscador corto y pegajoso, acciones en el «···». 🔴 **Ningún número cambia de cuenta** (`NB2570001` = 935 · 552 · 345 · 62 % · $15.05 · 39 %, prendido y apagado) ni el Excel de forma. `false` = la pantalla de hoy. Candados `referencia-2026-09` · `referencia-modelo-3-caracteres`.
- 🔴 **«EL 80 % SE VENDIÓ EN N SEMANAS» ES UNA LECTURA APARTE** (`lib/ventas/referencia-llegadas.ts`): la bodega se mira como una FILA —lo que llegó primero sale primero— con `stockPrevio = comprado − stock − vendido`; **no toca Stock, ni Vendí, ni el cuadre**, y en pantalla se dice **«aprox.»**. 🔑 Convive con «Nada de FIFO»: no se le atribuye una venta a una compra. Medido: NB2570 nov-2025 · 360 pzas · **21 sem**; ago-2026 **«—», queda 100 %**. Candado `referencia-llegadas-80`.
...
- 🔴 **2022 y 2023 se sirven** en Productos y Utilidad (`anioValido`) y un año sin datos dice «tiene datos desde <mes> <año>»; el Excel de Clientes marca el mostrador y su TOTAL dice qué suma; la matriz dice «no vendiste» en las SEIS llamadas (`deltaCeldaDe`).
- 🔴 **EN EL CELULAR, VENTAS SE REHIZO (25-sep-2026)**: `VENTAS_CELULAR` (`lib/ventas/celular.ts`, hoy `true`; `false` = las tres pestañas de antes). **Resumen**: los cuatro números en UNA línea con la cifra corta y el exacto con centavos **una sola vez al pie** —🩸 $7.069.116,31 se decía DOS veces—, y la lista por empresa es una tabla compacta de **un solo alto** (eran 65, 78 y 112 px). **Clientes**: abre por **plata del año** (🩸 abría por última compra: el más grande quedaba cuarto de casualidad), el **% manda** en la fila y la línea gris dice «compró ayer» / «3 meses sin comprar» de la última compra REAL. **La hoja del cliente** trae el año contra el pasado **por MISMOS DÍAS** (+28,0 %, no +20,9 %), una fila por empresa con su %, y el mes a mes al tocarla —🔴 **un mes negativo va hacia ABAJO en rojo**: enero 2026 de Fashion Wear en D-25 fue **−$1.026,14**—; ruta nueva `GET /api/clientes/[codigo]/por-empresa-anual`. **Productos**: **CINCO columnas** (Descripción · Precio prom. · Margen · Cantidad · Total) en las DOS pantallas, «Dejó de venderse» **debajo** de la lista (🩸 empujaba de 123 a 385 px), los chips en una fila, las notas de débito en un ⓘ y **la última empresa recordada**. 🔴 **«Descargar» es UN botón**: esta pestaña, o **las tres en un solo Excel** —reusa los TRES generadores de siempre—. 🔴 **Ningún número cambia**. Candados `ventas-celular` · `descargar-un-boton`.
- 🔴 **LA HOJA DEL CLIENTE YA CARGA EN EL TELÉFONO (25-sep-2026, sin interruptor: es un defecto)**. 🩸 Tres toques, **CERO peticiones** a `historial-mensual` y 3 barras grises a los 30 s; en la computadora los mismos tres clientes cargaban. La causa: decidía si pedir con una variable que llenaba el actualizador de `setState`, y ése **solo corre en el acto** si la pantalla no tiene ya un cambio pendiente — en el teléfono siempre lo tiene (`setSheetCliente` del toque). Ahora decide un `ref`. Candado `hoja-cliente-carga`.
- 🔴 **EN EL CELULAR, COMISIONES SE REHIZO (25-sep-2026)**: `COMISIONES_CELULAR` (`lib/comisiones/celular.ts`, hoy `true`). El **total del mes arriba y chico** (🩸 la barra negra del pie se leía «$5,97…», tapada por el ☰) y **una fila** con la empresa y «‹ Ago 2026 ›», que **nunca va al futuro**; ⚙ · ⓘ · Descargar · Actualizar viven en el «···». 🔴 **En el detalle la PLATA va primero** (🩸 «% Util.» y «Comisión» se veían **0 px** de 69 y 93): no se quita ninguna columna. **Tasas** y **Descuentos** dejan lo demás para el toque (🩸 la fila de Reynaldo medía 165 px contra 57, y Descuentos se salía 265 px). 🔴 **Multifashion cierra con su propia barra «TOTAL A PAGAR · Multifashion»** ($255,27 en agosto) y **NUNCA se suma con el grupo** (barrido); «1 ticket» y «por la venta de la tienda», no «retail contra retail». 🔴 **«Descargar» es UN botón** (PDF del mes · Excel del mes · PDF de un vendedor). 🔴 **«Mandar» abre la hoja de compartir del teléfono con el PDF, como Guías** (25-sep): sin link, WhatsApp ni correo propio; en la computadora se descarga y el cajón `comisiones-papeles` **no existe**. 🔴 **Ningún número cambia: agosto 2026 sigue siendo $5.978,55.** Candados `comisiones-celular` · `mandar-comision`.
- 🔴 **«CLIENTES QUE NO COMISIONAN» SE DICE EN PALABRAS, SIN CASILLAS (25-sep-2026, celular y computadora)**. Daniel: *«¿la casilla llena significa que comisiona o no?»* — marcada quería decir **excluido**. 🩸 Eran **18 filas activas que son 12 reglas** en SEIS tablas (D-108 salía 6 veces). Hoy: UNA lista, un encabezado, «No comisiona **venta ni cobro**» / «**solo el cobro**» (D-81 · Edwin es la única), vendedor y empresas como chips —«Las 6 empresas» una sola vez—, dos desplegables, y el alta pregunta **«¿Qué no comisiona?»** con dos interruptores. 🔴 **La base no cambia ni una columna** (`comision_exclusion`, CHECK «al menos una», soft delete firmado). Candado `comisiones-exclusiones-en-palabras`.
- 🔴 **«MANDAR» LA COMISIÓN, DESDE EL DETALLE DEL VENDEDOR (25-sep-2026)**: Correo · WhatsApp · Copiar el link, con el **MISMO PDF** que baja «Descargar» (`construirPdfComision`; la ruta no dibuja papel). El correo **pide la dirección** —no se guarda el correo de ningún vendedor— y sale de `notificaciones@fashiongr.com` con el papel adjunto; el enlace se firma **30 días** en el cajón privado `comisiones-papeles`, y 🔴 **sin el cajón se dice que no se pudo, nunca se manda un link roto**. Queda rastro en `activity_logs`. Candado `mandar-comision`. ⚠️ **El cajón `comisiones-papeles` todavía NO existe**: hasta que se cree, «Correo» funciona y las otras dos avisan.
