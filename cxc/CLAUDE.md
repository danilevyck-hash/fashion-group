...
> **¿Qué foto de Storage está EN USO? — criterio en `src/lib/catalogos/fotos-en-uso.ts` (puro).** Pedido de Daniel: *"revisa todas mis fotos del catalogo de fashion shoes tommy, y borra solo las que no esten en uso"*. **Este módulo SOLO CLASIFICA — no borra, y el borrado no está implementado**: una foto borrada no vuelve y Daniel subió 389 a mano. Informe: `npx tsx scripts/_diag-fotos-tommy.ts [--lista]` (read-only; sirve para las 3 marcas con `MARCA=`), que le da datos de producción al MISMO módulo que cubre el test, así que informe y candado no pueden contradecirse.
...
> - **Inventario de Tommy al 30-jul-2026:** 2.667 archivos / 74,73 MB → **468 EN USO (16,31 MB) · 2.157 BANCO VIVO (52,74 MB) · 42 REEMPLAZADAS (5,67 MB) · 0 HUÉRFANAS**. Las 42 son 39 subidas el 29-jul y re-subidas ~8 min después (Daniel subió el lote dos veces) + 3 miniaturas de 7-11 KB del 27-jul. Pendiente el OK de Daniel para borrarlas.
