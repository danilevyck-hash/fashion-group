...
> - **Residual conocido, sin decidir:** `multifashion_overview_serie_v1(p_year)` devuelve la serie del AÑO completo, así que acotar `year`/`mes` limita QUÉ años se piden, no recorta el payload a un mes. Recortarlo vaciaría el gráfico anual del Resumen → es decisión de producto aparte.
> - Candado: `src/__tests__/lib/multifashion-ventana-gerente.test.ts` (49 tests). Llama a los 7 handlers REALES con cookie firmada rol por rol y mira qué parámetros llegan al RPC/Switch, congela el inventario de rutas y exige el clamp en toda ruta nueva. Verificado por mutación: poner el offset de Panamá en 0 rompe 4 tests, agregar una ruta sin clamp rompe 4, y saltarse el clamp en `bonos` rompe 1.
