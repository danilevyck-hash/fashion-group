...
> ### Medición y candados
>
> **Los 3 anchos (+ el iPad acostado), en el navegador contra el build de producción y con datos de producción** (`BASE=… GUIA_PENDIENTE=… node scripts/_medir-guias-entrega-directa.mjs`, solo lectura, **nunca toca "Despachar"**): **390 · 834 · 1024 · 1440 → 0 px de arrastre de página y 0 textos bajo 12 px** en los 20 casos (lista con la pendiente abierta · guía con transportista · la misma cambiada a entrega directa · los renglones con el cliente atado · guía nueva). Los recortes y los blancos táctiles que quedan son **PRE-EXISTENTES**: los 8 px del `-mx-2` de `SignatureCanvas`, el `w-40 truncate` del resumen de la fila, el input de búsqueda de 39 px y los campos densos de `pointer:fine` en escritorio. El script **falla** si no encuentra el botón "Despachar", la explicación de la entrega directa o la dirección sugerida primera.
>
> **Candados:** `guias-modo-despacho.test.ts` (17 + el PDF **generado de verdad** y leído), `guias-direccion-sugerida.test.ts`, **`guias-frecuencias-ruta.test.ts`** (llama al handler REAL) y **dos de CONDUCTA que PINTAN la pantalla**: `components/guias-entrega-directa.test.tsx` y `components/guias-direccion-primera.test.tsx`.
