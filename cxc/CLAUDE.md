...
> 🔴 **CUATRO MARCAS TAMBIÉN EN EL TELÉFONO (24-sep-2026, `MARCACION_CUATRO_MARCAS` en `lib/marcacion/cuatro-marcas.ts`, hoy `true`)** — Daniel: *«Cuatro marcas también en el teléfono»* · *«el botón va solo en orden»*. El botón las pide EN ORDEN —**entrada → salida a almuerzo → vuelta → salida**— y tras la cuarta queda gris; **nadie elige cuál es**. 🔴 **Foto en la ENTRADA y en la SALIDA; el ALMUERZO es UN SOLO TOQUE, sin cámara** —quién pide foto lo decide el SERVIDOR por el orden, no el teléfono—. **Ubicación en las cuatro**, «Deshacer» de 2 min sobre la última (la nombra), y la cola las aguanta. 🔴 **Lo que se guarda NO cambia**: el `tipo` sigue siendo `entrada`/`salida` alternado por el orden y el motor **ni lo lee** (asigna por ORDEN); ⚠️ lo único distinto es que el almuerzo cae con `foto_path` en **NULL**, columna nullable desde que nació y así están TODAS las del reloj. `false` = dos marcas. Candado `marcacion-cuatro-marcas`.
...
- 🔴 **GUARDAR UNA HORA NO BORRA LA TABLA NI SALTA ARRIBA (24-sep-2026)**: la recarga de después es SILENCIOSA —pastilla FIJA, la tabla con lo viejo—. 🔴 **La fila recién corregida no se va** aunque «Solo a revisar» la saque (chip «listo»). El POST no cambió. `ASISTENCIA_GUARDAR_SIN_SALTO`; candado `asistencia-guardar-sin-salto`.
- 🔴 **QUIEN NO MARCÓ EN EL PERÍODO APARECE IGUAL, EN GRIS (24-sep-2026)**: 🩸 dos colaboradoras no existían en su quincena y no había cómo arreglarles las horas. Sale con sus días **en cero y NINGUNO ausencia** —«Tú decides»— y se corrige por `correcciones/dia`. Excel y PDF igual. `sin-marcas.ts`, `ASISTENCIA_SIN_MARCAS_VISIBLE`; candado `asistencia-sin-marcas-visible`.
